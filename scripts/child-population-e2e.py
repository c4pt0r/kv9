#!/usr/bin/env python3
"""Real-process child-population gate: under the sealed parent fence, each
half of the parent's data is copied into its child group through the
child's OWN log in bounded committed batches, with independently
recomputed digests proving the copy exact and idempotent.
...
def main():
    ...
    parser.add_argument('--base-port', type=int, default=26980)
    ...
    env = dict(os.environ, KV9_CLUSTER_TOKEN='populate-e2e-cluster',
               KV9_CLIENT_TOKENS='admin=populate-e2e-client', KV9_CLIENT_TOKEN='populate-e2e-client',
               KV9_BOOTSTRAP_TOKEN='populate-e2e-bootstrap', KV9_STORAGE='minio')
    ...
    try:
        ...
        manifest_record.update(verdict='accepted', region=region,
                               checks=['intent and durable fence as previously qualified; population refuses before the seal',
                                       'each half copies into its child through the child log in committed batches',
                                       'independently recomputed digests prove both copies exact and the halves distinct',
                                       'population reruns converge, including after a child leader restart',
                                       'the parent stays fenced and the catalog directory stays untouched throughout'])
