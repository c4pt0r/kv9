#!/usr/bin/env python3
"""Real-process cross-range gate: after the published split, spans that
cross the child boundary serve in range-sized chunks — scans return both
halves' keys in order, delete-range clears across the boundary — with
per-chunk atomicity and the retired parent untouched.
...
def main():
    ...
    parser.add_argument('--base-port', type=int, default=27010)
    ...
    env = dict(os.environ, KV9_CLUSTER_TOKEN='xrange-e2e-cluster',
               KV9_CLIENT_TOKENS='admin=xrange-e2e-client', KV9_CLIENT_TOKEN='xrange-e2e-client',
               KV9_BOOTSTRAP_TOKEN='xrange-e2e-bootstrap', KV9_STORAGE='minio')
    ...
    try:
        ...
        # --- The published split retires the sealed parent EVERYWHERE.
        for n in (1, 2, 3):
            wait(f'node {n} retires its sealed parent automatically',
                 lambda n=n: data_groups(n).get(region, {}).get('state') == 'retired', 180)
            parent_dir = output / f'n{n}' / 'data-groups' / str(region)
            require(parent_dir.is_dir() and any(parent_dir.iterdir()),
                    'retirement must keep the durable parent storage')

        # Children keep serving; a further restart keeps the parent retired
        # and opens nothing.
        survivor = wait('leader among children after retirement',
                        lambda: next((n for n in (1, 2, 3) if n in processes
                                      and data_groups(n).get(low_region, {}).get('role') == 'Leader'),
                                     None))
        stop(survivor)
        start(survivor, 'post-retirement-restart', ticket=None)
        wait('restarted node ready', lambda: status(survivor).get('endpoint_ready') == 'true')
        wait('the parent stays retired after restart',
             lambda: data_groups(survivor).get(region, {}).get('state') == 'retired')
        wait('the children still serve after the retirement restart',
             child_serves('616c706861', '6f6e65'), 180)

        # --- Cross-range spans serve in chunks across the boundary.
        def scan_page(start_hex, end_hex, limit):
            for attempt in range(30):
                for n in (1, 2, 3):
                    if n not in processes:
                        continue
                    got = command('xscan', 'client', 'raw-scan', '--addr', addresses[n],
                                  '--keyspace', keyspace_id, '--start-hex', start_hex,
                                  '--end-hex', end_hex, '--limit', str(limit), success=None)
                    if got['exit_code'] == 0:
                        raw = open(output / max((p.name for p in output.iterdir()
                                                 if p.name.startswith('xscan-probe-')),
                                                key=lambda n: int(n.split('-')[-1].split('.')[0]))).read()
                        page = [l.split(' ')[0].split('=', 1)[1]
                                for l in raw.splitlines() if l.startswith('key_hex=')]
                        return page, got.get('resume_from', '')
                time.sleep(0.5)
            require(False, 'cross-range scan did not serve')

        def any_leader_scan(start_hex, end_hex, limit):
            # A page carries an explicit resume cursor when the walk paused
            # at a foreign-leader chunk (even over an empty page); otherwise
            # standard pagination continues past the last key.
            keys, cursor = [], start_hex
            while True:
                page, resume = scan_page(cursor, end_hex, limit - len(keys))
                keys.extend(page)
                if len(keys) >= limit:
                    return keys
                if resume:
        ...
        keys = any_leader_scan('', '', 100)
        require(keys == sorted(keys), 'the cross-range scan returns keys in order')
        require('616c706861' in keys and '7979' in keys,
                'the scan must return keys from BOTH halves')
        low_only = any_leader_scan('', '6d', 100)
        require(all(k < '6d' or not k for k in low_only) and '616c706861' in low_only,
                'a low-half scan stays below the boundary')

        # Cross-boundary delete-range: alpha (low) and yy (high) both go.
        def routed_delete_range(start_hex, end_hex):
            # Committed chunks stay committed; a paused walk hands back the
            # exact resume cursor and the client continues (idempotently) at
            # whichever node leads the next chunk.
            cursor = start_hex
            for round_ in range(60):
                ...
                for n in (1, 2, 3):
                    if n not in processes:
                        continue
                    got = command('xdelete', 'client', 'raw-delete-range', '--addr', addresses[n],
                                  '--keyspace', keyspace_id, '--start-hex', cursor,
                                  '--end-hex', end_hex, success=None)
                    if got['exit_code'] == 0:
                        if got.get('resume_from'):
                            cursor = got['resume_from']
                            progressed = True
                            break
                        return got
                if not progressed:
                    time.sleep(0.5)
            require(False, 'cross-range delete did not complete')

        routed_delete_range('616c', '7a')  # [al.., z): spans the boundary
        def gone(key_hex):
            def probe():
                for n in (1, 2, 3):
                    if n not in processes:
                        continue
                    got = command('xgone', 'client', 'raw-get', '--addr', addresses[n],
                                  '--keyspace', keyspace_id, '--key-hex', key_hex, success=None)
                    if got['exit_code'] == 0:
                        return got.get('found') == 'false'
                return False
            return probe
        wait('the low-half key is deleted across the boundary', gone('616c706861'), 120)
        wait('the high-half key is deleted across the boundary', gone('7979'), 120)
        still = any_leader_scan('', '', 100)
        require('7a7a' in still, 'keys outside the deleted span survive')

        manifest_record.update(verdict='accepted', region=region,
                               checks=['the full manual split and automatic parent retirement as previously qualified',
                                       'cross-range scans serve both halves in order through range-sized chunks',
                                       'bounded scans clamp at the boundary; limits are honored per chunk',
                                       'delete-range clears keys on BOTH sides of the boundary with per-chunk receipts',
                                       'keys outside the span survive; the retired parent stays untouched'])
