#!/usr/bin/env python3
"""Real-process removal gate: a committed decision naming one exact source
replica retires it from the voter set through the group's own log; three
voters (including the migrated destination) carry the range, and every
side restarts under committed configuration history.
...
def main():
    ...
    parser.add_argument('--base-port', type=int, default=26940)
    ...
    env = dict(os.environ, KV9_CLUSTER_TOKEN='removal-e2e-cluster',
               KV9_CLIENT_TOKENS='admin=removal-e2e-client', KV9_CLIENT_TOKEN='removal-e2e-client',
               KV9_BOOTSTRAP_TOKEN='removal-e2e-bootstrap', KV9_STORAGE='minio')
    ...
    try:
        ...
        # The removed node restarts as a healthy NON-MEMBER: its isolated
        # replica neither serves nor votes, and the survivors are undisturbed.
        stop(removed)
        ...
        wait('removed replica stays locally recovered',
             lambda: data_groups(removed).get(region, {}).get('state') in ('active', 'failed'))
        ...
        manifest_record.update(verdict='accepted', region=region,
                               checks=['settlement, compaction and promotion as previously qualified',
                                       'removal refuses without the committed decision; the destination refuses as target',
                                       'the exact committed replica leaves the voter set through the group log, idempotently',
                                       'three voters including the migrated destination keep committing writes',
                                       'the removed node restarts as a healthy isolated non-member'])
