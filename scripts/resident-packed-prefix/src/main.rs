...
use kv9_packed_prefix_experiment::PackedMap;
use packed_reference::PackedMap as OriginalPackedMap;
mod probes;
...
impl Index for OriginalPackedMap {
    fn put(&mut self, key: Vec<u8>, value: Vec<u8>) {
        self.insert(key, value);
    }
    fn get(&self, key: &[u8]) -> Option<&Vec<u8>> {
...
fn read_case<M: Index>(work: &Workload, op: &str, probes: &[Vec<u8>], passes: usize) -> Value {
    ...
    json!({"operation":op,"workload":work.name,"dataset":work.dataset,"pinned":false,"passes":passes,"queries":probes.len(),
...
fn write_backend(backend: &str, work: &Workload, pinned: bool, passes: usize) -> Value {
    match backend {
        "rpds" => write_case::<RedBlackTreeMapSync<Vec<u8>, Vec<u8>>>(work, pinned, passes),
        "packed" => write_case::<OriginalPackedMap>(work, pinned, passes),
        "prefix" => write_case::<PackedMap>(work, pinned, passes),
        _ => unreachable!(),
    }
}
fn read_backend(
    ...
    match backend {
        "rpds" => read_case::<RedBlackTreeMapSync<Vec<u8>, Vec<u8>>>(work, op, probes, passes),
        "packed" => read_case::<OriginalPackedMap>(work, op, probes, passes),
        "prefix" => read_case::<PackedMap>(work, op, probes, passes),
        _ => unreachable!(),
    }
}
fn main() {
    let args: Vec<_> = std::env::args().collect();
    assert_eq!(args.len(), 4, "artifact-root fresh-output prepare|measure");
    ...
    let orders = ["rpds", "packed", "prefix", "prefix", "packed", "rpds"];
    ...
    if args[3] == "prepare" {
        for work in &works {
            for pinned in [false, true] {
                ...
                check_workload::<OriginalPackedMap>(work, pinned);
        ...
        emit(
            &output,
            &json!({"complete":true,"mode":"prepare","input_plan_sha256":hash(&plan_bytes),"group_file_sha256":hash(&group_bytes),"workloads":metadata,"read_inputs":inputs,"correctness_live_prefixes":5724,"correctness_old_views":2862}),
        );
        return;
    }
        ...
        for pinned in [false, true] {
            if !cfg!(feature = "allocation-counting") {
                for backend in ["rpds", "packed", "prefix"] {
                    black_box(write_backend(backend, work, pinned, 1));
                }
            }
            for (order, backend) in orders.into_iter().enumerate() {
                let mut row = write_backend(backend, work, pinned, passes);
                row["backend"] = json!(backend);
                row["order"] = json!(order);
                println!(
                    "{}",
                    json!({"event":"write_row_complete","dataset":work.dataset,"workload":work.name,"pinned":pinned,"backend":backend,"order":order})
                );
                rows.push(row);
            }
        }
    }
    for work in works.iter().filter(|w| w.name != "initial_fill") {
        for op in ["get_hit", "get_miss", "predecessor", "scan16"] {
            let probes = queries(work, op);
            if !cfg!(feature = "allocation-counting") {
                for backend in ["rpds", "packed", "prefix"] {
                    black_box(read_backend(backend, work, op, &probes, 1));
                }
            }
            for (order, backend) in orders.into_iter().enumerate() {
                let mut row = read_backend(backend, work, op, &probes, passes);
                row["backend"] = json!(backend);
                row["order"] = json!(order);
                rows.push(row);
            }
    ...
    emit(
        ...
        "scope":"Standalone index calls only. Owned key/value clones in writes; borrowed point/predecessor returns; owned scan16. Per-operation timer overhead remains. No engine, WAL, Raft, RPC or database QPS. Allocation counters report requested bytes, not RSS/usable sizes."}),
    );
    println!(
        "{}",
        json!({"complete":true,"rows":252,"counting_build":cfg!(feature="allocation-counting")})
    );
}
