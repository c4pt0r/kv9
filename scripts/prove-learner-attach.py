#!/usr/bin/env python3
"""Check the unified-cut learner-attach model and explicit defect controls."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    contract = json.loads(
        (root / "proofs/lean/learner-attach/source-contract.json").read_text()
    )
    assert contract["schema"] == 1
    for name, digest in contract["source_sha256"].items():
        assert hashlib.sha256((root / name).read_bytes()).hexdigest() == digest, name
    source = (root / "proofs/lean/learner-attach/Attach.lean").read_text()
    ...
    def compile_source(label, text, success):
        ...
        directory.mkdir()
        path = directory / "Attach.lean"
        path.write_text(text)
        command = [args.lean, "-DwarningAsError=true", "-o", str(directory / "Attach.olean"), str(path)]
    ...
    audit.write_text("import Attach\n" + "\n".join(
        f"#print axioms Kv9.LearnerAttach.{name}" for name in contract["theorems"]) + "\n")
    ...
    for name in contract["theorems"]:
        line = next(line for line in result.stdout.splitlines()
                    if f"Kv9.LearnerAttach.{name}'" in line)
    ...
    controls = [
        ('attach-without-intent',
         '(committed : s.intent = true) (fresh : s.voter = false)',
         '(committed : s.intent = s.intent) (fresh : s.voter = false)'),
        ('attach-without-cut-advance',
         '{s with learner := true, cutAdvanced := true}',
         '{s with learner := true}'),
        ('attach-promotes-a-voter',
         '{s with learner := true, cutAdvanced := true}',
         '{s with learner := true, cutAdvanced := true, voter := true}'),
        ('attach-grants-serving',
         '{s with learner := true, cutAdvanced := true}',
         '{s with learner := true, cutAdvanced := true, serving := true}'),
        ('confirm-without-learner',
         '| confirm {s} (attached : s.learner = true) :',
         '| confirm {s} (attached : s.learner = s.learner) :'),
        ('confirm-detaches',
         '| confirm {s} (attached : s.learner = true) :\n      Step s {s with cutAdvanced := true}',
         '| confirm {s} (attached : s.learner = true) :\n      Step s {s with cutAdvanced := true, learner := false}'),
        ('commit-serves',
         '| commit {s} : Step s {s with intent := true}',
         '| commit {s} : Step s {s with intent := true, serving := true}'),
    ]
    ...
    print(f"LEARNER_ATTACH_PROOF_ACCEPTED: {len(contract['theorems'])} theorems; {len(controls)} semantic and 2 policy controls")
