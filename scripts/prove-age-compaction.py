#!/usr/bin/env python3
"""Check the age-compaction model and explicit defect controls."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    contract = json.loads(
        (root / "proofs/lean/age-compaction/source-contract.json").read_text()
    )
    assert contract["schema"] == 1
    for name, digest in contract["source_sha256"].items():
        assert hashlib.sha256((root / name).read_bytes()).hexdigest() == digest, name
    source = (root / "proofs/lean/age-compaction/AgeCompaction.lean").read_text()
    ...
    def compile_source(label, text, success):
        ...
        directory.mkdir()
        path = directory / "AgeCompaction.lean"
        path.write_text(text)
        command = [args.lean, "-DwarningAsError=true", "-o", str(directory / "AgeCompaction.olean"), str(path)]
    ...
    audit.write_text("import AgeCompaction\n" + "\n".join(
        f"#print axioms Kv9.AgeCompaction.{name}" for name in contract["theorems"]) + "\n")
    ...
    for name in contract["theorems"]:
        line = next(line for line in result.stdout.splitlines()
                    if f"Kv9.AgeCompaction.{name}'" in line)
    ...
    controls = [
        ('a-proposal-without-aging',
         '| propose {s} (a : s.aged = true) (r : s.hasRetained = true) (fresh : s.committed = false) :',
         '| propose {s} (a : s.aged = s.aged) (r : s.hasRetained = true) (fresh : s.committed = false) :'),
        ('a-proposal-without-something-to-compact',
         '| propose {s} (a : s.aged = true) (r : s.hasRetained = true) (fresh : s.committed = false) :',
         '| propose {s} (a : s.aged = true) (r : s.hasRetained = s.hasRetained) (fresh : s.committed = false) :'),
    ...
    report = {
        ...
        "compacts_an_empty_window_on_age": False, "new_chaos_acceptance": False, "new_qps": False,
    }
    (output / "result.json").write_text(json.dumps(report, indent=2) + "\n")
    print(f"AGE_COMPACTION_PROOF_ACCEPTED: {len(contract['theorems'])} theorems; {len(controls)} semantic and 2 policy controls")
