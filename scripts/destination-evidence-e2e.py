#!/usr/bin/env python3
"""Real-process destination-evidence gate: the adopted destination commits
install evidence, and only that committed row unlocks the source pin's
quiesce and release through the unchanged retention ledger.
...
def main():
    ...
    parser.add_argument('--base-port', type=int, default=26910)
    ...
    env = dict(os.environ, KV9_CLUSTER_TOKEN='evidence-e2e-cluster',
               KV9_CLIENT_TOKENS='admin=evidence-e2e-client', KV9_CLIENT_TOKEN='evidence-e2e-client',
               KV9_BOOTSTRAP_TOKEN='evidence-e2e-bootstrap', KV9_STORAGE='minio')
    ...
    try:
        ...
        manifest_record.update(verdict='accepted', region=region,
                               checks=['adoption and retained-tail catchup as previously qualified',
                                       'quiesce and release refuse before any committed evidence',
                                       'the destination replays its durable receipt with the exact image and cut',
                                       'a divergent subject refuses at commit; the identical receipt confirms idempotently',
                                       'committed evidence unlocks source quiesce then release through the unchanged ledger',
                                       'the destination pin never drops and the replica keeps tracking the tail'])
