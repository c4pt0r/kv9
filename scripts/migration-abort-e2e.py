#!/usr/bin/env python3
"""Real-process migration-abort gate: the stranded-destination recovery.
...
def main():
    ...
    parser.add_argument('--base-port', type=int, default=27100)
    ...
    env = dict(os.environ, KV9_CLUSTER_TOKEN='abort-e2e-cluster',
               KV9_CLIENT_TOKENS='admin=abort-e2e-client', KV9_CLIENT_TOKEN='abort-e2e-client',
               KV9_BOOTSTRAP_TOKEN='abort-e2e-bootstrap', KV9_STORAGE='minio')
    ...
    try:
        ...
        command('mid-put', 'client', 'raw-put', '--addr', addresses[node],
        ...
        command('revoke-4', 'client', 'revoke-admission', '--addr', addresses[owner_node],
                '--node-id', 4)
        admitted = command('admit-node-4b', 'client', 'admit-node', '--addr', addresses[owner_node],
                           '--node-id', 4, '--node-addr', addresses[4], '--ttl-seconds', 600)
        ...
        command('migrate-2', 'client', 'migrate-data-group', '--addr', addresses[owner_node],
        ...
        command('bind-2', 'client', 'bind-migration-image', '--addr', addresses[owner_node],
                '--root-digest', root, '--operation-id', f'{11:032x}',
