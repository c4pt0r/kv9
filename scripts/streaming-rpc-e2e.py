#!/usr/bin/env python3
"""Check normal-port streaming and unary RPC with three WAL voters and complete histories."""
import argparse
import hashlib
import json
...
def normal_build(build):
    ...
    if (digest(build / "kv9") != manifest["binaries"]["kv9"]["sha256"] or
        digest(build / "workload/kv9-workload") != workload["binary_sha256"] or
    ...
    for name, path, command, wanted in (
        ...
        ("workload", build / "workload/cargo.jsonl", sources["command"],
         ("kv9-workload", "kv9_engine", "kv9_raft", "kv9_server")),
    ):
...
def main():
    ...
    bound_paths = [build / name for name in ("build.json", "kv9", "kv9-cargo.jsonl",
        "workload/build.json", "workload/kv9-workload", "workload/cargo.jsonl", "workload/sources.json")]
    ...
    summary = dict(complete=False, cases=[], lifetimes=[], server_sha256=server_hash,
        ...
        helper_sources={name: digest(Path(__file__).parent / name) for name in
            ("streaming-rpc-e2e.py", "benchmark.py", "workload_report.py", "root_provision.py",
             "wal_layout.py", "history/checker.py")},
        scope="Point GET/PUT/DELETE only: normal standalone build, shared advertised endpoint, default streaming and explicit unary; local three-process Raft correctness with ordinary WAL storage. No batch API, throughput, Chaos Mesh, cross-host or power-loss claim")
    ...
    try:
        ...
        for transport in ("tonic_stream", "tonic_unary"):
            ...
            if report["version"] != 2 or report["configuration"] != dict(configuration, rpc_transport=transport):
                raise ValueError("normal workload did not record its exact version 2 transport/configuration")
        ...
        summary["owned_children"] = [dict(**fixture.identities[process.pid], exit_code=process.poll())
        ...
        summary["complete"] = (summary.get("workloads_complete", False) and summary["owned_lifetimes_exited"]
                               and summary["owned_children_exited"] and summary["bound_artifacts_unchanged"])
    ...
    print("PASS: default streaming and explicit unary retain checked histories on the ordinary port across leader restart", flush=True)
