#!/usr/bin/env python3
"""Check the deferred-sync model and explicit defect controls."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    contract = json.loads(
        (root / "proofs/lean/deferred-sync/source-contract.json").read_text()
    )
    assert contract["schema"] == 1
    for name, digest in contract["source_sha256"].items():
        assert hashlib.sha256((root / name).read_bytes()).hexdigest() == digest, name
    source = (root / "proofs/lean/deferred-sync/DeferredSync.lean").read_text()
    ...
    def compile_source(label, text, success):
        ...
        directory.mkdir()
        path = directory / "DeferredSync.lean"
        path.write_text(text)
        command = [args.lean, "-DwarningAsError=true", "-o", str(directory / "DeferredSync.olean"), str(path)]
    ...
    audit.write_text("import DeferredSync\n" + "\n".join(
        f"#print axioms Kv9.DeferredSync.{name}" for name in contract["theorems"]) + "\n")
    ...
    for name in contract["theorems"]:
        line = next(line for line in result.stdout.splitlines()
                    if f"Kv9.DeferredSync.{name}'" in line)
    ...
    controls = [
        ('an-ack-without-the-durable-raft-entry',
         '| ack {s} (r : s.raftDurable = true) (w : s.engineWritten = true) :',
         '| ack {s} (r : s.raftDurable = s.raftDurable) (w : s.engineWritten = true) :'),
        ('the-crash-keeps-the-unsynced-tail',
         '| crash {s} : Step s {s with crashed := true, engineWritten := s.engineSynced}',
         '| crash {s} : Step s {s with crashed := true}'),
        ('compaction-discards-the-replay-source-under-deferral',
         '| compact {s} (synced : s.engineSynced = true) : Step s s',
         '| compact {s} (synced : s.engineSynced = s.engineSynced) :\n      Step s {s with compactedUnderDeferral := true}'),
        ('the-sync-covers-an-unwritten-record',
         '| syncEngine {s} (w : s.engineWritten = true) :',
         '| syncEngine {s} (w : s.engineWritten = s.engineWritten) :'),
        ('a-crash-drops-the-acknowledgement',
         '| crash {s} : Step s {s with crashed := true, engineWritten := s.engineSynced}',
         '| crash {s} :\n      Step s {s with crashed := true, engineWritten := s.engineSynced, acked := false}'),
    ]
    ...
    report = {
        ...
        "verified_rust_extraction": False, "lost_ack_or_unsynced_compaction": False,
        "new_chaos_acceptance": False, "new_qps": False,
    }
    (output / "result.json").write_text(json.dumps(report, indent=2) + "\n")
    print(f"DEFERRED_SYNC_PROOF_ACCEPTED: {len(contract['theorems'])} theorems; {len(controls)} semantic and 2 policy controls")
