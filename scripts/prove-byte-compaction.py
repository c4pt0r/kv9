#!/usr/bin/env python3
"""Check the byte-compaction model and explicit defect controls."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    contract = json.loads(
        (root / "proofs/lean/byte-compaction/source-contract.json").read_text()
    )
    assert contract["schema"] == 1
    for name, digest in contract["source_sha256"].items():
        assert hashlib.sha256((root / name).read_bytes()).hexdigest() == digest, name
    source = (root / "proofs/lean/byte-compaction/ByteCompaction.lean").read_text()
    ...
    def compile_source(label, text, success):
        ...
        directory.mkdir()
        path = directory / "ByteCompaction.lean"
        path.write_text(text)
        command = [args.lean, "-DwarningAsError=true", "-o", str(directory / "ByteCompaction.olean"), str(path)]
    ...
    audit.write_text("import ByteCompaction\n" + "\n".join(
        f"#print axioms Kv9.ByteCompaction.{name}" for name in contract["theorems"]) + "\n")
    ...
    for name in contract["theorems"]:
        line = next(line for line in result.stdout.splitlines()
                    if f"Kv9.ByteCompaction.{name}'" in line)
    ...
    controls = [
        ('a-proposal-without-grown-bytes',
         '| propose {s} (b : s.bytesGrown = true) (fresh : s.committed = false) :',
         '| propose {s} (b : s.bytesGrown = s.bytesGrown) (fresh : s.committed = false) :'),
        ('a-proposal-fires-on-entries-alone',
         '| propose {s} (b : s.bytesGrown = true) (fresh : s.committed = false) :',
         '| propose {s} (b : s.entriesGrown = true) (fresh : s.committed = false) :'),
    ...
    report = {
        ...
        "byte_trigger_fires_on_entries_alone": False, "new_chaos_acceptance": False, "new_qps": False,
    }
    (output / "result.json").write_text(json.dumps(report, indent=2) + "\n")
    print(f"BYTE_COMPACTION_PROOF_ACCEPTED: {len(contract['theorems'])} theorems; {len(controls)} semantic and 2 policy controls")
