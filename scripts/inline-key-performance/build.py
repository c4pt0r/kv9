...
assert sha(Q/'candidate/crates/engine/src/mem_key.rs')==declared['candidate_key_sha256']
...
try:
    for profile in ('timing','counting'):
        for arm in ('baseline','candidate'):
            ...
            paths=[Path(__file__).resolve(),R/'scripts/inline-key-performance/prepare.py',S/'plan.json',S/'declared-plan.json',S/'harness-correspondence.json',work/'Cargo.toml',work/'Cargo.lock',*sorted((work/'src').glob('*.rs'))]
            ...
            with cache.BuildCache(work,out,True,sources) as build:
                ...
                shutil.copyfile(R/('target/release/kv9-inline-key-'+profile),binary);binary.chmod(0o755)
