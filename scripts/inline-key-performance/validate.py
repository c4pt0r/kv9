...
for case,spec in enumerate(plan['cases']):
    ...
    inline=dataset=='original';cost_delta=24-key_len if inline else 24
    for i,(b,c) in enumerate(zip(raws['baseline'],raws['candidate'])):
        ...
        bc,cc=b[0],c[0]
        expected=[bc[0]-(n if inline else 0),bc[1]+n*cost_delta,0,0,bc[4]-(freed if inline else 0),bc[5]+freed*cost_delta]
    ...
    assert footprints['candidate']-footprints['baseline']==len(models[dataset,workload])*cost_delta+2*(24-len(b'engine-interface-sentinel'))
