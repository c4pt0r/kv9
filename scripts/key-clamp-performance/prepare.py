...
Q=Path('/mnt/data/kv9-work/key-clamp-qualification-20260916-first')
sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
load=lambda p:json.loads(p.read_text())
declared=load(Q/'declared-plan.json');timing=declared['timing_after_qualification']
assert load(Q/'qualification-audit.json')['complete']
roots=load(Q/'preparation.json')['qualified_source_roots']
assert sha(Path(roots['clamped_candidate'])/'crates/engine/src/entry_buffer.rs')==declared['candidate_key_sha256']
for p,h in load(Q/'sources.json').items():assert sha(Path(p))==h,p
shutil.copyfile(Q/'declared-plan.json',S/'declared-plan.json')
previous=load(R/'docs/entry-buffer-qualification-v1/next-performance-plan.json')
shutil.copyfile('/mnt/data/kv9-work/outlined-mutation-path-20260916-first/groups.bin',S/'groups.bin')
assert sha(S/'groups.bin')==previous['corpus']['sha256']
plan=dict(declared_plan_sha256=sha(S/'declared-plan.json'),execution_base=subprocess.check_output(['git','rev-parse','HEAD'],cwd=R,text=True).strip(),
          qualified_root=str(Q),source_roots=roots,arms=timing['arms'],orders=timing['orders'],write_passes=12,write_excluded_passes=1,
          ...
          corpus_sha256=previous['corpus']['sha256'],cpu=4,helper_cpus='6-15,22-31',
          process_timeout_seconds=180,process_output_max_bytes=67108864,minimum_free_bytes=8589934592,
          expected_prepare_processes=6,expected_counting_processes=24,expected_timing_processes=48,
          ...
          scopes=timing['scope'],stage_two=previous['stage_two'],
...
assert plan['execution_base']=='b23506dc5ee43d52b3a82a0c7f2b3d5abe0003ad'
assert plan['arms']==['baseline','single_buffer_control','clamped_candidate']
assert plan['orders']==plan['arms']+list(reversed(plan['arms']))
...
def transform(source):
    source=replace(source,'json!(["baseline", "candidate", "candidate", "baseline"])',
        'json!(["baseline", "single_buffer_control", "clamped_candidate", "clamped_candidate", "single_buffer_control", "baseline"])')
    source=replace(source,'assert!(order < 4);','assert!(order < 6);')
...
'''
    ...
    manifest=(R/f'scripts/{origin}/Cargo.toml').read_text().replace('kv9-'+origin,'kv9-key-clamp-'+profile)
    ...
    assert old==new
correspondence=dict(complete=True,operation_transform='Same shared dataset/refusal/three-arm configuration transformation in both inherited harnesses; unchanged write loops and windows. Count-only final footprint observation is outside all windows.',
