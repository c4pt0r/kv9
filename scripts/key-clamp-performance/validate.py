...
raw=(S/'groups.bin').read_bytes();assert hashlib.sha256(raw).hexdigest()==plan['corpus_sha256']=='d978ba9e49131f6277f975ecd333c33854845cc03bd14ee57a22fff915a08350'
...
accepted=dict(complete=True,models=metadata,prepared_sha256=prepared,live_prefixes=5088,old_views=2544,refusal_checks=10176,independent_reconstruction=True,query_identities=query_identity)
summary('prepare',6)
if mode=='prepare':
...
summary('counting',24);cells=[]
for case,spec in enumerate(plan['cases']):
    result={};raws={}
    for order,arm in enumerate(plan['arms']):
    ...
    assert raws['clamped_candidate']==raws['single_buffer_control'],('allocation control mismatch',case)
    ...
    for i,(b,c) in enumerate(zip(raws['baseline'],raws['single_buffer_control'])):
    ...
    assert footprints['clamped_candidate']==footprints['single_buffer_control']
    assert footprints['clamped_candidate']-footprints['baseline']==(len(models[dataset,workload])+2)*cost_delta
...
allocation=dict(complete=True,accepted=True,rows=24,cells=cells,windows_compared=16*1272,control_equivalence_windows=8*1272,elapsed_time_recorded=False,input_acceptance=accepted,
...
if mode=='counting':
    ...
    print(json.dumps({'accepted':True,'rows':24,'windows_compared':16*1272,'control_equivalence_windows':8*1272}));sys.exit(0)
assert load(S/'allocation-analysis.json')==allocation
summary('timing',48);comparisons=[];failures=[]
for case,spec in enumerate(plan['cases']):
    orders={}
    ...
    baseline=metric(orders[0]+orders[5]);control=metric(orders[1]+orders[4]);candidate=metric(orders[2]+orders[3])
    delta=lambda b,c:{k:(c[k]/b[k]-1)*100 for k in ('mean_ns','p99_ns')}
    pooled=delta(baseline,candidate);by_order=[delta(metric(orders[b]),metric(orders[c])) for b,c in ((0,2),(5,3))]
    control_effect=dict(pooled=delta(control,candidate),by_order=[delta(metric(orders[b]),metric(orders[c])) for b,c in ((1,2),(4,3))])
    failed=[]
    ...
    row=dict(case=case,spec=spec,unit='ns_per_group',baseline=baseline,single_buffer_control=control,clamped_candidate=candidate,
             percent_change=pooled,by_order=by_order,accessor_control_effect=control_effect,failed_metrics=failed)
    comparisons.append(row)
    if failed:failures.append(dict(case=case,metrics=failed))
assert len(terminals)==78
result=dict(complete=True,processes=78,timing_rows=48,counting_rows=24,comparisons=comparisons,allocations_accepted=True,
            ...
            production_promoted=False,limits='Shared-host engine component, three arms and two order directions; not WAL/Raft/RPC/database QPS or Redis comparison.')
