#!/usr/bin/env python3
"""Build six separately identified engine-interface executables from qualified sources."""
import hashlib
...
assert plan['orders']==declared['timing_after_qualification']['orders'] and len(plan['cases'])==declared['timing_after_qualification']['cases']
roots={arm:Path(path) for arm,path in plan['source_roots'].items()}
assert sha(roots['clamped_candidate']/'crates/engine/src/entry_buffer.rs')==declared['candidate_key_sha256']
qualified=load(Q/'sources.json')
for p,h in qualified.items():assert sha(Path(p))==h,p
for path,h in load(R/'proofs/lean/key-clamp/source-contract.json')['sources'].items():
    root,rel=path.split('/',1);p=(R if root=='repo' else Q)/rel;assert sha(p)==h;qualified[str(p)]=h
for p in [Q/'qualification-audit.json',Q/'proof/result.json',Q/'tests-summary.json',Q/'layout-summary.json',R/'proofs/lean/key-clamp/source-contract.json']:
    qualified[str(p)]=sha(p)
...
result=dict(complete=False,started_ns=time.time_ns(),plan_sha256=sha(S/'plan.json'),sources={},arms={arm:{} for arm in plan['arms']})
try:
    for profile in ('timing','counting'):
        for arm in plan['arms']:
            ...
            for name in ('engine','common'):
                manifest=manifest.replace('../../crates/'+name,str(roots[arm]/'crates'/name))
            ...
            paths=[Path(__file__).resolve(),R/'scripts/key-clamp-performance/prepare.py',S/'plan.json',S/'declared-plan.json',S/'harness-correspondence.json',work/'Cargo.toml',work/'Cargo.lock',*sorted((work/'src').glob('*.rs'))]
            ...
            with cache.BuildCache(work,out,True,sources) as build:
                ...
                for name in ('kv9_engine','kv9_common','rpds','archery','triomphe'):
                    ...
                    if name in ('kv9_engine','kv9_common'):
                        assert not selected[0]['fresh'] and str(roots[arm]) in selected[0]['package_id']
                    else:assert 'registry+' in selected[0]['package_id']
                binary=out/'engine-interface'
                shutil.copyfile(R/('target/release/kv9-key-clamp-'+profile),binary);binary.chmod(0o755)
