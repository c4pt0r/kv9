#!/usr/bin/env python3
"""Execute the fixed three-arm key-accessor write screen; no read stage runs after a write failure."""
import hashlib
import json
import os
...
try:
    if stage == 'prepare':
        ...
        sequence = plan['orders'] if stage == 'timing' else plan['arms']
    ...
    assert len(summary['processes']) == {'prepare': 6, 'counting': 24, 'timing': 48}[stage]
