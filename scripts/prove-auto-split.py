#!/usr/bin/env python3
"""Check the auto-split model and explicit defect controls."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    contract = json.loads(
        (root / "proofs/lean/auto-split/source-contract.json").read_text()
    )
    assert contract["schema"] == 1
    for name, digest in contract["source_sha256"].items():
        assert hashlib.sha256((root / name).read_bytes()).hexdigest() == digest, name
    source = (root / "proofs/lean/auto-split/AutoSplit.lean").read_text()
    ...
    def compile_source(label, text, success):
        ...
        directory.mkdir()
        path = directory / "AutoSplit.lean"
        path.write_text(text)
        command = [args.lean, "-DwarningAsError=true", "-o", str(directory / "AutoSplit.olean"), str(path)]
    ...
    audit.write_text("import AutoSplit\n" + "\n".join(
        f"#print axioms Kv9.AutoSplit.{name}" for name in contract["theorems"]) + "\n")
    ...
    for name in contract["theorems"]:
        line = next(line for line in result.stdout.splitlines()
                    if f"Kv9.AutoSplit.{name}'" in line)
    ...
    controls = [
        ('trigger-without-a-breach',
         '(b : s.bound = true) (o : s.breached = true)',
         '(b : s.bound = true) (o : s.breached = s.breached)'),
        ('trigger-without-a-bound-range',
         '(b : s.bound = true) (o : s.breached = true)',
         '(b : s.bound = s.bound) (o : s.breached = true)'),
        ('trigger-twice',
         'Step s {s with triggered := true}',
         'Step s {s with triggered := true, secondTrigger := true}'),
        ('drive-without-the-trigger',
         '| drive {s} (t : s.triggered = true) : Step s {s with pipelineDone := true}',
         '| drive {s} (t : s.triggered = s.triggered) : Step s {s with pipelineDone := true}'),
        ('drive-skips-a-stage',
         '| drive {s} (t : s.triggered = true) : Step s {s with pipelineDone := true}',
         '| drive {s} (t : s.triggered = true) : Step s {s with pipelineDone := true, stageSkipped := true}'),
    ]
    ...
    report = {
        ...
        "verified_rust_extraction": False, "trigger_without_breach_or_skipped_stage": False,
        "new_chaos_acceptance": False, "new_qps": False,
    }
    (output / "result.json").write_text(json.dumps(report, indent=2) + "\n")
    print(f"AUTO_SPLIT_PROOF_ACCEPTED: {len(contract['theorems'])} theorems; {len(controls)} semantic and 2 policy controls")
