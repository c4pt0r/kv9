"""Same-artifact unary/tarpc/stream/tarpc/unary bracket with paired Redis and volatile tmpfs."""
...
BUILD=Path('/tmp/kv9-stream-release-first')
REDIS=Path('/tmp/kv9-redis-reference-build-v2/kv9-redis-reference')
OUT=Path('/tmp/kv9-stream-comparison-first');OUT.mkdir(exist_ok=False)
...
protocol=dict(version=1,diagnostic_only=True,volatile=True,power_loss_durability=False,revision=REV,
 ...
 order=['tonic-before','tarpc-before','stream','tarpc-after','tonic-after'],mixes=['read','write','mixed'],
 ...
 scope='Repeated same-artifact RPC framework diagnostic on one shared host with volatile tmpfs; no durable/cross-host/Chaos or production acceptance. The same new experimental client is used in all KV9 arms; no old client relabeling.',
...
class ExperimentFixture(rpc.RpcFixture,tmpfs.TmpfsFixture):
 ...
 def launch(self,command,logfile,client=False):
  if client:
   ...
   endpoints={'tonic_unary':self.addresses,'tarpc_tcp':self.rpc_addresses,'tonic_stream':self.stream_addresses}[self.transport]
   c['client']['peers']=[dict(node_id=n,address=endpoints[n]) for n in self.nodes]
   save(path,c)
...
try:
 for arm,transport in [('tonic-before','tonic_unary'),('tarpc-before','tarpc_tcp'),('stream','tonic_stream'),('tarpc-after','tarpc_tcp'),('tonic-after','tonic_unary')]:
  f=ExperimentFixture(OUT/arm,transport);fixture_record=dict(arm=arm,complete=False);result['fixtures'].append(fixture_record)
  try:
   f.start();save(f.out/'listener-evidence-before.json',{str(n):f.listener_evidence(n) for n in f.nodes});f.trial('guard-before','mixed',4,0,protocol,guard=True)
   ...
   save(f.out/'listener-evidence-after.json',{str(n):f.listener_evidence(n) for n in f.nodes});f.finish();fixture_record['workloads_complete']=True
  finally:
   ...
   save(OUT/'matrix.json',result)
 assert len(result['attempts'])==60 and all(e['complete'] for e in result['attempts'])
...
print('PASS: complete 60-cohort unary/tarpc/stream/tarpc/unary bracket with paired Redis; independent final audit still required',flush=True)
