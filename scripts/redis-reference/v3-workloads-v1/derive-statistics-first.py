...
def merge_histograms(raws):
    ...
    count=total=0
    for raw in raws:
        assert len(raw['buckets'])==len(buckets) and sum(raw['buckets'])==raw['count']
...
def aggregate(cases,reports):
    ...
    return dict(calls=calls,input_items=items,issued=sum(c['issued'] for c in cases),
        ...
        whole_call_latency=hist,operations=operations)
...
lines=['# V3 point and batch64 workload diagnostic readout','',
       ...
       '| Workers | Workload | Role | Pooled calls/s | Pooled keys/s | Mean µs | p95 µs | p99 µs |',
       '|---:|---|---|---:|---:|---:|---|---|']
for row in pooled:
    h=row['whole_call_latency'];lines.append(f"| {row['concurrency']} | {row['workload']} | {row['role']} | {row['completed_calls_per_second']:.3f} | {row['completed_input_items_per_second']:.3f} | {h['mean_ns']/1000:.3f} | {qtext(h,95)} | {qtext(h,99)} |")
