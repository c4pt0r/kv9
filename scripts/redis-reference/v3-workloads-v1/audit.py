#!/usr/bin/env python3
"""Independent 72-cohort v3 read/write diagnostic readback; no workload execution."""
import argparse
import base64
...
parser.add_argument('--successful-arms', type=int, choices=(72,), required=True)
...
NEW_REV = '57ff6851e40ed63c837189d6eb0a11190704725a'
REDIS_REV = CLIENT_REV
REDIS_SOURCE = SOURCE
PINS = {
 'old': (OLD_REV, '0d5ffa081482945d213b88aef12222afab44a44ed03bf46cf27bd292db7b1711'),
 'new': (NEW_REV, '6d76779a3e421388692b01ce08bd289477283fc05fd28b734eaed9138e0f7e30'),
...
PROTOCOL_ID = 'kv9-v3-workloads-c1-c64-v1'
CONCURRENCIES = (1, 64)
WORKLOADS = (('point-r000',1,0), ('point-r050',1,50), ('point-r100',1,100),
             ('batch64-r000',64,0), ('batch64-r050',64,50), ('batch64-r100',64,100))
ROLES = ('old', 'new', None)
ROLE_PATHS = {
 'old': ('/tmp/kv9-rpc-worker-pair', '/tmp/kv9-rpc-pair-release-first', 'kv9'),
 'new': ('/tmp/kv9-read-group-credit', '/tmp/kv9-read-group-credit-release-first', 'kv9'),
...
SERVER_MANIFESTS = {
 'old': '1bc2c98587f344f0066033450018d0a4c81b4f16a5c25a691b18136659ebe417',
 'new': '7e5179f856b8828c26a6bf68ce0056755e16e12bbdc35c9a11845178712caabd',
}
...
def validate_matrix_protocol(matrix):
    ...
    require(len(matrix['attempts'])==72 and same_json(matrix['cohort_inventory'],expected),
            'seventy-two-cohort inventory/order differs')
...
result = dict(complete=False, matched_diagnostic_accepted=False, cases=[],
              scope='Two-repeat 10-second v3 point/batch64 read/write c1/c64 shared-host tmpfs diagnostic; no sustained-capacity, equal-durability, exact issued nonce ledger or full-history claim',
...
try:
    ...
    require(args.successful_arms==72 and args.root_exit_code==0, 'expected complete diagnostic scope differs')
    ...
    expected_argv=['/usr/bin/python3','/tmp/kv9-v3-workload-comparison-preparation/matched-driver.py',
                   ...
                   '--old-server-source','/tmp/kv9-rpc-worker-pair',
                   '--old-server-build','/tmp/kv9-rpc-pair-release-first',
                   '--new-server-source','/tmp/kv9-read-group-credit',
                   '--new-server-build','/tmp/kv9-read-group-credit-release-first',
    ...
    require(invocation['argv']==expected_argv and invocation['wrapper_sha256']==
            ...
            invocation['driver_arguments_sha256']=='ecf364a3d10671c1c5749d6e45b6f7b4ec751a9f63c557c393adb697ba1393b1',
    ...
    require(matrix['helper_hashes'][str(Path('/tmp/kv9-v3-workload-comparison-preparation/matched-driver.py'))]==args.expected_driver_sha256,
    ...
    require(len(result['cases'])==72 and len(lifetimes)==240 and drains==144 and writer_bindings==144,
            'full cohort/lifetime/drain/listener counts differ')
    result['memory_comparison'] = memory_pairs
    result.update(complete=True,matched_diagnostic_accepted=args.successful_arms==72,
...
print('PASS: seventy-two v3 c1/c64 matched cohorts and exact outer restoration accepted',flush=True)
