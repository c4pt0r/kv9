//! Preserve the bounded FIFO receipt sequence without shifting retained entries.
//!
...
impl AppliedRing {
    ...
    pub(super) fn push(&mut self, entry: RingEntry) {
        ...
        // Reachable lengths never exceed the unchanged limit. Removing the
        // oldest entry before insertion avoids a transient capacity expansion.
        if self.entries.len() == APPLIED_RING {
...
mod tests {
    ...
    fn compare_with_original_fifo(trace: impl IntoIterator<Item = RingEntry>) -> AppliedRing {
        ...
        for next in trace {
            ...
            assert_eq!(ring.entries.iter().copied().collect::<Vec<_>>(), reference);
            for index in [
                0,
                18,
                u64::MAX,
    ...
    #[test]
    fn indexed_receipts_preserve_fifo_across_wraps_gaps_and_extreme_indexes() {
