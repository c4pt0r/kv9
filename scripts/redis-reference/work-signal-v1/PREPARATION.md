# Raft owner wake coalescing: predeclared matched screen

Protocol `kv9-work-signal-c1-c64-v1` compares direct parent `57ff6851` with
candidate `1b70dba6`. The general baseline remains `5ee897a`; calling the
...
inbox-vector screen; duration, repeated schedule and every population check
remain identical. All original outcome,
...
```sh
env PYTHONOPTIMIZE=0 PYTHONDONTWRITEBYTECODE=1 taskset -c 6-15,22-31 python3 /tmp/kv9-work-signal-comparison-preparation/isolate-and-run.py --driver /tmp/kv9-work-signal-comparison-preparation/matched-driver.py --driver-arguments-file /tmp/kv9-work-signal-comparison-preparation/driver-arguments.json --output /tmp/kv9-work-signal-matched-diagnostic-attempt1
```
...
already passed 219 Raft tests and Clippy; independently checked process E2E
passed 351 operations. Neither that process gate nor this timing screen is
