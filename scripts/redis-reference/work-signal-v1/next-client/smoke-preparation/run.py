...
def check_populations(report, config, reference, resp):
    """Independent selected API and complete, one-attempt outcome accounting."""
    result = {}
    for phase, metrics in report['metrics'].items():
        ...
        if phase == 'warmup':
            reads = sum(resp.mix(config['seed'] ^ n) % 100 < config['read_percent']
                        for n in range(1, config['warmup_calls'] + 1))
            require(counts == [reads, config['warmup_calls'] - reads], 'warmup deterministic mix differs')
        if phase == 'measurement':
            require(sum(counts) == report['measured_issued'] > 0, 'measured population is incomplete')
            require(all(counts[i] > 0 for i in range(2)
            ...
            if config['read_percent'] in (0, 100):
                require(counts[0 if config['read_percent'] == 0 else 1] == 0, 'pure measurement mix differs')
            # Slots can be allocated and then declined at the second cutoff
            # check. Aggregate measured_issued does not identify their nonce set.
...
def dataset_check(config, dataset, report, resp):
    ...
    limit = config['warmup_calls'] + config['max_calls']
    changed = 0
    for index in range(config['keys']):
        ...
        if nonce:
            require(nonce <= limit and resp.mix(config['seed'] ^ nonce) % 100 >= config['read_percent'],
                    'final value refers to an out-of-budget or read-only nonce')
    ...
    return dict(result, changed_keys=changed, maximum_configured_nonce=limit,
...
def main():
    ...
    try:
        ...
        for role in ('native', 'redis'):
            pin = protocol['clients'][role]
            require(pin['revision'] == args.client_revision and pin['source'] == str(CLIENT_SOURCE) and
                    pin['build'] == str(getattr(args, role+'_build')) and
                    pin['manifest_sha256'] == getattr(args, role+'_manifest_sha256') and
                    pin['binary_sha256'] == getattr(args, role+'_binary_sha256'), 'protocol client pin differs')
        require(protocol['server'] == dict(revision=SERVER_REV, source=str(SERVER_SOURCE), build=str(SERVER_BUILD),
                binary_sha256=SERVER_SHA, manifest_sha256=SERVER_MANIFEST), 'protocol control server pin differs')
