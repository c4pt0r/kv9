...
labels. The smoke additionally recomputes the complete issued nonce mix and
requires all-success, one-attempt populations. Setup and verification retain
batch labels. The final scan/MGET checks all 128 keys and the sentinel, exact
deterministic bytes, issued write nonce bounds and whether that nonce writes
the returned key. These aggregate clients do not retain full call histories;
this is not a concurrent linearizability or atomicity proof.
