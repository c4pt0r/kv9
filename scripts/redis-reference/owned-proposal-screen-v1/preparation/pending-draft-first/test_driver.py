...
class Protocol(unittest.TestCase):
    def test_independent_24_order_and_exact_12_smoke(self):
        ...
        self.assertEqual(d.PROTOCOL_ID, 'kv9-owned-proposal-write-screen-c1-c64-v1')
        for repeat in (0, 1):
            for row, (c, workload, batch, reads, role) in zip(plan[repeat*12:(repeat+1)*12],
        ...
        for row, wanted in zip(smoke, plan[:12]):
    ...
    def test_failed_outcomes_preserved_and_health_separate(self):
        # Retain the generic mixed-accounting corruption control outside the write-only inventory.
        row=copy.deepcopy(next(r for r in d.make_plan(False) if r['server_role']=='old'))
        row['shared']['read_percent']=50
    ...
    def test_frozen_role_bindings_without_build_or_runtime(self):
        if d.SERVER_PINS['new']['binary_sha256'].startswith('PENDING_'):
            self.skipTest('Actual candidate release binding is pending; no runtime readiness claim')
        args=types.SimpleNamespace(old_server_source=Path('/tmp/kv9-wal-crc32-table'),
            old_server_build=Path('/tmp/kv9-wal-crc32-release-first'),new_server_source=Path('/tmp/kv9-owned-proposal-buffers'),
            new_server_build=Path('/tmp/kv9-owned-proposal-release-first'),client_source=SOURCE,redis_client_source=SOURCE,
