...
sys.path.insert(0,'/tmp/kv9-indexed-receipt-screen-preparation/statistics')
from core import phase_accounting
root=Path('/tmp/kv9-indexed-receipt-screen-root-preparation')
...
p=Path('/tmp/kv9-indexed-receipt-screen-smoke-first/matrix.json');matrix=json.loads(p.read_text())
assert matrix['complete'] and matrix['smoke'] and len(matrix['attempts'])==12
rows=[];total_calls=0;retained=0;largest=0
for arm in matrix['attempts']:
 assert arm['complete']
 if arm['descriptor']['target']=='kv9':
  assert arm['cleanup_complete']
 else:
  assert arm['cleanup_errors']==[]
