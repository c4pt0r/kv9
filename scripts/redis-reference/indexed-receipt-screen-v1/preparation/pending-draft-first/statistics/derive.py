#!/usr/bin/env python3
"""Derive the first indexed-receipt write-only screen only after accepted auditing."""
import hashlib
import json
...
AUDIT_DIR = Path('/tmp/kv9-indexed-receipt-screen-preparation/results-first')
GATE = Path('/tmp/kv9-indexed-receipt-screen-root-preparation/audit-terminal-first.json')
...
assert audit['protocol_id'] == 'kv9-indexed-receipt-write-screen-c1-c64-v1' and len(audit['cases']) == 24
reports = {}
bound = {}
...
pairs = []
pooled_pairs = []
workloads = ['point-r000', 'batch64-r000']
...
lines = ['# Indexed-receipt write-only screen: first accepted recording', '', result['limitation'], '',
