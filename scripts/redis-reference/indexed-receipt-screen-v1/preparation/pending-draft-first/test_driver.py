...
class Protocol(unittest.TestCase):
    def test_independent_24_order_and_exact_12_smoke(self):
        ...
        self.assertEqual(d.PROTOCOL_ID, 'kv9-indexed-receipt-write-screen-c1-c64-v1')
        for repeat in (0, 1):
    ...
    def test_frozen_role_bindings_without_build_or_runtime(self):
        args=types.SimpleNamespace(old_server_source=Path('/tmp/kv9-wal-crc32-table'),
            old_server_build=Path('/tmp/kv9-wal-crc32-release-first'),new_server_source=Path('/tmp/kv9-indexed-apply-receipts'),
            new_server_build=Path('/tmp/kv9-indexed-receipt-release-first'),client_source=SOURCE,redis_client_source=SOURCE,
