...
def matrix():
    ...
    return dict(version=2,protocol_id='kv9-crc-broad-workloads-c1-c64-v1',concurrency_points=[1,64],
...
class AuditContract(unittest.TestCase):
    ...
    def test_exact_72_entire_reverse_order_and_run_ids(self):
        ...
        self.assertEqual([(d['repeat'],d['arm'],d['shared']['run_id']) for d in
                          [m['cohort_inventory'][i] for i in (0,18,36,54,71)]],
                         [(0,'old-point-r000','p00001'),(0,'old-point-r000','p00064'),
                          (1,'redis-batch64-r100','p10064'),(1,'redis-batch64-r100','p10001'),
                          (1,'old-point-r000','p10001')])

    def test_reject_partial_reverse_missing_duplicate_and_old_schedule(self):
        bads=[]
        m=matrix();m['cohort_inventory'][36:]=m['cohort_inventory'][54:]+m['cohort_inventory'][36:54];bads.append(m)
        m=matrix();m['cohort_inventory'][18:36]=copy.deepcopy(m['cohort_inventory'][:18]);bads.append(m)
        m=matrix();m['cohort_inventory']=m['cohort_inventory'][:36];m['attempts']=[{}]*36;bads.append(m)
        m=matrix();m['cohort_inventory'][36:]=copy.deepcopy(m['cohort_inventory'][:36]);bads.append(m)
        m=matrix();m['attempts']=[{}]*48;bads.append(m)
        for m in bads:
