...
/// WAIT confirms replication on the write connection, not Raft or fsync.
#[derive(Clone, Copy, Debug, Deserialize, Serialize, Eq, PartialEq)]
#[serde(tag = "kind", rename_all = "snake_case", deny_unknown_fields)]
pub enum WriteConfirmation {
    Async {},
    Wait { replicas: u8, timeout_ms: u64 },
}

fn deserialize_confirmation<'de, D: serde::Deserializer<'de>>(
    deserializer: D,
) -> Result<Option<WriteConfirmation>, D::Error> {
    WriteConfirmation::deserialize(deserializer).map(Some)
}
...
pub struct Config {
    ...
    #[serde(
        ...
        deserialize_with = "deserialize_confirmation"
    )]
    pub write_confirmation: Option<WriteConfirmation>,
    #[serde(
        default,
        skip_serializing_if = "Option::is_none",
...
pub struct WireSizes {
    ...
    pub confirmation_request_bytes: Option<usize>,
...
impl Config {
    pub fn validate(&self) -> Result<WireSizes, &'static str> {
        ...
        if let Some(WriteConfirmation::Wait {
            replicas,
            timeout_ms,
        }) = self.write_confirmation
        {
            if !(1..=2).contains(&replicas) || timeout_ms == 0 || timeout_ms > self.deadline_ms {
                return Err("invalid replication confirmation count or timeout");
            }
        }
        ...
        let sizes = WireSizes {
            confirmation_request_bytes: self.confirmation_request().map(|bytes| bytes.len()),
        ...
        .any(|n| n > MAX_WIRE)
            || sizes.mset_request_bytes + sizes.confirmation_request_bytes.unwrap_or(0) > MAX_WIRE
            || sizes.set_request_bytes.unwrap_or(0) + sizes.confirmation_request_bytes.unwrap_or(0)
                > MAX_WIRE
    ...
    pub fn confirmation_request(&self) -> Option<Vec<u8>> {
        match self.write_confirmation {
            Some(WriteConfirmation::Wait {
                replicas,
                timeout_ms,
            }) => {
    ...
    pub fn workload_model(&self) -> &'static str {
        if self.version == 4 {
            "bounded_redis_replication_confirmed_performance"
