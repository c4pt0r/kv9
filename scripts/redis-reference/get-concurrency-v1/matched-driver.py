#!/usr/bin/env python3
"""Accepted-server and Redis single-GET offered-concurrency diagnostic.
...
def make_plan(smoke):
    # Offered SDK concurrency varies; public admission and read-registry limits
    # remain fixed. A valid high-concurrency sample may contain retries/refusals.
    points = (1, 256) if smoke else CONCURRENCY_POINTS
    forward = [(workers, arm, server, api) for workers in points
               for arm, server, api in (("kv9-point", "old", "point_get"),
                                         ("redis-get", None, "get"))]
    rows = []
    for repeat in range(1 if smoke else 2):
        for workers, arm, server, api in (forward if repeat == 0 else reversed(forward)):
            shared = dict(run_id=f"g{repeat}{workers:04d}", seed=71, workers=workers,
            ...
            rows.append(dict(repeat=repeat, arm=arm, server_role=server, read_api=api,
                             mix="read", target="kv9" if server else "redis-memory",
                             shared=shared))
    require(len(rows) == (4 if smoke else 24), "concurrency-curve inventory differs")
    return rows
...
def main():
    ...
    for name in ("client_source", "client_build", "old_server_source", "old_server_build",
                 "redis_client_source", "redis_client_build", "output", "redis_server", "resp_helper"):
    ...
    for source in (args.client_source, args.old_server_source, args.redis_client_source):
        require(not args.output.is_relative_to(source), "output must not mutate a source worktree")
    for retained in (args.client_build, args.old_server_build, args.redis_client_build):
    ...
    plan = dict(version=2, protocol_id=PROTOCOL_ID,
                concurrency_points=list((1, 256) if args.smoke else CONCURRENCY_POINTS),
                complete=False, client_revision=args.expected_client_revision,
                smoke=args.smoke, placement=placement(args.smoke), role_bindings=roles, client_cargo=cargo,
                scope="Four-arm low/high-concurrency correctness smoke; no timing acceptance" if args.smoke else
                      "Shared-host single-GET offered-concurrency curve with fixed admission limits; no sustained-capacity or equal-durability claim",
