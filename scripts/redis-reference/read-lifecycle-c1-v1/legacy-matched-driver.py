...
OLD_REVISION = "3bd1751bddf9a85b07447b64a12adf28ed0df8a9"
NEW_REVISION = "d3dcea0355dd6c4ec23f0833708f6dc978412093"
SERVER_PINS = {
    "old": dict(revision=OLD_REVISION,
                binary_sha256="82cf715e6d8d1ea8898db1ad3624431958a21213c50ebc00ae5320943cc03991",
                manifest_sha256="80a0987ba19e24812e6c554bde7d0df2193d2702823dfb0430449e9d2b87d645"),
    "new": dict(revision=NEW_REVISION,
                binary_sha256="08e8103b5230933bf65b4b605fe864803f766a1b3cf3776d7bb796bf4e3611e1",
                manifest_sha256="0d724224a936d3195c877d9c28c069f9460c63ee3bf7d8c04a7deb30f0c68c26"),
}
REDIS_CLIENT_SHA = "d5c2069f00787a37a14b1cb25848aad05c3d2c684d9609b914becbafc0ec9e2b"
...
def verify_inputs(args):
    ...
    for name, source, build in (("old", args.old_server_source, args.old_server_build),
        ...
        if name == "old":
            ...
            require(summary["complete"] is True and summary["manifest_sha256"] == pin["manifest_sha256"],
                    "original release build incomplete")
            rows = [r for r in summary["commands"] if r["name"] == "server"]
            require(len(rows) == 1 and rows[0]["command"] == SERVER_COMMAND and rows[0]["exit_code"] == 0,
                    "original server build command differs")
            require(summary["source"]["revision"] == pin["revision"] and summary["source"]["dirty"] is False and
                    summary["source"]["sources"] == manifest["sources"], "original build source binding differs")
            declared = manifest["server_binary_sha256"]
            cargo = build / "server.stdout"
            selected = cargo_server(cargo, manifest["cargo_artifacts"]["server"])
            files += [build / "summary.json", cargo]
        else:
            declaration = manifest["binaries"]["kv9"]
            require(declaration["command"] == SERVER_COMMAND, "candidate server command differs")
    ...
    for name, source, build, revision, binary in (
        ...
        ("redis", args.old_server_source, args.redis_client_build, OLD_REVISION, "kv9-redis-batch-reference")):
        manifest = read(build / "build.json")
        ...
        if name == "redis":
            require(manifest["binary_sha256"] == REDIS_CLIENT_SHA and
                    inventory["sources"] == roles["old"]["source"]["sources"], "original Redis client changed")
...
def verify_client_cargo(args, validator):
    ...
    for role, build, revision, reference in (("client", args.client_build, args.expected_client_revision, False),
                                            ("redis", args.redis_client_build, OLD_REVISION, True)):
...
def make_plan(smoke):
    arms = (("old-point", "old", "point_get"), ("old-batch1", "old", "batch_get"),
            ...
            ("redis-mget1", None, None))
    rows = []
    for repeat in range(1 if smoke else 2):
        shared = dict(run_id=f"p{repeat}0064", seed=71, workers=64, keys=4096, batch_size=1,
                      value_bytes=128, read_percent=100, warmup_calls=128, measure_ms=1500,
                      max_calls=10_000_000, load=dict(kind="closed_loop"))
        for arm, server, api in (arms if repeat == 0 else reversed(arms)):
            rows.append(dict(repeat=repeat, arm=arm, server_role=server, read_api=api,
                             mix="read", target="kv9" if server else "redis-memory", shared=dict(shared)))
    require(len(rows) == (5 if smoke else 10), "matched cohort inventory differs")
    return rows


def configs(row, peers, keyspace, redis_address):
    shared = row["shared"]
    native = dict(version=2, rpc_transport="tonic_stream", read_api=row["read_api"] or "batch_get", **shared,
    ...
    redis = dict(version=1, address=redis_address, deadline_ms=1500, **shared)
    return native, redis
...
def redis_trial(row, directory, args, modules, roles):
    ...
    result = dict(complete=False, client_binary_sha256=roles["redis"]["binary_sha256"],
                  server_binary_sha256=args.redis_server_sha256, operation="mget", batch_size=1)
    try:
        ...
        result["validated"] = redis_validator.validate(directory / "run", args.redis_client_build, directory / "requested-config.json", OLD_REVISION, require_timing=not args.smoke)
        dataset = {}
...
def main():
    ...
    parser.add_argument("--old-server-source", type=Path, default=Path("/tmp/kv9-native-batch-benchmark"))
    parser.add_argument("--old-server-build", type=Path, default=Path("/tmp/kv9-native-batch-paired-release-build-first"))
    parser.add_argument("--new-server-source", type=Path, default=Path("/tmp/kv9-async-native-batch-read"))
    parser.add_argument("--new-server-build", type=Path, default=Path("/tmp/kv9-async-native-batch-read-release-first"))
    parser.add_argument("--redis-client-build", type=Path, default=Path("/tmp/kv9-native-batch-paired-release-build-first/redis"))
    ...
    for name in ("client_source", "client_build", "old_server_source", "old_server_build", "new_server_source",
                 "new_server_build", "redis_client_build", "output", "redis_server", "resp_helper"):
    ...
    for source in (args.client_source, args.old_server_source, args.new_server_source):
    ...
    for loaded in list(sys.modules.values()) + [support, tmpfs, comparison]:
        ...
        if path.is_relative_to(args.client_source):
            relative = str(path.relative_to(args.client_source))
            require(relative in roles["client"]["source"]["sources"], "imported helper is outside source inventory")
            require(digest(path) == roles["client"]["source"]["sources"][relative], "imported helper bytes differ")
            helper_hashes[str(path)] = digest(path)
    rows = make_plan(args.smoke)
    args.output.mkdir(parents=True)
    plan = dict(version=1, complete=False, client_revision=args.expected_client_revision,
                smoke=args.smoke, placement=placement(args.smoke), role_bindings=roles, client_cargo=cargo,
                scope="Five-arm correctness smoke only; no timing acceptance" if args.smoke else
