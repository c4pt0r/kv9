#!/usr/bin/env python3
"""Derive CRC write rates and whole-call histograms only after accepted auditing."""
import hashlib
import json
...
AUDIT_DIR = Path('/tmp/kv9-crc-write-audit-preparation/results-first')
GATE = Path('/tmp/kv9-crc-write-root-preparation/audit-terminal-first.json')
...
assert audit['parent_confirmed_session'] == 63194
assert audit['protocol_id'] == 'kv9-crc-write-c64-v1' and len(audit['cases']) == 12
reports = {}
bound = {}
...
for workload in ['point-r000', 'batch64-r000']:
    for role in ['old', 'new', 'redis']:
        selected = [c for c in cases if c['workload'] == workload and c['role'] == role]
        assert len(selected) == 2 and {c['repeat'] for c in selected} == {0, 1}
        assert all(c['concurrency'] == 64 and c['read_percent'] == 0 for c in selected)
        ...
        pooled.append(row)
    for repeat in [0, 1]:
        selected = {c['role']: c for c in cases if c['workload'] == workload and c['repeat'] == repeat}
        assert set(selected) == {'old', 'new', 'redis'}
        pairs.append(dict(workload=workload, repeat=repeat, comparison=compare(selected['old'], selected['new'])))

pooled_pairs = []
for workload in ['point-r000', 'batch64-r000']:
    selected = {c['role']: c for c in pooled if c['workload'] == workload}
    pooled_pairs.append(dict(workload=workload, comparison=compare(selected['old'], selected['new']),
...
result = dict(complete=True, accepted_audit_sha256=sha(AUDIT_DIR / 'audit.json'), accepted_inventory_sha256=sha(AUDIT_DIR / 'input-inventory.json'),
    ...
    limitation='Two 10-second forward/reverse c64 write repeats on a shared host; three-voter KV9 quorum/sync with tmpfs WAL versus standalone Redis with persistence and pipelining disabled. No equal-durability, sustained-capacity, significance or new Chaos claim.')
...
lines = ['# CRC write screen: first accepted recording', '', result['limitation'], '',
    '| Workload | Role | Calls/s | Keys/s | Mean us | p95 us | p99 us | Client cores | Server cores |',
    '| --- | --- | ---: | ---: | ---: | --- | --- | ---: | ---: |']
for row in pooled:
    hist = row['whole_call_latency']
    lines.append(f"| {row['workload']} | {row['role']} | {row['successful_calls_per_second']:.3f} | {row['successful_input_items_per_second']:.3f} | {hist['mean_ns']/1000:.3f} | {qtext(hist,95)} | {qtext(hist,99)} | {row['client_cpu_cores']:.3f} | {row['server_cpu_cores']:.3f} |")
lines += ['', 'Quantiles are bucket intervals; batch latency covers the whole call.', '', '| Workload | Repeat | QPS change | Mean change | Old p99 ns | New p99 ns |', '| --- | ---: | ---: | ---: | --- | --- |']
for row in pairs:
    c = row['comparison']
    lines.append(f"| {row['workload']} | {row['repeat']} | {c['successful_qps_change_percent']:+.3f}% | {c['mean_change_percent']:+.3f}% | {c['old_p99']} | {c['new_p99']} |")
...
with (HERE / 'READOUT.md').open('x') as stream:
    stream.write('\n'.join(lines) + '\n')
print(json.dumps(dict(complete=True, cases=len(cases), pooled_rows=len(pooled), totals=totals, pooled_pairs=pooled_pairs)))
