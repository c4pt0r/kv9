#!/usr/bin/env python3
"""Matched CRC write screen: c64 point PUT and native BatchPut(64).
...
NEW_REVISION = "ca0002c7f8e9ee6f595efcc9f4151085ccce87cb"
...
def require_candidate_pins():
    require(isinstance(NEW_REVISION, str) and re.fullmatch(r"[0-9a-f]{40}", NEW_REVISION) is not None,
            "candidate revision pin is required before any run")
    pin = SERVER_PINS["new"]
    require(pin["revision"] == NEW_REVISION, "candidate revision pins disagree")
    for field in ("binary_sha256", "manifest_sha256"):
        require(isinstance(pin[field], str) and re.fullmatch(r"[0-9a-f]{64}", pin[field]) is not None,
                "candidate " + field + " pin is required before any run")


def verify_inputs(args):
    """Separate original role attestations; never fabricate a mixed-revision build."""
    require_candidate_pins()
    roles = {}
...
def make_plan(smoke):
    first = [(concurrency, name, batch, reads, role) for concurrency in (64,)
    ...
    require(len(rows) == (6 if smoke else 12), "matched CRC c64 write cohort inventory differs")
    return rows
...
def main():
    ...
    parser.add_argument("--new-server-source", type=Path, default=Path("/tmp/kv9-wal-crc32-table"))
    parser.add_argument("--new-server-build", type=Path, required=True)
    ...
    plan = dict(version=2, protocol_id=PROTOCOL_ID, workload_cells=WORKLOAD_CELLS, storage_guards=SPACE_GUARDS,
                concurrency_points=[64], complete=False, client_revision=args.expected_client_revision,
                smoke=args.smoke, placement=placement(args.smoke), role_bindings=roles, client_cargo=cargo,
                scope="6-cohort CRC c64 write correctness smoke only; no timing acceptance" if args.smoke else
                      "12-cohort shared-host CRC point/batch64 write diagnostic; no exclusive host or equivalent durability",
