#!/usr/bin/env python3
"""Independent 12-cohort CRC write diagnostic readback; no workload execution."""
import argparse
import base64
...
parser.add_argument('--successful-arms', type=int, choices=(12,), required=True)
...
PROTOCOL_ID = 'kv9-crc-write-c64-v1'
CONCURRENCIES = (64,)
...
def validate_matrix_protocol(matrix):
    require(type(matrix['version']) is int and matrix['version']==2 and
            ...
            same_json(matrix['concurrency_points'],list(CONCURRENCIES)), 'c64 protocol scope differs')
    ...
    require(len(matrix['attempts'])==12 and same_json(matrix['cohort_inventory'],expected),
            'twelve-cohort inventory/order differs')
...
result = dict(complete=False, matched_diagnostic_accepted=False, cases=[],
              scope='Two-repeat 10-second point/batch64 write-only c64 shared-host tmpfs diagnostic; no sustained-capacity, equal-durability, exact issued nonce ledger or full-history claim',
...
try:
    ...
    require(args.successful_arms==12 and args.root_exit_code==0, 'expected complete diagnostic scope differs')
    ...
    expected_argv=['/usr/bin/python3','/tmp/kv9-crc-write-screen-preparation/matched-driver.py',
    ...
    require(matrix['helper_hashes'][str(Path('/tmp/kv9-crc-write-screen-preparation/matched-driver.py'))]==args.expected_driver_sha256,
    ...
    require(len(result['cases'])==12 and len(lifetimes)==40 and drains==24 and writer_bindings==24,
            'full cohort/lifetime/drain/listener counts differ')
    result['memory_comparison'] = memory_pairs
    result.update(complete=True,matched_diagnostic_accepted=args.successful_arms==12,
...
print('PASS: twelve CRC write c64 matched cohorts and exact outer restoration accepted',flush=True)
