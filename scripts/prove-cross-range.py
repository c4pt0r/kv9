#!/usr/bin/env python3
"""Check the cross-range model and explicit defect controls."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    contract = json.loads(
        (root / "proofs/lean/cross-range/source-contract.json").read_text()
    )
    assert contract["schema"] == 1
    for name, digest in contract["source_sha256"].items():
        assert hashlib.sha256((root / name).read_bytes()).hexdigest() == digest, name
    source = (root / "proofs/lean/cross-range/CrossRange.lean").read_text()
    ...
    def compile_source(label, text, success):
        ...
        directory.mkdir()
        path = directory / "CrossRange.lean"
        path.write_text(text)
        command = [args.lean, "-DwarningAsError=true", "-o", str(directory / "CrossRange.olean"), str(path)]
    ...
    audit.write_text("import CrossRange\n" + "\n".join(
        f"#print axioms Kv9.CrossRange.{name}" for name in contract["theorems"]) + "\n")
    ...
    for name in contract["theorems"]:
        line = next(line for line in result.stdout.splitlines()
                    if f"Kv9.CrossRange.{name}'" in line)
    ...
    controls = [
        ('serve-high-before-low',
         '(p : s.split = true) (ordered : s.lowChunkDone = true)',
         '(p : s.split = true) (ordered : s.lowChunkDone = s.lowChunkDone)'),
        ('serve-without-the-partition',
         '| serveLow {s} (p : s.split = true) : Step s {s with lowChunkDone := true}',
         '| serveLow {s} (p : s.split = s.split) : Step s {s with lowChunkDone := true}'),
        ('serve-skips-a-key',
         '| serveLow {s} (p : s.split = true) : Step s {s with lowChunkDone := true}',
         '| serveLow {s} (p : s.split = true) : Step s {s with lowChunkDone := true, skipped := true}'),
        ('serve-duplicates-a-key',
         'Step s {s with highChunkDone := true}',
         'Step s {s with highChunkDone := true, duplicated := true}'),
        ('serve-atomically',
         'Step s {s with highChunkDone := true}',
         'Step s {s with highChunkDone := true, atomicSpan := true}'),
    ]
    ...
    report = {
        ...
        "verified_rust_extraction": False, "skipped_duplicated_or_atomic_span": False,
        "new_chaos_acceptance": False, "new_qps": False,
    }
    (output / "result.json").write_text(json.dumps(report, indent=2) + "\n")
    print(f"CROSS_RANGE_PROOF_ACCEPTED: {len(contract['theorems'])} theorems; {len(controls)} semantic and 2 policy controls")
