...
def main():
    ...
    parser.add_argument('--base-port', type=int, default=27360)
    ...
    env = dict(os.environ, KV9_CLUSTER_TOKEN='compaction-e2e-cluster',
               KV9_CLIENT_TOKENS='admin=compaction-e2e-client', KV9_CLIENT_TOKEN='compaction-e2e-client',
               KV9_BOOTSTRAP_TOKEN='compaction-e2e-bootstrap', KV9_STORAGE='minio')
    ...
    try:
        ...
        def compact_round(name, count):
            term, index = burst(name, count)
            before = leader_first_index()
            recorded = command(f'{name}-record', 'client', 'record-group-compaction',
                               '--addr', addresses[wait('meta leader', leader)],
                               '--root-digest', root, '--region', region,
                               '--floor-term', term, '--floor-index', index)
            require(recorded['compaction_outcome'] in ('recorded', 'confirmed'),
                    f'decision receipt missing: {recorded}')

            # Compaction advances the raft-log FIRST INDEX past the floor
            # (the on-disk log is append-only; the retained prefix shrinks in
            # the index, not the file). The leader executes through the
            # all-matched-gated seam.
            def leader_compacted():
                first = leader_first_index()
                return first is not None and int(first) > 1 and int(first) >= index
            first = wait(f'{name}: the group leader advances its log first index past the floor',
                         leader_compacted, 240)
            print(f'PASS: {name} log_first_index {before} -> {first} (floor {index})', flush=True)
            return term, index

        compact_round('round1', 120)
        compact_round('round2', 120)

        # A stale (non-increasing) floor refuses.
        stale = command('stale-floor', 'client', 'record-group-compaction',
                        '--addr', addresses[wait('meta leader2', leader)],
                        '--root-digest', root, '--region', region,
                        '--floor-term', 1, '--floor-index', 1, success=None)
        require(stale['exit_code'] != 0, 'a non-increasing floor must refuse')
        ...
        manifest_record.update(
            ...
            checks=['a committed kind-110 floor drives all-matched-gated compaction at the group leader (log first-index advances past the floor)',
                    'a second, higher floor compacts again; a non-increasing floor refuses typed',
                    'a full-cluster restart recovers on the compacted logs and every sampled key serves',
                    'the group keeps taking writes after the compacted restart'])
