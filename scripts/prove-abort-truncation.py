#!/usr/bin/env python3
"""Check the abort-truncation model and explicit defect controls."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    contract = json.loads(
        (root / "proofs/lean/abort-truncation/source-contract.json").read_text()
    )
    assert contract["schema"] == 1
    for name, digest in contract["source_sha256"].items():
        assert hashlib.sha256((root / name).read_bytes()).hexdigest() == digest, name
    source = (root / "proofs/lean/abort-truncation/AbortTruncation.lean").read_text()
    ...
    def compile_source(label, text, success):
        ...
        directory.mkdir()
        path = directory / "AbortTruncation.lean"
        path.write_text(text)
        command = [args.lean, "-DwarningAsError=true", "-o", str(directory / "AbortTruncation.olean"), str(path)]
    ...
    audit.write_text("import AbortTruncation\n" + "\n".join(
        f"#print axioms Kv9.AbortTruncation.{name}" for name in contract["theorems"]) + "\n")
    ...
    for name in contract["theorems"]:
        line = next(line for line in result.stdout.splitlines()
                    if f"Kv9.AbortTruncation.{name}'" in line)
    ...
    controls = [
        ('both-settlements-coexist',
         '| settleAbort {s} (clean : s.settledByEvidence = false) :',
         '| settleAbort {s} (clean : s.settledByEvidence = s.settledByEvidence) :'),
        ('a-decision-without-any-settlement',
         '| recordDecision {s}\n      (settled : s.settledByEvidence = true ∨ s.settledByAbort = true) :',
         '| recordDecision {s} :'),
        ('compaction-without-the-released-pin',
         '| compact {s} (d : s.decisionCommitted = true) (p : s.pinReleased = true)\n      (m : s.allMatched = true) : Step s {s with compacted := true}',
         '| compact {s} (d : s.decisionCommitted = true)\n      (m : s.allMatched = true) : Step s {s with compacted := true}'),
        ('compaction-with-a-lagging-voter',
         '| compact {s} (d : s.decisionCommitted = true) (p : s.pinReleased = true)\n      (m : s.allMatched = true) : Step s {s with compacted := true}',
         '| compact {s} (d : s.decisionCommitted = true) (p : s.pinReleased = true) :\n      Step s {s with compacted := true}'),
        ('compaction-discards-a-foreign-prefix',
         '| compact {s} (d : s.decisionCommitted = true) (p : s.pinReleased = true)\n      (m : s.allMatched = true) : Step s {s with compacted := true}',
         '| compact {s} (d : s.decisionCommitted = true) (p : s.pinReleased = true)\n      (m : s.allMatched = true) :\n      Step s {s with compacted := true, foreignPrefixDiscarded := true}'),
    ]
    ...
    report = {
        ...
        "verified_rust_extraction": False, "unsettled_or_unguarded_compaction": False,
        "new_chaos_acceptance": False, "new_qps": False,
    }
    (output / "result.json").write_text(json.dumps(report, indent=2) + "\n")
    print(f"ABORT_TRUNCATION_PROOF_ACCEPTED: {len(contract['theorems'])} theorems; {len(controls)} semantic and 2 policy controls")
