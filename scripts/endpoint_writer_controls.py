...
def mutations(model):
    ...
    for name, before, after, prop, pattern in [
        ('retain-obsolete-admission', "ewAdmission' = IF registration THEN \"Consumed\" ELSE EWCancel(ewAdmission)",
         "ewAdmission' = IF registration THEN \"Consumed\" ELSE ewAdmission", 'EWEffects', r'PROVE\s+EWRevocation'),
        ('endpoint-change-decommissions-member', 'THEN admission ELSE "Superseded"',
         'THEN admission ELSE "Revoked"', 'EWEffects', r'PROVE\s+EWPreserveMember'),
        ...
        ('ignore-installed-generation', 'EWCaptureWins == ewCaptureGeneration >= ewInstalledGeneration',
         'EWCaptureWins == TRUE', 'EWEffects', r'PROVE\s+EWInstallOrder'),
        ...
        ('missing-install-fairness', 'EWStableFairness == WF_ewVars(EWStart) /\\ WF_ewVars(EWInstall)',
         'EWStableFairness == WF_ewVars(EWStart) /\\ TRUE', 'EWConverges', r'PROVE\s+EWStableSpec => EWOld ~>'),
    ]:
