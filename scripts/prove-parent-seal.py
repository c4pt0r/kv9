#!/usr/bin/env python3
"""Check the parent-seal model and explicit defect controls."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    contract = json.loads(
        (root / "proofs/lean/parent-seal/source-contract.json").read_text()
    )
    assert contract["schema"] == 1
    for name, digest in contract["source_sha256"].items():
        assert hashlib.sha256((root / name).read_bytes()).hexdigest() == digest, name
    source = (root / "proofs/lean/parent-seal/Seal.lean").read_text()
    ...
    def compile_source(label, text, success):
        ...
        directory.mkdir()
        path = directory / "Seal.lean"
        path.write_text(text)
        command = [args.lean, "-DwarningAsError=true", "-o", str(directory / "Seal.olean"), str(path)]
    ...
    audit.write_text("import Seal\n" + "\n".join(
        f"#print axioms Kv9.ParentSeal.{name}" for name in contract["theorems"]) + "\n")
    ...
    for name in contract["theorems"]:
        line = next(line for line in result.stdout.splitlines()
                    if f"Kv9.ParentSeal.{name}'" in line)
    ...
    controls = [
        ('fence-without-the-intent',
         '(i : s.intent = true) (fresh : s.sealed = false)',
         '(i : s.intent = s.intent) (fresh : s.sealed = false)'),
        ('fence-serves',
         'Step s {s with sealed := true}',
         'Step s {s with sealed := true, servedWhileSealed := true}'),
        ('fence-touches-the-catalog',
         'Step s {s with sealed := true}',
         'Step s {s with sealed := true, catalogChanged := true}'),
        ('restart-unseals',
         '| restart {s} (r : s.sealed = true) : Step s s',
         '| restart {s} (r : s.sealed = true) : Step s {s with sealed := false}'),
        ('commit-seals-without-authority',
         '| commit {s} : Step s {s with intent := true}',
         '| commit {s} : Step s {s with sealed := true}'),
    ]
    ...
    report = {
        ...
        "verified_rust_extraction": False, "seal_without_intent_or_catalog_change": False,
        "new_chaos_acceptance": False, "new_qps": False,
    }
    (output / "result.json").write_text(json.dumps(report, indent=2) + "\n")
    print(f"PARENT_SEAL_PROOF_ACCEPTED: {len(contract['theorems'])} theorems; {len(controls)} semantic and 2 policy controls")
