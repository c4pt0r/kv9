#!/usr/bin/env python3
"""Check the single-file segment durability model and its audited parameterized TLAPS proofs."""
import argparse
...
from segment_controls import mutations

ROOT = Path(__file__).resolve().parents[1]
SOURCE = ROOT / "proofs/tla/segment"
PROOFS = ROOT / "proofs/tlaps/segment"
MODULES = ("SegmentDurability.tla", "SegmentMC.tla")
CONFIGS = ("Segment2.cfg", "Segment3.cfg")
ACTIONS = ("SGStart", "SGWrite", "SGSync", "SGPublish", "SGSealStart", "SGSealSync", "SGFail", "SGCrash", "SGOpen", "SGValidate", "SGReject", "SGRepair", "SGRecoverySync", "SGRecoveryPublish", "SGClosedReplay", "SGQuiesce")
...
tlc = module("segment_tlc_gate", "check-tla.py")
proof = module("segment_proof_gate", "check-tlaps.py")
require, digest = tlc.require, tlc.digest


def verdict(output, status, expected=None, coverage=False, temporal=False):
    return tlc.verdict(output, status, "EventuallyDrained" if expected == "SGProgress" else expected,
                       temporal=temporal, coverage=coverage,
                       module="SegmentDurability", actions=ACTIONS)
...
def model_case(args, name, config, model, expected=None, fp=0, coverage=False):
    ...
    (work / "SegmentDurability.tla").write_text(model)
    (work / "Run.cfg").write_text(config)
    record = dict(name=name, expected=expected, fingerprint=fp, temporal="SGMCLiveSpec" in config,
                  sources={p.name: digest(p) for p in sorted(work.iterdir())})
    started = time.monotonic()
    try:
        with tempfile.TemporaryDirectory(prefix="kv9-segment-tlc-states.") as states:
...
def proof_case(args, name, model, source, expected=None, pattern=None):
    ...
    work.mkdir()
    (work / "SegmentDurability.tla").write_text(model)
    (work / "SegmentDurabilityProof.tla").write_text(source)
    ...
    try:
        ...
        if pattern:
            output = (work / "SegmentDurabilityProof.log").read_text()
...
def main():
    ...
    require({p.name for p in SOURCE.glob("*.tla")} == set(MODULES), "segment model inventory mismatch")
    require({p.name for p in SOURCE.glob("*.cfg")} == set(CONFIGS) | {"SegmentLive.cfg"}, "segment configuration inventory mismatch")
    require({p.stem for p in PROOFS.glob("*.tla")} == set(inventory["modules"]), "segment proof inventory mismatch")
    require(inventory["roots"] == ["SegmentDurabilityProof"] and set(inventory["models"]) == {"SegmentDurability"},
            "segment root/model inventory mismatch")
    ...
    model = (SOURCE / "SegmentDurability.tla").read_text()
    source = (PROOFS / "SegmentDurabilityProof.tla").read_text()
    models, proofs = [], []
    ...
    def control_group(control):
        config = (SOURCE / "SegmentLive.cfg").read_text() if control.get("live") else (
            (SOURCE / CONFIGS[0]).read_text().split("INVARIANTS", 1)[0] + f"INVARIANT {control['property']}\n")
        ...
        for suffix, text, invalid in (("baseline", model, False), ("mutant", control["model"], True), ("restored", model, False)):
            ...
            proof_group.append(proof_case(args, f"proof-{control['name']}-{suffix}", text, source,
                                          "SegmentDurabilityProof: TLAPS exit 10" if invalid else None,
                                          control["proof_pattern"] if invalid else None))
        check_triple(model_group, "SegmentDurability.tla")
        check_triple(proof_group, "SegmentDurability.tla")
    ...
    live, live_output = model_case(args, "SegmentLive", (SOURCE / "SegmentLive.cfg").read_text(), model)
    models.append(live)
    for witness in ("SGNoAcknowledgement", "SGNoRecoveredUnknown", "SGNoUnpositioned"):
    ...
    audit_controls = [
        ("omitted-proof", tlc.replace_once(source,
         ...
         "OMITTED"), "proof hole: SegmentDurabilityProof.SGInvariantInit"),
        ("custom-axiom", tlc.replace_once(source, "EXTENDS SegmentDurability, TLAPS, NaturalsInduction",
         "EXTENDS SegmentDurability, TLAPS, NaturalsInduction\nAXIOM FALSE"), "unapproved module assumption: SegmentDurabilityProof"),
    ]
    for name, mutant, reason in audit_controls:
        ...
        check_triple(group, "SegmentDurabilityProof.tla")
        proofs.extend(group)
    ...
    for code in (2192, 2267):
        ...
        verdict(live_output, 0, temporal=True)
        model_outputs += 1
    proof_output = (args.output / "proof-baseline/SegmentDurabilityProof.log").read_text()
    proof_outputs = proof.output_controls(proof_output, "SegmentDurabilityProof", inventory["modules"]["SegmentDurabilityProof"]["obligations"])
    summary = dict(jar_sha256=digest(args.jar), tlapm_version=version, tlapm_sha256=digest(args.tlapm), jobs=args.jobs,
                   sources={str(p.relative_to(ROOT)): digest(p) for p in [
                       ...
                       ROOT / "scripts/segment_controls.py", ROOT / "scripts/ready_controls.py",
    ...
    count = len(inventory["modules"]["SegmentDurabilityProof"]["theorems"])
    obligations = inventory["modules"]["SegmentDurabilityProof"]["obligations"]
    print(f"PASS: 4 full segment model checks and 1 fair continuation; {len(controls)} protocol controls; "
          f"3 witnesses; {count} TLAPS declarations; {obligations} obligations; 2 audit controls; "
