...
def main():
    ...
    try:
        ...
        for name,text in [('CRC32',core),('Slicing8',slicing),('RustTable',bt),('RustSlicing',st)]:
            b.require(sorted(re.findall(r'^ *theorem ([A-Za-z_0-9]+)',text,re.M))==sorted(b.NAMES[name]),f'theorem inventory mismatch: {name}')
...
'''
        ...
        for name,old,new in [('big-endian','u32::from_le_bytes','u32::from_be_bytes'),
            ...
            function=declarations['pub(crate) fn crc32_parts(']
            b.require(function.count(old)==1,'source mutant function anchor mismatch: '+name)
            mutant=candidate.replace(function,function.replace(old,new))
