#!/usr/bin/env python3
"""Inspect the exact candidate's ordinary-release layout and allocation requests."""
import hashlib
...
assert (R/'scripts/inline-key/mem_key.rs').read_bytes()==(S/'candidate/crates/engine/src/mem_key.rs').read_bytes()
work=S/(label+'-source');work.mkdir(exist_ok=False)
out=S/label;out.mkdir(exist_ok=False)
for source,name in [(R/'scripts/inline-key/layout.rs','main.rs'),(R/'scripts/inline-key/mem_key.rs','mem_key.rs'),
                    (R/'scripts/engine-interface-counting/src/counting.rs','counting.rs')]:
    shutil.copyfile(source,work/name)
(work/'Cargo.toml').write_text('''[package]
name = "kv9-inline-key-layout"
version = "0.0.0"
edition = "2021"
publish = false
[workspace]
...
try:
    with cache.BuildCache(work,out,True,sources) as build:
        ...
        elf=out/'layout';shutil.copyfile(R/'target/release/kv9-inline-key-layout',elf);elf.chmod(0o755)
...
print(json.dumps({'complete':True,'key_bytes':observation['key_bytes'],'baseline_key_bytes':observation['baseline_key_bytes'],'rows':len(observation['rows'])}))
