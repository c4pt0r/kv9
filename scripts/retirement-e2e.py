#!/usr/bin/env python3
"""Real-process retirement gate: the committed removal decision that
retired the replica from the voter set also permanently fences its local
storage — automatically, durably across restarts, with nothing deleted
and the surviving quorum undisturbed.
...
def main():
    ...
    parser.add_argument('--base-port', type=int, default=26950)
    ...
    env = dict(os.environ, KV9_CLUSTER_TOKEN='retirement-e2e-cluster',
               KV9_CLIENT_TOKENS='admin=retirement-e2e-client', KV9_CLIENT_TOKEN='retirement-e2e-client',
               KV9_BOOTSTRAP_TOKEN='retirement-e2e-bootstrap', KV9_STORAGE='minio')
    ...
    try:
        ...
        # --- Removal: the committed decision retires one exact source replica.
        # The metadata leader may have moved across the earlier restarts.
        owner_node = wait('metadata leader before removal', leader)
        leader_r = wait('leader before removal', group_leader)
        early = command('remove-early', 'client', 'remove-source-replica', '--addr', addresses[leader_r],
                        '--root-digest', root, '--operation-id', f'{7:032x}', success=None)
        require(early['exit_code'] != 0, 'removal before the committed decision must refuse')

        removed = next(n for n in (1, 2, 3) if n != owner_node and n != leader_r)
        # The destination can never be the removed replica.
        wrong = command('record-removal-destination', 'client', 'record-source-removal',
                        '--addr', addresses[owner_node], '--root-digest', root,
                        '--operation-id', f'{7:032x}', '--source-node', 4,
                        '--source-incarnation', status(4)['store_incarnation'], success=None)
        require(wrong['exit_code'] != 0, 'the destination must refuse as the removal target')

        recorded = command('record-removal', 'client', 'record-source-removal',
                           '--addr', addresses[owner_node], '--root-digest', root,
                           '--operation-id', f'{7:032x}', '--source-node', removed,
                           '--source-incarnation', status(removed)['store_incarnation'])
        require(recorded['removal_outcome'] == 'recorded', 'first decision commit is a mutation')
        confirm = command('record-removal-again', 'client', 'record-source-removal',
                          '--addr', addresses[owner_node], '--root-digest', root,
                          '--operation-id', f'{7:032x}', '--source-node', removed,
                          '--source-incarnation', status(removed)['store_incarnation'])
        require(confirm['removal_outcome'] == 'confirmed'
                and confirm['removal_task'] == recorded['removal_task'],
                'an identical decision confirms the same committed row')

        expected_voters = ','.join(str(n) for n in sorted({1, 2, 3, 4} - {removed}))

        def remove(label):
            for attempt in range(20):
                node_now = wait('leader for removal', group_leader)
                probe = command(label, 'client', 'remove-source-replica', '--addr', addresses[node_now],
                                '--root-digest', root, '--operation-id', f'{7:032x}', success=None)
                if probe['exit_code'] == 0:
                    return probe
                time.sleep(0.5)
            require(False, 'removal did not commit in time')

        first = remove('remove')
        require(first['remove_outcome'] == 'removed' and first['removed_node'] == str(removed),
                'the exact committed replica must be removed')
        require(first['voters'] == expected_voters, 'three voters including the destination remain')
        again4 = remove('remove-again')
        require(again4['remove_outcome'] == 'confirmed' and again4['voters'] == expected_voters,
                'a removal retry confirms the same configuration')

        # Three voters (with the migrated destination) still commit writes.
        node_w = wait('leader after removal', group_leader)
        command('post-removal-put', 'client', 'raw-put', '--addr', addresses[node_w],
                '--keyspace', keyspace_id, '--key-hex', '72656d6f766564', '--value-hex', '676f6e65')
        removal_applied = int(fields(open(output / 'post-removal-put.log').read()).get('applied_index', 0))
        require(removal_applied > 0, 'missing post-removal apply receipt')

        def survivors_track():
            return all(int((data_groups(n).get(region, {}).get('driver_applied') or {})
                           .get('index', 0)) >= removal_applied
                       for n in sorted({1, 2, 3, 4} - {removed}) if n in processes)
        wait('the three remaining voters track the write', survivors_track, 120)

        # The committed decision fences the local replica AUTOMATICALLY: the
        # removed node's own reconcile retires it while running.
        wait('removed replica retires itself under the committed decision',
             lambda: data_groups(removed).get(region, {}).get('state') == 'retired', 120)
        # Storage stays durable: the group directory keeps its files.
        group_dir = output / f'n{removed}' / 'data-groups' / str(region)
        require(group_dir.is_dir() and any(group_dir.iterdir()),
                'retirement must keep the durable group storage')

        # Restart: the retired record survives and nothing reopens.
        stop(removed)
        start(removed, 'removed-restart', ticket=None)
        wait('removed node ready after restart', lambda: status(removed).get('endpoint_ready') == 'true')
        wait('retirement survives the restart',
             lambda: data_groups(removed).get(region, {}).get('state') == 'retired')
        require(group_dir.is_dir() and any(group_dir.iterdir()),
                'restart must not delete retired storage')
        node_f = wait('leader for the final removal write', group_leader)
        command('final-removal-put', 'client', 'raw-put', '--addr', addresses[node_f],
                '--keyspace', keyspace_id, '--key-hex', '73757276697665', '--value-hex', '6f6b')
        final_r = int(fields(open(output / 'final-removal-put.log').read()).get('applied_index', 0))
        require(final_r > 0, 'missing final removal apply receipt')

        def survivors_final():
            return all(int((data_groups(n).get(region, {}).get('driver_applied') or {})
                           .get('index', 0)) >= final_r
                       for n in sorted({1, 2, 3, 4} - {removed}) if n in processes)
        wait('survivors track the final write after the removed restart', survivors_final, 120)

        manifest_record.update(verdict='accepted', region=region,
                               checks=['settlement, compaction, promotion and removal as previously qualified',
                                       'the committed decision retires the local replica automatically',
                                       'retirement keeps the durable group storage; nothing is deleted',
                                       'the retired record survives restart and nothing reopens',
                                       'the surviving three-voter quorum keeps committing throughout'])
