...
fn snapshots_share_roots_and_unchanged_subtrees() {
    ...
    let (Node::Branch(current), Node::Branch(previous)) =
        (map.root.as_deref().unwrap(), old.root.as_deref().unwrap())
    else {
...
fn independent_model_detects_corrupted_separator_and_size() {
    ...
    let Node::Branch(children) = Arc::make_mut(map.root.as_mut().unwrap()) else {
        unreachable!()
    };
