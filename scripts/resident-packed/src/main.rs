...
use kv9_packed_index_experiment::PackedMap;
...
fn write_case<M: Index>(work: &Workload, pinned: bool, passes: usize) -> Value {
    ...
    let mut row = json!({"operation":"write","workload":work.name,"pinned":pinned,"passes":passes,
...
fn queries(work: &Workload, op: &str) -> Vec<Vec<u8>> {
    ...
    let mut out = Vec::new();
    for i in 0..512 {
        let index = (i * work.final_rows.len() / 512 + 71) % work.final_rows.len();
    ...
    // Deterministic permutation avoids a monotonic lookup-only stream.
    let mut state = 71u64;
...
fn read_case<M: Index>(work: &Workload, op: &str, probes: &[Vec<u8>], passes: usize) -> Value {
    let map = populate::<M>(&work.final_rows);
    let model: Model = work.final_rows.iter().cloned().collect();
    for key in probes {
        let expected: Vec<Pair> = match op {
            "get_hit" | "get_miss" => model
                .get(key)
                .map(|v| vec![(vec![], v.clone())])
                .unwrap_or_default(),
            "predecessor" => model
                .range(..=key.clone())
                .next_back()
    ...
    let mut metrics = Metrics::new(probes.len() * passes);
    for _ in 0..passes {
        for key in probes {
            let stamp = begin();
    ...
    json!({"operation":op,"workload":work.name,"pinned":false,"passes":passes,"queries":probes.len(),
        "keys":work.final_rows.len(),"query_sha256":digest(&probes.iter().map(|k|(k.clone(),vec![])).collect::<Vec<_>>()),
        "metrics":metrics.report(),"all_query_outputs_checked":true})
}

fn main() {
    let root = PathBuf::from(std::env::args().nth(1).expect("artifact root"));
    let output = PathBuf::from(std::env::args().nth(2).expect("fresh output"));
    ...
    println!(
        "{}",
        json!({"event":"correctness_passed","live_prefixes":1272,"retained_old_views":636})
    );
    let mut rows = Vec::new();
    for work in &works {
        for pinned in [false, true] {
            if !cfg!(feature = "allocation-counting") {
                black_box(write_case::<PackedMap>(work, pinned, 1));
                black_box(write_case::<RedBlackTreeMapSync<Vec<u8>, Vec<u8>>>(
                    work, pinned, 1,
                ));
            }
            for (order, backend) in ["rpds", "packed", "packed", "rpds"].into_iter().enumerate() {
                let mut row = if backend == "packed" {
                    write_case::<PackedMap>(work, pinned, passes)
                } else {
                    write_case::<RedBlackTreeMapSync<Vec<u8>, Vec<u8>>>(work, pinned, passes)
                };
                row["backend"] = json!(backend);
                row["order"] = json!(order);
                println!(
                    "{}",
                    json!({"event":"write_row_complete","workload":work.name,"pinned":pinned,"backend":backend,"order":order})
                );
                rows.push(row);
            }
        }
    ...
    for work in [&works[0], &works[2]] {
        for op in ["get_hit", "get_miss", "predecessor", "scan16"] {
            let probes = queries(work, op);
            if !cfg!(feature = "allocation-counting") {
                black_box(read_case::<PackedMap>(work, op, &probes, 1));
                black_box(read_case::<RedBlackTreeMapSync<Vec<u8>, Vec<u8>>>(
                    work, op, &probes, 1,
                ));
            }
            for (order, backend) in ["rpds", "packed", "packed", "rpds"].into_iter().enumerate() {
                let mut row = if backend == "packed" {
                    read_case::<PackedMap>(work, op, &probes, passes)
                } else {
                    read_case::<RedBlackTreeMapSync<Vec<u8>, Vec<u8>>>(work, op, &probes, passes)
                };
                row["backend"] = json!(backend);
                row["order"] = json!(order);
                rows.push(row);
    ...
    let metadata:Vec<_>=works.iter().map(|w|json!({"workload":w.name,"groups":w.groups.len(),"initial_keys":w.initial.len(),"final_keys":w.final_rows.len(),"final_state_sha256":digest(&w.final_rows)})).collect();
    let result = json!({"complete":true,"counting_build":cfg!(feature="allocation-counting"),"input_plan_sha256":hash(&plan_bytes),
        "group_file_sha256":hash(&group_bytes),"workloads":metadata,"rows":rows,"correctness_live_prefixes":1272,"correctness_old_views":636,
        "scope":"Standalone index calls only. Same owned-key/value clone behavior in writes. Borrowed point/predecessor returns, owned scan16 results. Per-operation timer overhead remains. No engine lock, WAL, Raft, RPC or database QPS. Requested-live-byte counters are not RSS or allocator usable sizes."});
    ...
    println!(
        "{}",
        json!({"complete":true,"rows":56,"counting_build":cfg!(feature="allocation-counting")})
    );
}
