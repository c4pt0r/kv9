#!/usr/bin/env python3
"""Check the completion notification and outbound coalescing boundaries and parameterized proofs."""
import argparse
...
from raft_completion_controls import mutations

ROOT = Path(__file__).resolve().parents[1]
SOURCE = ROOT / "proofs/tla/raft_completion"
PROOFS = ROOT / "proofs/tlaps/raft_completion"
SCOPES = {
    "RaftCompletion": dict(wrapper="RaftCompletionMC", configs=("CompletionShared.cfg", "CompletionDistinct.cfg"),
        live=("CompletionLive.cfg",), standards=("Naturals", "TLAPS"),
        actions=("RCData", "RCEvict", "RCRequestHint", "RCStop", "RCFatal", "RCNotify", "RCObserve", "RCCheck", "RCRegister", "RCReturn", "RCExpire", "RCQuiesce"),
        witnesses=(("RCNoSuccessWitness", "CompletionShared.cfg"), ("RCNoExhaustionWitness", "CompletionShared.cfg"), ("RCNoAllWakeWitness", "CompletionShared.cfg"), ("RCNoEvictedWitness", "CompletionShared.cfg"))),
    "RaftOutbound": dict(wrapper="RaftOutboundMC", configs=("OutboundFIFO.cfg", "OutboundBytes.cfg", "OutboundStale.cfg"),
        live=("OutboundFIFOLive.cfg", "OutboundStaleLive.cfg"), standards=("Naturals", "Integers", "NaturalsInduction", "TLAPS"),
        actions=("ROArrive", "RORouteChange", "ROPop", "ROFinish", "ROOffer", "ROCancel", "ROQuiesce"),
        witnesses=(("RONoStaleWitness", "OutboundStale.cfg"), ("RONoConcurrentWitness", "OutboundFIFO.cfg"), ("RONoOvershootWitness", "OutboundBytes.cfg"), ("RONoOfferWitness", "OutboundFIFO.cfg"))),
}
...
tlc = module("completion_tlc_gate", "check-tla.py")
proof = module("completion_proof_gate", "check-tlaps.py")
...
def verdict(output, status, module, expected=None, temporal=False, coverage=False, action=False):
    return tlc.verdict(output, status,
        "EventuallyDrained" if expected in {"RCRecheckProgress", "ROProgress"} else expected,
...
def model_case(args, name, module, config, model, expected=None, fp=0, coverage=False, temporal=False, action=False):
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="kv9-raft-completion-states.") as states:
...
def scope_inventory(inventory, module):
    ...
    return dict(inventory, modules={module + "Proof": inventory["modules"][module + "Proof"]},
                ...
                standard_modules={s: inventory["standard_modules"][s] for s in SCOPES[module]["standards"]})
...
def main():
    ...
    summary = dict(verdict="finite preflight only" if args.model_only else "accepted", affinity=affinity,
        ...
        sources={str(p.relative_to(ROOT)): digest(p) for p in [Path(__file__), ROOT / "scripts/check-tla.py",
            ROOT / "scripts/check-tlaps.py", ROOT / "scripts/raft_completion_controls.py", ROOT / "scripts/ready_controls.py",
