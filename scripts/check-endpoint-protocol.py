#!/usr/bin/env python3
"""Check the endpoint CAS model and its audited parameterized TLAPS proofs."""
import argparse
...
from endpoint_controls import mutations

ROOT = Path(__file__).resolve().parents[1]
SOURCE = ROOT / "proofs/tla/endpoint"
PROOFS = ROOT / "proofs/tlaps/endpoint"
MODULES = ("EndpointCAS.tla", "EndpointMC.tla")
CONFIGS = ("Endpoint2.cfg", "Endpoint3.cfg")
ACTIONS = ("EPIssue", "EPEvaluate", "EPInterfere", "EPRetry")
...
def verdict(output, status, expected=None, coverage=False, temporal=False):
    result = tlc.verdict(output, status, "EventuallyDrained" if expected in ("EPProgress", "EPSuccess") else expected,
                         temporal=temporal, coverage=coverage, action_property=expected == "EPEffects",
                         module="EndpointCAS", actions=ACTIONS)
    return result
...
def model_case(args, name, config, model, expected=None, fp=0, coverage=False):
    ...
    (work / "EndpointCAS.tla").write_text(model)
    (work / "Run.cfg").write_text(config)
    record = dict(name=name, expected=expected, fingerprint=fp, temporal=any(x in config for x in ("EPMCFairSpec", "EPMCStableSpec")),
...
def proof_case(args, name, model, source, expected=None, pattern=None):
    ...
    work.mkdir()
    (work / "EndpointCAS.tla").write_text(model)
    (work / "EndpointCASProof.tla").write_text(source)
    ...
    try:
        ...
        if pattern:
            output = (work / "EndpointCASProof.log").read_text()
...
def main():
    ...
    require({p.name for p in SOURCE.glob("*.cfg")} == set(CONFIGS) | {"EndpointLive.cfg", "EndpointSuccess.cfg"}, "endpoint configuration inventory mismatch")
    require({p.stem for p in PROOFS.glob("*.tla")} == set(inventory["modules"]), "endpoint proof inventory mismatch")
    require(inventory["roots"] == ["EndpointCASProof"] and set(inventory["models"]) == {"EndpointCAS"},
    ...
    model = (SOURCE / "EndpointCAS.tla").read_text()
    source = (PROOFS / "EndpointCASProof.tla").read_text()
    models, proofs = [], []
    ...
    for control in controls:
        ...
        for suffix, text, invalid in (("baseline", model, False), ("mutant", control["model"], True), ("restored", model, False)):
            ...
            proof_group.append(proof_case(args, f"proof-{control['name']}-{suffix}", text, source,
                                          "EndpointCASProof: TLAPS exit 10" if invalid else None,
                                          control["proof_pattern"] if invalid else None))
        check_triple(model_group, "EndpointCAS.tla")
        check_triple(proof_group, "EndpointCAS.tla")
        models.extend(model_group)
        proofs.extend(proof_group)
    live, live_output = model_case(args, "EndpointLive", (SOURCE / "EndpointLive.cfg").read_text(), model)
    models.append(live)
    stop, _ = model_case(args, "EndpointSuccess", (SOURCE / "EndpointSuccess.cfg").read_text(), model)
    models.append(stop)
    for witness in ("EPNoABA", "EPNoConfirmation"):
    ...
    audit_controls = [
        ("omitted-proof", tlc.replace_once(source,
         ...
         "OMITTED"), "proof hole: EndpointCASProof.EPInvariantInit"),
        ("custom-axiom", tlc.replace_once(source, "EXTENDS EndpointCAS, TLAPS",
         "EXTENDS EndpointCAS, TLAPS\nAXIOM FALSE"), "unapproved module assumption: EndpointCASProof"),
    ]
    for name, mutant, reason in audit_controls:
        ...
        check_triple(group, "EndpointCASProof.tla")
        proofs.extend(group)
    ...
    for code in (2192, 2267):
        ...
        verdict(live_output, 0, temporal=True)
        model_outputs += 1
    proof_output = (args.output / "proof-baseline/EndpointCASProof.log").read_text()
    proof_outputs = proof.output_controls(proof_output, "EndpointCASProof", 80)
    summary = dict(jar_sha256=digest(args.jar), tlapm_version=version, tlapm_sha256=digest(args.tlapm),
                   sources={str(p.relative_to(ROOT)): digest(p) for p in [
                       ...
                       ROOT / "scripts/endpoint_controls.py", ROOT / "scripts/ready_controls.py",
    ...
    print(f"PASS: 4 full endpoint model checks and 2 fair continuations; {len(controls)} protocol controls; 2 witnesses; "
          "11 TLAPS theorems; 80 obligations; 2 audit controls; 8 output controls", flush=True)
