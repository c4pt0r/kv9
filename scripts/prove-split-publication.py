#!/usr/bin/env python3
"""Check the split-publication model and explicit defect controls."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    contract = json.loads(
        (root / "proofs/lean/split-publication/source-contract.json").read_text()
    )
    assert contract["schema"] == 1
    for name, digest in contract["source_sha256"].items():
        assert hashlib.sha256((root / name).read_bytes()).hexdigest() == digest, name
    source = (root / "proofs/lean/split-publication/Publication.lean").read_text()
    ...
    def compile_source(label, text, success):
        ...
        directory.mkdir()
        path = directory / "Publication.lean"
        path.write_text(text)
        command = [args.lean, "-DwarningAsError=true", "-o", str(directory / "Publication.olean"), str(path)]
    ...
    audit.write_text("import Publication\n" + "\n".join(
        f"#print axioms Kv9.SplitPublication.{name}" for name in contract["theorems"]) + "\n")
    ...
    for name in contract["theorems"]:
        line = next(line for line in result.stdout.splitlines()
                    if f"Kv9.SplitPublication.{name}'" in line)
    ...
    controls = [
        ('publish-without-the-fence',
         '(i : s.intent = true) (f : s.fenced = true)\n      (lo : s.lowExact = true) (hi : s.highExact = true)',
         '(i : s.intent = true) (f : s.fenced = s.fenced)\n      (lo : s.lowExact = true) (hi : s.highExact = true)'),
        ('publish-without-verified-children',
         '(lo : s.lowExact = true) (hi : s.highExact = true)',
         '(lo : s.lowExact = s.lowExact) (hi : s.highExact = true)'),
        ('publish-partially',
         'Step s {s with published := true, routedToChildren := true}',
         'Step s {s with published := true}'),
        ('publish-loses-a-row',
         'Step s {s with published := true, routedToChildren := true}',
         'Step s {s with published := true, routedToChildren := true, lostData := true}'),
        ('confirm-diverges',
         '| confirm {s} (p : s.published = true) : Step s s',
         '| confirm {s} (p : s.published = true) : Step s {s with routedToChildren := false}'),
        ('fence-publishes',
         '| fence {s} (i : s.intent = true) : Step s {s with fenced := true}',
         '| fence {s} (i : s.intent = true) : Step s {s with fenced := true, published := true}'),
    ]
    ...
    report = {
        ...
        "verified_rust_extraction": False, "partial_publication_or_lost_rows": False,
        "new_chaos_acceptance": False, "new_qps": False,
    }
    (output / "result.json").write_text(json.dumps(report, indent=2) + "\n")
    print(f"SPLIT_PUBLICATION_PROOF_ACCEPTED: {len(contract['theorems'])} theorems; {len(controls)} semantic and 2 policy controls")
