#!/usr/bin/env python3
"""Real-process parent-retirement gate: after the published split, every
replica retires its sealed parent automatically and durably — storage
kept, children serving throughout, restarts opening nothing.
...
def main():
    ...
    env = dict(os.environ, KV9_CLUSTER_TOKEN='pretire-e2e-cluster',
               KV9_CLIENT_TOKENS='admin=pretire-e2e-client', KV9_CLIENT_TOKEN='pretire-e2e-client',
               KV9_BOOTSTRAP_TOKEN='pretire-e2e-bootstrap', KV9_STORAGE='minio')
    ...
    try:
        ...
        manifest_record.update(verdict='accepted', region=region,
                               checks=['the full manual split as previously qualified, through the atomic publication',
                                       'every replica retires its sealed parent automatically under the published directory',
                                       'retirement keeps the durable parent storage on every node',
                                       'the retired state survives restart and nothing reopens',
                                       'the children keep serving the split keyspace throughout'])
