#!/usr/bin/env python3
"""Check the client retry model and its audited parameterized TLAPS proofs."""
import argparse
...
from client_controls import mutations

ROOT = Path(__file__).resolve().parents[1]
SOURCE = ROOT / "proofs/tla/client"
PROOFS = ROOT / "proofs/tlaps/client"
MODULES = ("ClientRetry.tla", "ClientMC.tla")
CONFIGS = ("Client2.cfg", "Client3.cfg")
ACTIONS = ("CRDispatch", "CRRefuse", "CREffect", "CRSuccess", "CRUnknown", "CRStop", "CRTick", "CRQuiesce")
...
tlc = module("client_tlc_gate", "check-tla.py")
proof = module("client_proof_gate", "check-tlaps.py")
require, digest = tlc.require, tlc.digest


def verdict(output, status, expected=None, coverage=False):
    return tlc.verdict(output, status, expected, coverage=coverage,
                       module="ClientRetry", actions=ACTIONS)
...
def model_case(args, name, config, model, expected=None, fp=0, coverage=False):
    ...
    (work / "ClientRetry.tla").write_text(model)
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="kv9-client-tlc-states.") as states:
        ...
        record.update(exit_code=result.returncode, seconds=time.monotonic() - started,
                      statistics=verdict(output, result.returncode, expected, coverage), verdict="accepted")
...
def proof_case(args, name, model, source, expected=None, pattern=None):
    ...
    work.mkdir()
    (work / "ClientRetry.tla").write_text(model)
    (work / "ClientRetryProof.tla").write_text(source)
    ...
    try:
        ...
        if pattern:
            output = (work / "ClientRetryProof.log").read_text()
...
def main():
    ...
    require({p.name for p in SOURCE.glob("*.tla")} == set(MODULES), "client model inventory mismatch")
    require({p.name for p in SOURCE.glob("*.cfg")} == set(CONFIGS), "client configuration inventory mismatch")
    require({p.stem for p in PROOFS.glob("*.tla")} == set(inventory["modules"]), "client proof inventory mismatch")
    require(inventory["roots"] == ["ClientRetryProof"] and set(inventory["models"]) == {"ClientRetry"},
            "client root/model inventory mismatch")
    ...
    model = (SOURCE / "ClientRetry.tla").read_text()
    source = (PROOFS / "ClientRetryProof.tla").read_text()
    models, proofs = [], []
    ...
    for control in controls:
        config = f"SPECIFICATION CRSpec\nCONSTANTS CRMaxAttempts = 3 CRBudget = 3\nINVARIANT {control['property']}\n"
        ...
        for suffix, text, invalid in (("baseline", model, False), ("mutant", control["model"], True), ("restored", model, False)):
            ...
            proof_group.append(proof_case(args, f"proof-{control['name']}-{suffix}", text, source,
                                          "ClientRetryProof: TLAPS exit 10" if invalid else None,
                                          control["proof_pattern"] if invalid else None))
        check_triple(model_group, "ClientRetry.tla")
        check_triple(proof_group, "ClientRetry.tla")
        models.extend(model_group)
        proofs.extend(proof_group)
    for witness in ("CRNoRetriedSuccess", "CRNoLateEffect"):
    ...
    audit_controls = [
        ("omitted-proof", tlc.replace_once(source,
         ...
         "OMITTED"), "proof hole: ClientRetryProof.CRInvariantInit"),
        ("custom-axiom", tlc.replace_once(source, "EXTENDS ClientRetry, TLAPS",
         "EXTENDS ClientRetry, TLAPS\nAXIOM FALSE"), "unapproved module assumption: ClientRetryProof"),
    ]
    for name, mutant, reason in audit_controls:
        ...
        check_triple(group, "ClientRetryProof.tla")
        proofs.extend(group)
    model_outputs = model_output_controls(good_output)
    proof_output = (args.output / "proof-baseline/ClientRetryProof.log").read_text()
    proof_outputs = proof.output_controls(proof_output, "ClientRetryProof", 66)
    summary = dict(jar_sha256=digest(args.jar), tlapm_version=version, tlapm_sha256=digest(args.tlapm),
                   sources={str(p.relative_to(ROOT)): digest(p) for p in [
                       ...
                       ROOT / "scripts/client_controls.py", ROOT / "scripts/ready_controls.py",
    ...
    print("PASS: 4 full client model checks; 3 protocol controls; 2 witnesses; "
          "8 TLAPS theorems; 66 obligations; 2 audit controls; 6 output controls", flush=True)
