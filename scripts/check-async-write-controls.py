#!/usr/bin/env python3
"""Require compiled semantic failures for isolated asynchronous write mutations.
...
ASYNC = 'crates/raft/src/async_apply.rs'
DRIVER = 'crates/raft/src/driver.rs'
RUNTIME = 'crates/server/src/runtime.rs'
GRPC = 'crates/server/src/grpc.rs'
CASES = [
    ('acknowledging-commit-before-apply', DRIVER, [(
        '        // Snapshot the unified watermark BEFORE the ring lock (never',
        """        if self.status().raft_committed >= at.index.0 {
            return Some(Ok(ApplyWaitOutcome::Applied(kv9_common::AppliedPosition {
                term: at.term, index: at.index.0,
            })));
        }
        // Snapshot the unified watermark BEFORE the ring lock (never""")],
     'driver::tests::async_apply_requires_application_and_preserves_the_exact_receipt',
     'commit alone was fabricated as an applied write receipt'),
    ('omitting-registration-wake', ASYNC, [(
        '        owner.signal.notify();', '')],
     'driver::tests::async_apply_registration_after_publication_wakes_an_idle_owner',
     'registration after apply lost the completion wakeup'),
    ('retrying-evicted-receipt', DRIVER, [(
        '            if applied.len() == APPLIED_RING',
        '            if false && applied.len() == APPLIED_RING')],
     'driver::tests::async_apply_eviction_is_unknown_even_when_the_watermark_passed',
     'evicted receipt became a safely retryable replacement'),
    ('proposing-before-reservation', DRIVER, [(
        """        let reservation = self.async_applies.reserve(deadline)?;
        let at = self.propose(command)?;""",
        """        let at = self.propose(command)?;
        let reservation = self.async_applies.reserve(deadline)?;""")],
     'driver::tests::async_apply_capacity_is_reserved_before_any_proposal',
     'capacity refusal already submitted a command'),
    ('releasing-public-budget-before-completion', GRPC, [(
     ...
     'grpc::tests::admission::cancelled_prepared_write_keeps_capacity_across_queue_and_async_wait',
     'async write wait lost its public reservation'),
    ('retrying-unknown-outcome', RUNTIME, [(
        '        Err(e @ ApplyWaitError::Unconfirmed { .. }) => Err(e.into()),',
        '        Err(ApplyWaitError::Unconfirmed { .. }) => Ok(None),')],
     'runtime::tests::async_terminal_outcomes_and_expired_replacements_are_never_retried',
     'terminal asynchronous outcome was retried'),
    ('extending-retry-deadline', RUNTIME, [(
        '        pending = match propose(command.clone(), deadline).await {',
        '        pending = match propose(command.clone(), deadline + Duration::from_secs(1)).await {')],
     'runtime::tests::async_replacement_reuses_one_command_and_deadline_and_returns_the_receipt',
...
def main():
    ...
    try:
        ...
        originals = {name: (tree / name).read_text() for name in (ASYNC, DRIVER, RUNTIME, GRPC)}
        ...
        save()
        print(f'PASS: {len(CASES)} isolated asynchronous write implementation controls', flush=True)
    except Exception as error:
