#!/usr/bin/env python3
"""Audit and package concrete heap COW primitives without closing the index gate."""
import ast
import gzip
import hashlib
import io
...
contract = read(REPO / 'proofs/lean/radix/heap-cow-contract.json')
...
for row in contract['controls']:
    ...
    if row['kind'] == 'semantic':
        ...
        assert 'injectedHeapFalse' in (work / 'Audit.lean.log').read_text()
    else:
...
prior = read(REPO / 'docs/radix-seek-proof-v1/manifest.json')
assert prior['sha256'] == '798c29619a5bef88a7f7719bc3d398529af507120b59eefa92ef683226e4b1d3'
assert sha((REPO / 'docs/radix-seek-proof-v1/evidence.tar.gz').read_bytes()) == prior['sha256']
audit = {'complete': True, 'theorems_checked_together': len(axioms),
         ...
         'heap_cow_scope': contract['scope'],
         'remaining_implementation_scope': 'Complete mutations, native Arc/live allocation correspondence, reclamation, owned buffer storage, representability bridges and exact-source differential execution remain open.',
         'complete_algorithm_gate': False, 'remaining': contract['remaining'],
         'prior_seek_packet_sha256': prior['sha256']}
...
repo_files = set(contract['sources']) | {
    'proofs/lean/radix/heap-cow-contract.json', 'proofs/lean/radix/HEAP-COW-README.md',
    'scripts/resident-radix/heap-cow-proof-publish.py', 'docs/RADIX-HEAP-COW-PROOF.md',
}
...
manifest = {'archive': 'evidence.tar.gz', 'sha256': sha(archive.read_bytes()), 'bytes': archive.stat().st_size,
            ...
            'complete_algorithm_gate': False, 'prior_seek_packet_sha256': prior['sha256']}
