#!/usr/bin/env python3
"""Audit and package live graph ownership and reclamation without closing the index gate."""
import ast
import gzip
import hashlib
import io
...
contract = read(REPO / 'proofs/lean/radix/live-ownership-contract.json')
...
for row in contract['controls']:
    ...
    if row['kind'] == 'semantic':
        ...
        assert 'injectedLiveFalse' in (work / 'Audit.lean.log').read_text()
    else:
...
prior = read(REPO / 'docs/radix-heap-cow-proof-v1/manifest.json')
assert prior['sha256'] == '93034c8c62a3c8b24a6ddd0fea4c37280ea13ef1f641179bcf407f29ad45d9aa'
assert sha((REPO / 'docs/radix-heap-cow-proof-v1/evidence.tar.gz').read_bytes()) == prior['sha256']
audit = {'complete': True, 'theorems_checked_together': len(axioms),
         ...
         'live_ownership_scope': contract['scope'],
         'remaining_implementation_scope': 'Complete mutations, native Arc/concurrent COW and physical payload/allocator correspondence, buffer storage, representability and differential execution remain open.',
         'complete_algorithm_gate': False, 'remaining': contract['remaining'],
         'prior_heap_cow_packet_sha256': prior['sha256']}
...
repo_files = set(contract['sources']) | {
    'proofs/lean/radix/live-ownership-contract.json', 'proofs/lean/radix/LIVE-OWNERSHIP-README.md',
    'scripts/resident-radix/live-ownership-proof-publish.py', 'docs/RADIX-LIVE-OWNERSHIP-PROOF.md',
}
...
manifest = {'archive': 'evidence.tar.gz', 'sha256': sha(archive.read_bytes()), 'bytes': archive.stat().st_size,
            ...
            'complete_algorithm_gate': False, 'prior_heap_cow_packet_sha256': prior['sha256']}
