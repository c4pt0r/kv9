#!/usr/bin/env python3
"""Audit and package insertion correspondence without closing the index gate."""
import ast
import gzip
import hashlib
import io
...
contract = read(REPO / 'proofs/lean/radix/insert-contract.json')
...
prior = read(REPO / 'docs/radix-delete-proof-v1/manifest.json')
assert prior['sha256'] == 'b4d946414fc7ac4a24a26dcda6af6beb2911cbf658c9ab445e10d44437c73750'
audit = {'complete': True, 'theorems_checked_together': len(axioms),
         ...
         'insertion_loop_scope': contract['scope'],
         'remaining_implementation_scope': 'Indexed seek/pending-Vec correspondence, Arc/COW heap ownership, routing-buffer and resident-count premise bridges, and differential execution remain open.',
         'complete_algorithm_gate': False, 'remaining': contract['remaining'],
         'prior_delete_packet_sha256': prior['sha256']}
...
repo_files = set(contract['sources']) | {
    'proofs/lean/radix/insert-contract.json', 'proofs/lean/radix/INSERT-README.md',
    'scripts/resident-radix/insert-proof-publish.py', 'docs/RADIX-INSERT-LOOP-PROOF.md',
}
...
manifest = {'archive': 'evidence.tar.gz', 'sha256': sha(archive.read_bytes()), 'bytes': archive.stat().st_size,
            ...
            'complete_algorithm_gate': False, 'prior_delete_packet_sha256': prior['sha256']}
