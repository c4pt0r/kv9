#!/usr/bin/env python3
"""Audit and package the cursor extension without closing the index gate."""
import ast
import gzip
import hashlib
import io
...
contract = read(REPO / 'proofs/lean/radix/cursor-contract.json')
...
for row in contract['controls']:
    ...
    if row['kind'] == 'semantic':
        ...
        assert 'injectedCursorFalse' in (work / 'Audit.lean.log').read_text()
    else:
...
prior = read(REPO / 'docs/radix-size-proof-v1/manifest.json')
assert prior['sha256'] == '091794484ea1da74276c5464df1f0e47e6a85db8debedb82fa93bc8e4e8424d1'
audit = {'complete': True, 'theorems_checked_together': len(axioms),
         ...
         'cursor_model_scope': 'Exact seek/traversal, two-end range histories, conservation and predecessor; reviewed source mapping, not extraction.',
         'remaining_implementation_scope': 'Actual Rust loops/Vec/frame simulation, Arc/COW heap correspondence, resident word-count premises and differential execution remain open.',
         'complete_algorithm_gate': False, 'remaining': contract['remaining'],
         'prior_size_packet_sha256': prior['sha256']}
...
repo_files = set(contract['sources']) | {
    'proofs/lean/radix/cursor-contract.json', 'proofs/lean/radix/CURSOR-README.md',
    'scripts/resident-radix/cursor-proof-publish.py', 'docs/RADIX-CURSOR-PROOF.md',
}
...
manifest = {'archive': 'evidence.tar.gz', 'sha256': sha(archive.read_bytes()), 'bytes': archive.stat().st_size,
            ...
            'complete_algorithm_gate': False, 'prior_size_packet_sha256': prior['sha256']}
