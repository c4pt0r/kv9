#!/usr/bin/env python3
"""Check insertion action, mutable-slot and machine-word refinement with rejecting controls."""
import hashlib
import json
import os
...
contract = json.loads((PROOFS / 'insert-contract.json').read_text())
...
def check(modules, label):
    ...
    (work / 'Audit.lean').write_text('import InsertWordLoop\n' +
    ...
    for control in contract['controls']:
        ...
        if control['kind'] == 'axiom':
            changed[module] = changed[module].replace('namespace Kv9.Radix',
                'namespace Kv9.Radix\naxiom injectedInsertFalse : False', 1)
