#!/usr/bin/env python3
"""Audit and package iterative deletion correspondence without closing the index gate."""
import ast
import gzip
import hashlib
import io
...
PROOF, DEVELOPMENT, FAILED, DEST = [Path(arg).resolve() for arg in sys.argv[1:]]
assert PROOF.is_relative_to('/mnt/data/kv9-work')
assert DEVELOPMENT.is_relative_to('/mnt/data/kv9-work')
assert FAILED.is_relative_to('/mnt/data/kv9-work')
assert DEST.is_relative_to(REPO / 'docs')
DEST.mkdir(exist_ok=False)
read = lambda path: json.loads(path.read_text())
sha = lambda data: hashlib.sha256(data).hexdigest()
contract = read(REPO / 'proofs/lean/radix/delete-contract.json')
...
for row in contract['controls']:
    ...
    if row['kind'] == 'semantic':
        ...
        assert 'injectedDeleteFalse' in (work / 'Audit.lean.log').read_text()
    else:
...
prior = read(REPO / 'docs/radix-cursor-proof-v1/manifest.json')
assert prior['sha256'] == 'fb53d91310bf49c55776dd3f9f7f330d8fc2764e50123d45ba93ad4c5f02bc6d'
failed = read(FAILED / 'result.json')
assert failed['accepted'] is False and failed['complete'] is False
assert 'no substantive proof failure: restore-wrong-index' in failed['failure']
failed_log = (FAILED / 'control-restore-wrong-index/DeleteLoop.lean.log').read_text()
assert 'Application type mismatch' in failed_log
for name, digest in failed['sources'].items():
    if name != 'scripts/resident-radix/prove-delete.py':
        assert digest == contract['sources'][name], name
audit = {'complete': True, 'theorems_checked_together': len(axioms),
         ...
         'prior_qualification_failure_retained': failed['failure'],
         'classification_fix': 'Recognize Lean Application type mismatch as a substantive proof rejection; the incorrect restore index already failed the compiler.',
         'sources_match': True, 'protected_drafts_unchanged': True,
         'candidate_rust_and_production_unchanged': True,
         'deletion_loop_scope': contract['scope'],
         'remaining_implementation_scope': 'Insertion and indexed seek loop correspondence, Arc/COW heap ownership, resident word-count premises and differential execution remain open.',
         'complete_algorithm_gate': False, 'remaining': contract['remaining'],
         'prior_cursor_packet_sha256': prior['sha256']}
(DEST / 'audit.json').write_text(json.dumps(audit, indent=2) + '\n')
(DEST / 'result.json').write_text(json.dumps(result, indent=2) + '\n')
inputs = []
for prefix, root in [('qualification', PROOF), ('development', DEVELOPMENT), ('prior-qualification', FAILED)]:
...
repo_files = set(contract['sources']) | {
    'proofs/lean/radix/delete-contract.json', 'proofs/lean/radix/DELETE-README.md',
    'scripts/resident-radix/delete-proof-publish.py', 'docs/RADIX-DELETE-LOOP-PROOF.md',
}
...
manifest = {'archive': 'evidence.tar.gz', 'sha256': sha(archive.read_bytes()), 'bytes': archive.stat().st_size,
            ...
            'complete_algorithm_gate': False, 'prior_cursor_packet_sha256': prior['sha256']}
