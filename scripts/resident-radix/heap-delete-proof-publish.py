#!/usr/bin/env python3
"""Audit and package concrete graph deletion without closing the index gate."""
import ast
import gzip
import hashlib
import io
...
contract = read(REPO / 'proofs/lean/radix/heap-delete-contract.json')
...
prior = read(REPO / 'docs/radix-heap-insert-proof-v1/manifest.json')
assert prior['sha256'] == 'f097ecf28febe77a47d301ab95ef9ae51ffd4baca76478542d9e054cae548669'
assert sha((REPO / 'docs/radix-heap-insert-proof-v1/evidence.tar.gz').read_bytes()) == prior['sha256']
drop_observation = read(DEVELOPMENT / 'leaf-scope-drop-observation.json')
assert drop_observation['source_sha256'] == contract['sources']['scripts/resident-radix/src/lib.rs']
assert sha((DEVELOPMENT / 'radix-source.mir').read_bytes()) == drop_observation['mir_sha256']
assert sha((DEVELOPMENT / 'remove-mut.mir').read_bytes()) == drop_observation['remove_mut_mir_sha256']
assert drop_observation['status'] == 'observed_compiler_output_not_a_compiler_proof'
audit = {'complete': True, 'theorems_checked_together': len(axioms),
         ...
         'heap_deletion_scope': contract['scope'],
         'leaf_scope_drop_observation': drop_observation,
         'remaining_implementation_scope': 'Native Arc/concurrent COW and physical payload/allocator correspondence, machine-word/storage bridges and differential execution remain open.',
         'complete_algorithm_gate': False, 'remaining': contract['remaining'],
         'prior_heap_insertion_packet_sha256': prior['sha256']}
...
repo_files = set(contract['sources']) | {
    'proofs/lean/radix/heap-delete-contract.json', 'proofs/lean/radix/HEAP-DELETE-README.md',
    'scripts/resident-radix/heap-delete-proof-publish.py', 'docs/RADIX-HEAP-DELETE-PROOF.md',
}
...
manifest = {'archive': 'evidence.tar.gz', 'sha256': sha(archive.read_bytes()), 'bytes': archive.stat().st_size,
            ...
            'complete_algorithm_gate': False, 'prior_heap_insertion_packet_sha256': prior['sha256']}
