#!/usr/bin/env python3
"""Audit and package indexed seek correspondence without closing the index gate."""
import ast
import gzip
import hashlib
import io
...
contract = read(REPO / 'proofs/lean/radix/seek-contract.json')
...
for row in contract['controls']:
    ...
    if row['kind'] == 'semantic':
        ...
        assert 'injectedSeekFalse' in (work / 'Audit.lean.log').read_text()
    else:
...
prior = read(REPO / 'docs/radix-insert-proof-v1/manifest.json')
assert prior['sha256'] == '0bb26bd499b3482b537968b3971fdafaaf1ad24dd9b562a058d0d9c5af98b62b'
audit = {'complete': True, 'theorems_checked_together': len(axioms),
         ...
         'indexed_seek_scope': contract['scope'],
         'remaining_implementation_scope': 'Arc/COW heap ownership, routing-buffer/query-length/resident-count premise bridges and exact-source differential execution remain open.',
         'complete_algorithm_gate': False, 'remaining': contract['remaining'],
         'prior_insert_packet_sha256': prior['sha256']}
...
repo_files = set(contract['sources']) | {
    'proofs/lean/radix/seek-contract.json', 'proofs/lean/radix/SEEK-README.md',
    'scripts/resident-radix/seek-proof-publish.py', 'docs/RADIX-SEEK-LOOP-PROOF.md',
}
...
manifest = {'archive': 'evidence.tar.gz', 'sha256': sha(archive.read_bytes()), 'bytes': archive.stat().st_size,
            ...
            'complete_algorithm_gate': False, 'prior_insert_packet_sha256': prior['sha256']}
