#!/usr/bin/env python3
"""Audit and package native object/COW primitives without closing the index gate."""
import ast
import gzip
import hashlib
import io
...
contract = read(REPO / 'proofs/lean/radix/native-primitives-contract.json')
...
for row in contract['controls']:
    ...
    if row['kind'] == 'semantic':
        ...
        assert 'injectedNativeFalse' in (work / 'Audit.lean.log').read_text()
    else:
...
prior = read(REPO / 'docs/radix-heap-delete-proof-v1/manifest.json')
assert prior['sha256'] == '674294cf93f8fbb04747eec7185bf7e52a894cce60f37ec163d47235d0cc692a'
assert sha((REPO / 'docs/radix-heap-delete-proof-v1/evidence.tar.gz').read_bytes()) == prior['sha256']
layout_observation = read(DEVELOPMENT / 'layout-observation.json')
assert read(PROOF / 'native-library-pins.json') == read(REPO / 'proofs/lean/radix/native-library-pins.json')
candidate = (REPO / 'scripts/resident-radix/src/lib.rs').read_bytes()
observer = (DEVELOPMENT / 'layout-observer.rs').read_bytes()
assert layout_observation['status'] == 'observed_layout_not_a_compiler_proof'
assert sha(candidate) == layout_observation['candidate_sha256']
assert observer[:layout_observation['exact_candidate_prefix_bytes']] == candidate
assert sha(observer) == layout_observation['observer_sha256']
assert sha((DEVELOPMENT / 'layout-observer').read_bytes()) == layout_observation['executable_sha256']
assert sha(Path(layout_observation['command'][0]).read_bytes()) == layout_observation['compiler_sha256']
for name, digest in layout_observation['linked_rust_library_inputs'].items():
...
audit = {'complete': True, 'theorems_checked_together': len(axioms),
         ...
         'native_primitives_scope': contract['scope'],
         'layout_observation': layout_observation,
         'remaining_implementation_scope': 'Mutation-place applications, external clone interference, native buffers/transient payload placement, physical words and differential execution remain open.',
         'complete_algorithm_gate': False, 'remaining': contract['remaining'],
         'prior_heap_deletion_packet_sha256': prior['sha256']}
...
for prefix, root in [('qualification', PROOF), ('development', DEVELOPMENT)]:
    for path in sorted(root.rglob('*')):
        if not path.is_file() or '__pycache__' in path.parts:
            continue
        if '.olean' in path.name or path.suffix in ['.ilean', '.c', '.o'] or path.name == 'layout-observer':
            continue
        inputs.append((prefix + '/' + str(path.relative_to(root)), path))
repo_files = set(contract['sources']) | {
    'proofs/lean/radix/native-primitives-contract.json', 'proofs/lean/radix/NATIVE-PRIMITIVES-README.md',
    'scripts/resident-radix/native-primitives-proof-publish.py', 'docs/RADIX-NATIVE-PRIMITIVES-PROOF.md',
}
...
manifest = {'archive': 'evidence.tar.gz', 'sha256': sha(archive.read_bytes()), 'bytes': archive.stat().st_size,
            ...
            'all_members_read_back': True, 'excluded': ['Lean compiler outputs', 'Python bytecode', 'layout observer executable (hash in receipt)'],
            'complete_algorithm_gate': False, 'prior_heap_deletion_packet_sha256': prior['sha256']}
