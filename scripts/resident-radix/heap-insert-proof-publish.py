#!/usr/bin/env python3
"""Audit and package concrete graph insertion without closing the index gate."""
import ast
import gzip
import hashlib
import io
...
contract = read(REPO / 'proofs/lean/radix/heap-insert-contract.json')
...
for row in contract['controls']:
    ...
    if row['kind'] == 'semantic':
        ...
        assert 'injectedInsertFalse' in (work / 'Audit.lean.log').read_text()
    else:
...
prior = read(REPO / 'docs/radix-live-ownership-proof-v1/manifest.json')
assert prior['sha256'] == '1fb8f82dd588b7d10f14ea1a8393aa88429ec9689c188a02eda5a3936007e05c'
assert sha((REPO / 'docs/radix-live-ownership-proof-v1/evidence.tar.gz').read_bytes()) == prior['sha256']
audit = {'complete': True, 'theorems_checked_together': len(axioms),
         ...
         'heap_insertion_scope': contract['scope'],
         'remaining_implementation_scope': 'Detached deletion/normalization, native Arc/concurrent COW and physical payload/allocator correspondence, machine-word/storage bridges and differential execution remain open.',
         'complete_algorithm_gate': False, 'remaining': contract['remaining'],
         'prior_live_ownership_packet_sha256': prior['sha256']}
...
repo_files = set(contract['sources']) | {
    'proofs/lean/radix/heap-insert-contract.json', 'proofs/lean/radix/HEAP-INSERT-README.md',
    'scripts/resident-radix/heap-insert-proof-publish.py', 'docs/RADIX-HEAP-INSERT-PROOF.md',
}
...
manifest = {'archive': 'evidence.tar.gz', 'sha256': sha(archive.read_bytes()), 'bytes': archive.stat().st_size,
            ...
            'complete_algorithm_gate': False, 'prior_live_ownership_packet_sha256': prior['sha256']}
