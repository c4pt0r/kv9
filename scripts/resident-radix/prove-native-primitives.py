#!/usr/bin/env python3
"""Check native object storage and concurrent COW primitives with rejecting controls."""
import hashlib
import json
import os
...
contract = json.loads((PROOFS / 'native-primitives-contract.json').read_text())
...
def check(modules, label):
    ...
    (work / 'Audit.lean').write_text('import NativeCowExamples\n' +
    ...
    for control in contract['controls']:
        ...
        if control['kind'] == 'axiom':
            changed[module] = changed[module].replace('namespace Kv9.Radix',
                'namespace Kv9.Radix\naxiom injectedNativeFalse : False', 1)
