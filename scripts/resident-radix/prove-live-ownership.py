#!/usr/bin/env python3
"""Check live ownership and iterative/interleaved graph reclamation with rejecting controls."""
import hashlib
import json
import os
...
contract = json.loads((PROOFS / 'live-ownership-contract.json').read_text())
...
def check(modules, label):
    ...
    (work / 'Audit.lean').write_text('import HeapDropExamples\n' +
    ...
    for control in contract['controls']:
        ...
        if control['kind'] == 'axiom':
            changed[module] = changed[module].replace('namespace Kv9.Radix',
                'namespace Kv9.Radix\naxiom injectedLiveFalse : False', 1)
