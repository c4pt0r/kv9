#!/usr/bin/env python3
"""Check radix cardinality and scoped word/bounds proofs with rejecting controls."""
import hashlib
import json
import os
...
contract = json.loads((PROOFS / 'size-contract.json').read_text())
...
def check(modules, label):
    ...
    (work / 'Audit.lean').write_text('import MachineBounds\n' +
    ...
    for control in contract['controls']:
        ...
        if control['kind'] == 'axiom':
            changed[module] = changed[module].replace('namespace Kv9.Radix',
                'namespace Kv9.Radix\naxiom injectedCardinalityFalse : False', 1)
