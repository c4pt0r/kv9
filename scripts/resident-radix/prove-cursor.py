#!/usr/bin/env python3
"""Check radix cursor, range and predecessor refinement with rejecting controls."""
import hashlib
import json
import os
...
contract = json.loads((PROOFS / 'cursor-contract.json').read_text())
...
def check(modules, label):
    ...
    (work / 'Audit.lean').write_text('import RangeProperties\n' +
    ...
    for control in contract['controls']:
        ...
        if control['kind'] == 'axiom':
            changed[module] = changed[module].replace('namespace Kv9.Radix',
                'namespace Kv9.Radix\naxiom injectedCursorFalse : False', 1)
