#!/usr/bin/env python3
"""Check the route ownership model and its audited parameterized TLAPS proofs."""
import argparse
...
from route_controls import mutations

ROOT = Path(__file__).resolve().parents[1]
SOURCE = ROOT / "proofs/tla/route"
PROOFS = ROOT / "proofs/tlaps/route"
MODULES = ("RouteOwnership.tla", "RouteMC.tla")
CONFIGS = ("Route2.cfg", "Route3.cfg")
ACTIONS = ("RTSpawn", "RTObserve", "RTEnqueue", "RTDiscard", "RTTake", "RTEmit", "RTConnect", "RTFail", "RTBlock", "RTDrop", "RTClose", "RTTerminate", "RTQuiesce")
...
tlc = module("route_tlc_gate", "check-tla.py")
proof = module("route_proof_gate", "check-tlaps.py")
require, digest = tlc.require, tlc.digest


def verdict(output, status, expected=None, coverage=False, temporal=False):
    result = tlc.verdict(output, status, "EventuallyDrained" if expected in ("RTProgress", "RTStopProgress") else expected,
                         temporal=temporal, coverage=coverage, action_property=expected == "RTEffects",
                         module="RouteOwnership", actions=ACTIONS)
    if expected is None and coverage:
        hits = re.findall(r"^<RTMCUpdate line [^\n]+ of module RouteMC>: (\d+):(\d+)$", output, re.M)
        require(len(hits) == 1 and int(hits[0][1]) > 0, "missing bounded route-update coverage")
    return result
...
def model_case(args, name, config, model, expected=None, fp=0, coverage=False):
    ...
    (work / "RouteOwnership.tla").write_text(model)
    (work / "Run.cfg").write_text(config)
    record = dict(name=name, expected=expected, fingerprint=fp, temporal=any(x in config for x in ("RTMCLiveSpec", "RTMCStopSpec")),
                  sources={p.name: digest(p) for p in sorted(work.iterdir())})
    started = time.monotonic()
    try:
        with tempfile.TemporaryDirectory(prefix="kv9-route-tlc-states.") as states:
...
def proof_case(args, name, model, source, expected=None, pattern=None):
    ...
    work.mkdir()
    (work / "RouteOwnership.tla").write_text(model)
    (work / "RouteOwnershipProof.tla").write_text(source)
    ...
    try:
        ...
        if pattern:
            output = (work / "RouteOwnershipProof.log").read_text()
...
def main():
    ...
    require({p.name for p in SOURCE.glob("*.tla")} == set(MODULES), "route model inventory mismatch")
    require({p.name for p in SOURCE.glob("*.cfg")} == set(CONFIGS) | {"RouteLive.cfg", "RouteStop.cfg"}, "route configuration inventory mismatch")
    require({p.stem for p in PROOFS.glob("*.tla")} == set(inventory["modules"]), "route proof inventory mismatch")
    require(inventory["roots"] == ["RouteOwnershipProof"] and set(inventory["models"]) == {"RouteOwnership"},
            "route root/model inventory mismatch")
    ...
    model = (SOURCE / "RouteOwnership.tla").read_text()
    source = (PROOFS / "RouteOwnershipProof.tla").read_text()
    models, proofs = [], []
    ...
    for control in controls:
        ...
        for suffix, text, invalid in (("baseline", model, False), ("mutant", control["model"], True), ("restored", model, False)):
            ...
            proof_group.append(proof_case(args, f"proof-{control['name']}-{suffix}", text, source,
                                          "RouteOwnershipProof: TLAPS exit 10" if invalid else None,
                                          control["proof_pattern"] if invalid else None))
        check_triple(model_group, "RouteOwnership.tla")
        check_triple(proof_group, "RouteOwnership.tla")
        models.extend(model_group)
        proofs.extend(proof_group)
    live, live_output = model_case(args, "RouteLive", (SOURCE / "RouteLive.cfg").read_text(), model)
    models.append(live)
    stop, _ = model_case(args, "RouteStop", (SOURCE / "RouteStop.cfg").read_text(), model)
    models.append(stop)
    for witness in ("RTNoAddressReuse", "RTNoDelivery"):
    ...
    audit_controls = [
        ("omitted-proof", tlc.replace_once(source,
         ...
         "OMITTED"), "proof hole: RouteOwnershipProof.RTInvariantInit"),
        ("custom-axiom", tlc.replace_once(source, "EXTENDS RouteOwnership, TLAPS",
         "EXTENDS RouteOwnership, TLAPS\nAXIOM FALSE"), "unapproved module assumption: RouteOwnershipProof"),
    ]
    for name, mutant, reason in audit_controls:
        ...
        check_triple(group, "RouteOwnershipProof.tla")
        proofs.extend(group)
    ...
    for code in (2192, 2267):
        ...
        verdict(live_output, 0, temporal=True)
        model_outputs += 1
    proof_output = (args.output / "proof-baseline/RouteOwnershipProof.log").read_text()
    proof_outputs = proof.output_controls(proof_output, "RouteOwnershipProof", 197)
    summary = dict(jar_sha256=digest(args.jar), tlapm_version=version, tlapm_sha256=digest(args.tlapm),
                   sources={str(p.relative_to(ROOT)): digest(p) for p in [
                       ...
                       ROOT / "scripts/route_controls.py", ROOT / "scripts/ready_controls.py",
    ...
    print(f"PASS: 4 full route model checks and 2 fair continuations; {len(controls)} protocol controls; 2 witnesses; "
          "17 TLAPS theorems; 197 obligations; 2 audit controls; 8 output controls", flush=True)
