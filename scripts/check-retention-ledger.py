#!/usr/bin/env python3
"""Check atomic retention ownership, strict proofs and isolated fault controls.
...
MODEL = ROOT / 'proofs/tla/retention_ledger'
PROOF = ROOT / 'proofs/tlaps/retention_ledger'
...
def model_case(args, name, model, config, expected=None, mc=None):
    ...
    work.mkdir()
    (work / 'RetentionLedger.tla').write_text(model)
    (work / 'RetentionLedgerMC.tla').write_text((MODEL / 'RetentionLedgerMC.tla').read_text() if mc is None else mc)
    (work / 'Run.cfg').write_text(config)
    command = ['java', f'-Djava.io.tmpdir={work}', '-Xmx256m', '-cp', str(args.jar),
               'tlc2.TLC', '-tool', '-workers', '1', '-fp', '0',
               '-metadir', str(work / 'states'), '-config', 'Run.cfg', 'RetentionLedgerMC.tla']
    ...
    try:
        ...
        record['statistics'] = tlc.verdict(
            ...
            temporal='PROPERTY' in config, module='RetentionLedger',
...
def proof_case(args, name, model, source, expected=None):
    ...
    work.mkdir()
    (work / 'RetentionLedger.tla').write_text(model)
    (work / 'RetentionLedgerProof.tla').write_text(source)
    ...
    try:
        ...
        if expected is None or expected not in str(error):
            record.update(status='rejected', reason=str(error))
            raise
        if expected == 'RetentionLedgerProof: TLAPS exit 10':
            log = (work / 'RetentionLedgerProof.log').read_text()
...
def main():
    ...
    inputs = ['scripts/check-retention-ledger.py', 'scripts/check-tla.py', 'scripts/check-tlaps.py', 'scripts/ProofAudit.java', *inventory['source_pins']]
    ...
    model = (MODEL / 'RetentionLedger.tla').read_text()
    source = (PROOF / 'RetentionLedgerProof.tla').read_text()
    results = {'proof': proof_case(args, 'proof', model, source)}
    valid = (MODEL / 'Ledger.cfg').read_text()
    full = valid.replace('LClosure <- ModelClosure', 'LClosure <- FullClosure')
    separate = full.replace('LSubject <- ModelSubject', 'LSubject <- SeparateSubjects').replace('LSubjects = {1}', 'LSubjects = {1,2}')
    for name, config in [('partial-closure', valid), ('full-closure', full), ('separate-subjects', separate)]:
        results[name] = model_case(args, name, model, config)
    mutations = {
        'missing-resource-atomicity': ("/\\ lPins' = LPinsAfter(o, LRecord(g, phase))", "/\\ lPins' = lPins", valid, 'LExactLinks'),
        'release-published-source': ('lOwners[o].phase = "quiesced" /\\ g = lOwners[o].generation',
            'lOwners[o].phase \\in {"quiesced", "published"} /\\ g = lOwners[o].generation', full, 'LPublicationCovered'),
        'stale-publication-token': ('lOwners[o].phase = "held" /\\ g = lOwners[o].generation',
            'lOwners[o].phase = "held" /\\ TRUE', full, 'LInvariant'),
        'unpublished-successor': ('lOwners[to].phase = "published"',
            'lOwners[to].phase \\in {"held", "published"}', full, 'LPublicationCovered'),
        'partial-successor-closure': ('LSubject[from] = LSubject[to] /\\ LClosure[from] = LClosure[to]\n    /\\ LSet(from',
            'LSubject[from] = LSubject[to] /\\ TRUE\n    /\\ LSet(from', valid, 'LPublicationCovered'),
        'unrelated-successor-subject': ('LSubject[from] = LSubject[to] /\\ LClosure[from] = LClosure[to]\n    /\\ LSet(from',
            'TRUE /\\ LClosure[from] = LClosure[to]\n    /\\ LSet(from', separate, 'LPublicationCovered'),
    }
    for name, (before, after, config, expected) in mutations.items():
        results[name] = model_case(args, name, proof.replace_once(model, before, after), config, expected)
    start = source.index('BY LInputs, SMT DEF LInit')
    end = source.index('THEOREM LInvariantStep')
    results['proof-hole'] = proof_case(args, 'proof-hole', model, source[:start] + 'PROOF OMITTED\n' + source[end:], 'proof hole:')
    axiom = proof.replace_once(source, 'EXTENDS RetentionLedger, TLAPS', 'EXTENDS RetentionLedger, TLAPS\nAXIOM UnsafeShortcut == FALSE')
    results['custom-axiom'] = proof_case(args, 'custom-axiom', model, axiom, 'unapproved module assumption:')
    unsafe = proof.replace_once(model, *mutations['unpublished-successor'][:2])
    results['unsafe-transfer-proof'] = proof_case(args, 'unsafe-transfer-proof', unsafe, source, 'RetentionLedgerProof: TLAPS exit 10')
    save(args.output / 'summary.json', dict(status='PASS', theorems=7, obligations=55, positive_models=3,
        model_counterexamples=6, proof_rejections=3, results=results, source_pins=inventory['source_pins'],
        scope='Committed-state projection with immutable validated owner bindings, exact resource closures and atomic data/position application. Metadata planner freshness and Raft consensus are compositional premises. No Rust refinement, reference publication/settlement, complete legacy coverage, destination admission, reader drainage or deletion proof.'))
    print('PASS: retention ledger: 7 theorems / 55 obligations; 3 models, 6 counterexamples, 3 proof rejections', flush=True)
