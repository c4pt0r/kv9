#!/usr/bin/env python3
"""Check the storage-retirement model and explicit defect controls."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    contract = json.loads(
        (root / "proofs/lean/storage-retirement/source-contract.json").read_text()
    )
    assert contract["schema"] == 1
    for name, digest in contract["source_sha256"].items():
        assert hashlib.sha256((root / name).read_bytes()).hexdigest() == digest, name
    source = (root / "proofs/lean/storage-retirement/Retirement.lean").read_text()
    ...
    def compile_source(label, text, success):
        ...
        directory.mkdir()
        path = directory / "Retirement.lean"
        path.write_text(text)
        command = [args.lean, "-DwarningAsError=true", "-o", str(directory / "Retirement.olean"), str(path)]
    ...
    audit.write_text("import Retirement\n" + "\n".join(
        f"#print axioms Kv9.StorageRetirement.{name}" for name in contract["theorems"]) + "\n")
    ...
    for name in contract["theorems"]:
        line = next(line for line in result.stdout.splitlines()
                    if f"Kv9.StorageRetirement.{name}'" in line)
    ...
    controls = [
        ('retire-without-the-committed-removal',
         '| retire {s} (d : s.removalCommitted = true) :',
         '| retire {s} (d : s.removalCommitted = s.removalCommitted) :'),
        ('retire-keeps-running',
         'Step s {s with retired := true, running := false}',
         'Step s {s with retired := true}'),
        ('retire-deletes-storage',
         'Step s {s with retired := true, running := false}',
         'Step s {s with retired := true, running := false, storageDeleted := true}'),
        ('retire-serves',
         'Step s {s with retired := true, running := false}',
         'Step s {s with retired := true, running := false, serving := true}'),
        ('run-after-retirement',
         '| run {s} (fresh : s.retired = false) : Step s {s with running := true}',
         '| run {s} (fresh : s.retired = s.retired) : Step s {s with running := true}'),
        ('commit-serves',
         '| commit {s} : Step s {s with removalCommitted := true}',
         '| commit {s} : Step s {s with removalCommitted := true, serving := true}'),
    ]
    ...
    report = {
        ...
        "verified_rust_extraction": False, "retirement_without_decision_or_deletion": False,
        "new_chaos_acceptance": False, "new_qps": False,
    }
    (output / "result.json").write_text(json.dumps(report, indent=2) + "\n")
    print(f"STORAGE_RETIREMENT_PROOF_ACCEPTED: {len(contract['theorems'])} theorems; {len(controls)} semantic and 2 policy controls")
