...
fn main() {
    ...
    let mut row = if operation == "write" {
        if !cfg!(feature = "allocation-counting") {
        ...
        if !cfg!(feature = "allocation-counting") {
            black_box(read_case::<RedBlackTreeMapSync<Vec<u8>, Vec<u8>>>(
                work, operation, &probes, 1,
            ));
        }
        read_case::<RedBlackTreeMapSync<Vec<u8>, Vec<u8>>>(work, operation, &probes, passes)
    };
    ...
    emit(
        &output,
        &json!({"complete":true,"counting_build":cfg!(feature="allocation-counting"),"input_plan_sha256":hash(&plan_bytes),"group_file_sha256":hash(&group_bytes),"workloads":metadata,"rows":[row],
        "scope":"Standalone rpds calls only; two isolated dependency executables. Owned input clones in timed writes, borrowed point/predecessor returns and owned scan16. Per-call timer overhead remains. No engine/WAL/Raft/RPC/database-QPS claim. Allocation counts are requested bytes, not RSS/usable sizes."}),
    );
}
