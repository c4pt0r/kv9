//! Bounded comparison of unchanged MemEngine and ReadView interfaces.
...
impl Metrics {
    ...
    fn report(&self) -> Value {
        ...
        assert!(count > 0);
        let percentile = |q: usize| sorted[(count * q).div_ceil(100) - 1];
        let sum: u64 = self.samples.iter().sum();
        json!({"windows":count,"sum_ns":sum,"mean_ns":sum as f64/count as f64,
            "p50_ns":percentile(50),"p95_ns":percentile(95),"p99_ns":percentile(99),"samples_ns":self.samples})
    }
}
...
fn measure_snapshots(work: &Workload) -> Vec<Value> {
    ...
    vec![
        json!({"phase":"steady_snapshot","unit":"ns_per_snapshot","passes":12,"queries_per_pass":512,
...
fn main() {
    ...
    if args[3] == "prepare" {
        ...
        let measured = match spec["operation"].as_str().unwrap() {
