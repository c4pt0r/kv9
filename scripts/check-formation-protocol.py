#!/usr/bin/env python3
"""Check the root catalog formation model and its audited parameterized TLAPS proofs."""
import argparse
...
from formation_controls import mutations

ROOT = Path(__file__).resolve().parents[1]
SOURCE = ROOT / "proofs/tla/formation"
PROOFS = ROOT / "proofs/tlaps/formation"
MODULES = ("RootFormation.tla", "FormationMC.tla")
CONFIGS = ("Formation2.cfg", "Formation3.cfg")
ACTIONS = ("RFRecover", "RFCrash", "RFElectAny", "RFDrain", "RFPlan", "RFAppend", "RFCommit", "RFMark", "RFWriteUser", "RFQuiesce")
...
tlc = module("formation_tlc_gate", "check-tla.py")
proof = module("formation_proof_gate", "check-tlaps.py")
require, digest = tlc.require, tlc.digest


def verdict(output, status, expected=None, coverage=False, temporal=False):
    return tlc.verdict(output, status, "EventuallyDrained" if expected == "RFProgress" else expected,
                       temporal=temporal, coverage=coverage, action_property=expected == "RFSafety",
                       module="RootFormation", actions=ACTIONS)
...
def model_case(args, name, config, model, expected=None, fp=0, coverage=False):
    ...
    (work / "RootFormation.tla").write_text(model)
    (work / "Run.cfg").write_text(config)
    record = dict(name=name, expected=expected, fingerprint=fp, temporal="RFFairSpec" in config,
                  sources={p.name: digest(p) for p in sorted(work.iterdir())})
    started = time.monotonic()
    try:
        with tempfile.TemporaryDirectory(prefix="kv9-formation-tlc-states.") as states:
...
def proof_case(args, name, model, source, expected=None, pattern=None):
    ...
    work.mkdir()
    (work / "RootFormation.tla").write_text(model)
    (work / "RootFormationProof.tla").write_text(source)
    ...
    try:
        ...
        if pattern:
            output = (work / "RootFormationProof.log").read_text()
...
def main():
    ...
    require({p.name for p in SOURCE.glob("*.tla")} == set(MODULES), "formation model inventory mismatch")
    require({p.name for p in SOURCE.glob("*.cfg")} == set(CONFIGS) | {"FormationLive.cfg"}, "formation configuration inventory mismatch")
    require({p.stem for p in PROOFS.glob("*.tla")} == set(inventory["modules"]), "formation proof inventory mismatch")
    require(inventory["roots"] == ["RootFormationProof"] and set(inventory["models"]) == {"RootFormation"},
            "formation root/model inventory mismatch")
    ...
    model = (SOURCE / "RootFormation.tla").read_text()
    source = (PROOFS / "RootFormationProof.tla").read_text()
    models, proofs = [], []
    ...
    for control in controls:
        config = control.get("config", f"SPECIFICATION RFSpec\nCONSTANTS RFRoots = {{1, 2}} RFRoot = 1 RFMaxTerm = 2\nINVARIANT {control['property']}\n")
        ...
        for suffix, text, invalid in (("baseline", model, False), ("mutant", control["model"], True), ("restored", model, False)):
            ...
            proof_group.append(proof_case(args, f"proof-{control['name']}-{suffix}", text, source,
                                          "RootFormationProof: TLAPS exit 10" if invalid else None,
                                          control["proof_pattern"] if invalid else None))
        check_triple(model_group, "RootFormation.tla")
        check_triple(proof_group, "RootFormation.tla")
        models.extend(model_group)
        proofs.extend(proof_group)
    live, live_output = model_case(args, "FormationLive", (SOURCE / "FormationLive.cfg").read_text(), model)
    models.append(live)
    for witness in ("RFNoUser", "RFNoUnappliedRoot"):
    ...
    audit_controls = [
        ("omitted-proof", tlc.replace_once(source,
         ...
         "OMITTED"), "proof hole: RootFormationProof.RFInvariantInit"),
        ("custom-axiom", tlc.replace_once(source, "EXTENDS RootFormation, TLAPS",
         "EXTENDS RootFormation, TLAPS\nAXIOM FALSE"), "unapproved module assumption: RootFormationProof"),
    ]
    for name, mutant, reason in audit_controls:
        ...
        check_triple(group, "RootFormationProof.tla")
        proofs.extend(group)
    ...
    for code in (2192, 2267):
        ...
        verdict(live_output, 0, temporal=True)
        model_outputs += 1
    proof_output = (args.output / "proof-baseline/RootFormationProof.log").read_text()
    proof_outputs = proof.output_controls(proof_output, "RootFormationProof", 153)
    summary = dict(jar_sha256=digest(args.jar), tlapm_version=version, tlapm_sha256=digest(args.tlapm),
                   sources={str(p.relative_to(ROOT)): digest(p) for p in [
                       ...
                       ROOT / "scripts/formation_controls.py", ROOT / "scripts/ready_controls.py",
    ...
    print(f"PASS: 4 full formation model checks and 1 fair continuation; {len(controls)} protocol controls; 2 witnesses; "
          "14 TLAPS theorems; 153 obligations; 2 audit controls; 8 output controls", flush=True)
