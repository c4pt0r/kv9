#!/usr/bin/env python3
"""Check Raft method isolation, bounded queues and explicit defect controls."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    contract = json.loads((root / "proofs/lean/group-wire/source-contract.json").read_text())
    assert contract["schema"] == 1
    for name, digest in contract["source_sha256"].items():
        assert hashlib.sha256((root / name).read_bytes()).hexdigest() == digest, name
    source = (root / "proofs/lean/group-wire/Wire.lean").read_text()
    ...
    def compile_source(label, text, success):
        ...
        directory.mkdir()
        path = directory / "Wire.lean"
        path.write_text(text)
        command = [args.lean, "-DwarningAsError=true", "-o", str(directory / "Wire.olean"), str(path)]
    ...
    audit.write_text("import Wire\n" + "\n".join(
        f"#print axioms Kv9.GroupWire.{name}" for name in contract["theorems"]) + "\n")
    ...
    for name in contract["theorems"]:
        line = next(line for line in result.stdout.splitlines() if f"Kv9.GroupWire.{name}'" in line)
    ...
    controls = [
        ("fallback-to-legacy", "if 1 < region then some .data else none", "if 1 < region then some .metadata else none"),
        ("legacy-accepts-data", "| .legacy, .data => none", "| .legacy, .data => some 0"),
        ("missing-generation-fence", "if current = queued then", "if True then"),
        ("missing-receiver-domain", "if 1 < region then some region else none", "some region"),
        ("shared-queue", "| .data => {q with data := if q.data < capacity then q.data + 1 else q.data}",
         "| .data => {q with metadata := if q.metadata < capacity then q.metadata + 1 else q.metadata}"),
        ("unbounded-queue", "if q.metadata < capacity then q.metadata + 1 else q.metadata", "q.metadata + 1"),
    ]
    ...
    print("GROUP_WIRE_PROOF_ACCEPTED: 15 theorems; 6 semantic and 2 policy controls")
