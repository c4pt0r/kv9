#!/usr/bin/env python3
"""Run isolated admission source controls with baseline/mutant/restored evidence."""
import argparse
import hashlib
import json
import os
...
CASES = [
    ('missing-count-bound', 'crates/server/src/admission.rs',
     'state.in_flight == self.limits.max_requests', 'false',
     'admission::tests::admission_checks_aggregate_bytes_count_and_oversized_requests',
     'request count limit was bypassed'),
    ('missing-byte-bound', 'crates/server/src/admission.rs',
     'bytes > self.limits.max_encoded_bytes - state.encoded_bytes', 'false',
     'admission::tests::admission_checks_aggregate_bytes_count_and_oversized_requests',
     'aggregate encoded byte limit was bypassed'),
    ('released-before-submission', 'crates/server/src/grpc.rs',
     ...
     """{ drop(reservation); tokio::task::spawn_blocking(move || operation(inner.as_ref())) }""",
     'grpc::tests::admission::admission_cancelled_running_and_queued_jobs_retain_their_reservations',
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='kv9-admission-controls.') as temp:
        tree = Path(temp)
    ...
    print('PASS: 3 isolated admission source controls checked')
