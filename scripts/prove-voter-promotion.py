#!/usr/bin/env python3
"""Check the voter-promotion model and explicit defect controls."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    contract = json.loads(
        (root / "proofs/lean/voter-promotion/source-contract.json").read_text()
    )
    assert contract["schema"] == 1
    for name, digest in contract["source_sha256"].items():
        assert hashlib.sha256((root / name).read_bytes()).hexdigest() == digest, name
    source = (root / "proofs/lean/voter-promotion/Promotion.lean").read_text()
    ...
    def compile_source(label, text, success):
        ...
        directory.mkdir()
        path = directory / "Promotion.lean"
        path.write_text(text)
        command = [args.lean, "-DwarningAsError=true", "-o", str(directory / "Promotion.olean"), str(path)]
    ...
    audit.write_text("import Promotion\n" + "\n".join(
        f"#print axioms Kv9.VoterPromotion.{name}" for name in contract["theorems"]) + "\n")
    ...
    for name in contract["theorems"]:
        line = next(line for line in result.stdout.splitlines()
                    if f"Kv9.VoterPromotion.{name}'" in line)
    ...
    controls = [
        ('promote-without-evidence',
         '(e : s.evidence = true) (l : s.learner = true)',
         '(e : s.evidence = s.evidence) (l : s.learner = true)'),
        ('promote-keeps-the-learner',
         '{s with voter := true, learner := false}',
         '{s with voter := true}'),
        ('promote-grants-serving',
         '{s with voter := true, learner := false}',
         '{s with voter := true, learner := false, serving := true}'),
        ('attach-onto-a-voter',
         '(m : s.migration = true) (fresh : s.voter = false)',
         '(m : s.migration = true) (fresh : s.voter = s.voter)'),
        ('attach-without-migration',
         '(m : s.migration = true) (fresh : s.voter = false)',
         '(m : s.migration = s.migration) (fresh : s.voter = false)'),
        ('evidence-without-attachment',
         '| record {s} (l : s.learner = true) :',
         '| record {s} (l : s.learner = s.learner) :'),
        ('commit-serves',
    ...
    report = {
        ...
        "verified_rust_extraction": False, "promotion_without_evidence_or_serving": False,
        "new_chaos_acceptance": False, "new_qps": False,
    }
    (output / "result.json").write_text(json.dumps(report, indent=2) + "\n")
    print(f"VOTER_PROMOTION_PROOF_ACCEPTED: {len(contract['theorems'])} theorems; {len(controls)} semantic and 2 policy controls")
