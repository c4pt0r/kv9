# Resident index outlined shared-mutation path experiment
...
The candidate combines the prior static `as_ptr` callback with extraction of
the shared clone branch from `triomphe::Arc::make_mut`. The unique branch retains
one original reference-count observation and the original raw-pointer guard.
Check the retained report for the actual gate decision before advancing.
