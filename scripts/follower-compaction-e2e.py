#!/usr/bin/env python3
"""Real-process FOLLOWER-side compaction gate (#20): the group-compaction
open edge closed. After the group leader's own all-matched truncation it
publishes the confirmed floor into the GROUP'S OWN raft log — a fenced
write of a reserved key that replicates to every voter. Each replica reads
that key from its applied state and compacts its own prefix under it. No
cross-node catalog write, no metadata-leader auth. This fixture proves ALL
THREE replicas' log_first_index advance, not just the leader's.

Placeholder header (auto-compaction doc follows):
...
def main():
    ...
    parser.add_argument('--base-port', type=int, default=27400)
    ...
    env = dict(os.environ, KV9_CLUSTER_TOKEN='followcompact-e2e-cluster',
               KV9_CLIENT_TOKENS='admin=followcompact-e2e-client', KV9_CLIENT_TOKEN='followcompact-e2e-client',
               KV9_BOOTSTRAP_TOKEN='followcompact-e2e-bootstrap', KV9_STORAGE='minio', KV9_AUTO_COMPACT_ENTRIES='64')
    ...
    try:
        ...
        # auto-proposes a floor; the group leader truncates AND publishes the
        # confirmed floor into the group's own raft log (reserved key); then
        # EVERY voter (followers too) reads it and compacts its own prefix
        # past index 1.
        burst('fill', 400)
        ...
        fi = wait('EVERY voter compacts its own log (follower-side)', all_voters_compacted, 300)
        print(f'PASS: all voters log_first_index {fi} (follower compaction confirmed)', flush=True)
        ...
        manifest_record.update(
            ...
            checks=['a committed all-matched confirmation authorizes follower-side compaction',
                    'EVERY voter (followers included) advances its own log_first_index past 1',
                    'a second burst advances every voter\'s floor further',
