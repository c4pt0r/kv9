#!/usr/bin/env python3
"""Real-process source-truncation gate: with the transfer settled, a
committed decision bounded by the evidence cut authorizes the source group
leader to compact its retained log, and the compacted replica restarts
only under that committed authority.
...
def main():
    ...
    parser.add_argument('--base-port', type=int, default=26920)
    ...
    env = dict(os.environ, KV9_CLUSTER_TOKEN='truncation-e2e-cluster',
               KV9_CLIENT_TOKENS='admin=truncation-e2e-client', KV9_CLIENT_TOKEN='truncation-e2e-client',
               KV9_BOOTSTRAP_TOKEN='truncation-e2e-bootstrap', KV9_STORAGE='minio')
    ...
    try:
        ...
        manifest_record.update(verdict='accepted', region=region,
                               checks=['settlement chain (evidence, quiesce, release) as previously qualified',
                                       'compaction refuses without the committed decision; floors beyond the evidence cut refuse',
                                       'the committed decision compacts the leader log to exactly floor+1, idempotently',
                                       'the compacted replica restarts only under the committed decision',
                                       'acknowledged data below the floor survives; the learner keeps tracking the tail'])
