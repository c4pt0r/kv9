...
P=R/'proofs/lean/key-clamp/KeyClamp.lean'
C=R/'proofs/lean/key-clamp/source-contract.json'
...
def check(source,name):
    ...
    queries='\n'.join('#print axioms Kv9.KeyClamp.'+n for n in names)
    (work/'KeyClamp.lean').write_text(source+'\n'+queries+'\n')
    with (work/'lean.log').open('x') as stdout:
        p=subprocess.run([str(lean),'-DwarningAsError=true','-o','KeyClamp.olean','KeyClamp.lean'],cwd=work,stdout=stdout,stderr=subprocess.STDOUT,timeout=60)
    log=(work/'lean.log').read_text()
    require(p.returncode==0,'Lean rejected proof')
    axioms={}
    for name in names:
        qualified='Kv9.KeyClamp.'+name
...
try:
    ...
    for label,old,new,reason in [
        ('wrong-key-boundary','entry.bytes.take (min entry.keyLength entry.bytes.length)','entry.bytes.take (min (entry.keyLength + 1) entry.bytes.length)','Lean rejected proof'),
        ('wrong-clamp-bound','min entry.keyLength entry.bytes.length','min entry.keyLength (entry.bytes.length / 2)','Lean rejected proof'),
        ...
        if label=='custom-axiom':changed=changed.replace('namespace Kv9.KeyClamp','namespace Kv9.KeyClamp\naxiom fake : False',1)
    ...
    source=(S/'candidate/crates/engine/src/entry_buffer.rs').read_bytes()
    for name,old,new in [
        ('wrong-source-clamp',b'self.key_len.min(self.bytes.len())',b'self.key_len.min(self.bytes.len() / 2)'),
        ...
        try:bind(changed,contract['sources']['evidence/candidate/crates/engine/src/entry_buffer.rs'])
