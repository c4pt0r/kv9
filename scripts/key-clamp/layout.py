...
assert sha(S/'candidate/crates/engine/src/entry_buffer.rs')==json.loads((S/'declared-plan.json').read_text())['candidate_key_sha256']
work=S/(label+'-source');work.mkdir(exist_ok=False)
out=S/label;out.mkdir(exist_ok=False)
for source,name in [(R/'scripts/entry-buffer/layout.rs','main.rs'),(S/'candidate/crates/engine/src/entry_buffer.rs','entry_buffer.rs'),
                    (R/'scripts/engine-interface-counting/src/counting.rs','counting.rs')]:
    shutil.copyfile(source,work/name)
(work/'Cargo.toml').write_text('''[package]
name = "kv9-key-clamp-layout"
version = "0.0.0"
edition = "2021"
publish = false
[workspace]
...
try:
    with cache.BuildCache(work,out,True,sources) as build:
        ...
        elf=out/'layout';shutil.copyfile(R/'target/release/kv9-key-clamp-layout',elf);elf.chmod(0o755)
