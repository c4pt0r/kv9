#!/usr/bin/env python3
"""Check Protocol snapshot persistence and startup fencing and explicit defect controls."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    contract = json.loads((root / "proofs/lean/protocol-snapshot/source-contract.json").read_text())
    assert contract["schema"] == 1
    for name, digest in contract["source_sha256"].items():
        assert hashlib.sha256((root / name).read_bytes()).hexdigest() == digest, name
    source = (root / "proofs/lean/protocol-snapshot/Snapshot.lean").read_text()
    ...
    def compile_source(label, text, success):
        ...
        directory.mkdir()
        path = directory / "Snapshot.lean"
        path.write_text(text)
        command = [args.lean, "-DwarningAsError=true", "-o", str(directory / "Snapshot.olean"), str(path)]
    ...
    audit.write_text("import Snapshot\n" + "\n".join(
        f"#print axioms Kv9.ProtocolSnapshot.{name}" for name in contract["theorems"]) + "\n")
    ...
    for name in contract["theorems"]:
        line = next(line for line in result.stdout.splitlines() if f"Kv9.ProtocolSnapshot.{name}'" in line)
    ...
    controls = [
        ('publish-before-sync', '(synced : s.phase = .synced) :\n      Step old next s {s with phase := .published', '(synced : s.phase = .writing) :\n      Step old next s {s with phase := .published'),
        ('sync-missing-disk', 'phase := .synced, disk := next', 'phase := .synced, disk := old'),
        ('partial-recovery', 'phase := .recovered, live := s.disk', 'phase := .recovered, live := {s.disk with payload := s.disk.payload + 1}'),
        ('acknowledge-failed-write', 'phase := .failed, disk := retained', 'phase := .failed, disk := retained, acknowledged := true'),
        ('lose-synced-record', 'phase := .down, live := old', 'phase := .down, live := old, disk := old'),
        ('restart-failed-writer', '(idle : s.phase = .idle)', '(idle : s.phase = .failed)'),
        ('accept-compacted-start', 'firstIndex == 1', 'firstIndex >= 1'),
        ('change-state-on-receive', 'def receiveSnapshot (s : Image) : Image := s', 'def receiveSnapshot (s : Image) : Image := {s with cut := s.cut + 1}'),
        ('relabel-snapshot-index', 'then some image else none', 'then some {image with cut := requested} else none'),
    ]
    ...
    report = {
        ...
        "verified_rust_extraction": False, "complete_snapshot_installation": False,
        "new_chaos_acceptance": False, "new_qps": False,
    }
    (output / "result.json").write_text(json.dumps(report, indent=2) + "\n")
    print(f"PROTOCOL_SNAPSHOT_PROOF_ACCEPTED: {len(contract['theorems'])} theorems; {len(controls)} semantic and 2 policy controls")
