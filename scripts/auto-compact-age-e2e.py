#!/usr/bin/env python3
"""Real-process AGE-BASED auto-compaction gate (#20): a LOW-TRAFFIC group
whose raft log never grows past a size threshold should still compact so its
recovery-replay does not accumulate ancient entries forever. With BOTH size
triggers OFF (KV9_AUTO_COMPACT_ENTRIES=0, KV9_AUTO_COMPACT_BYTES=0) and
KV9_AUTO_COMPACT_AGE_SECS set, a handful of small writes followed by IDLE
time cross the age threshold — the metadata leader auto-proposes a committed
compaction floor (the exact manual verb, no new authority); the existing
all-matched-gated reconcile executes it and every voter compacts its prefix.

Because both size triggers are OFF and the fixture writes only a dozen tiny
values, any compaction here is driven PURELY by age: the group NEVER
approaches an entry or byte threshold. It asserts every voter's
log_first_index advances past 1 only AFTER the idle age threshold elapses, a
second small burst plus idle advances every voter's floor further, and a
full-cluster restart recovers on the compacted logs of all three voters with
every sampled key serving.
...
def main():
    ...
    parser.add_argument('--base-port', type=int, default=27480)
    ...
    env = dict(os.environ, KV9_CLUSTER_TOKEN='agecompact-e2e-cluster',
               KV9_CLIENT_TOKENS='admin=agecompact-e2e-client', KV9_CLIENT_TOKEN='agecompact-e2e-client',
               KV9_BOOTSTRAP_TOKEN='agecompact-e2e-bootstrap', KV9_STORAGE='minio',
               KV9_AUTO_COMPACT_ENTRIES='0', KV9_AUTO_COMPACT_BYTES='0',
               KV9_AUTO_COMPACT_AGE_SECS='5')
    ...
    try:
        ...
        def burst(name, count):
            ...
            for i in range(count):
                ...
                # Tiny 2-byte values: a dozen of these never approach any entry
                # or byte threshold, so the ONLY thing that can trigger a
                # compaction is age (both size triggers are OFF).
                value = '%04x' % i
                landed = None
        ...
        # A dozen tiny writes with BOTH size triggers OFF and ZERO manual verbs.
        # Only the AGE trigger can cause compaction: the group never approaches
        # an entry or byte threshold. After the writes the group goes IDLE, and
        # once the oldest retained entry has lingered past the 5 s age threshold
        # the metadata leader auto-proposes a floor; the gated reconcile
        # executes it and every voter compacts its prefix.
        burst('fill', 12)
        retained_before = leader_retained_entries()
        require(retained_before is not None and retained_before < 64,
                f'the group must stay tiny (retained entries={retained_before})')
        print(f'PASS: group is low-traffic (retained entries={retained_before}, '
              f'far below any size threshold)', flush=True)

        def all_voters_compacted():
            fi = first_indices()
            return fi if len(fi) == 3 and all(v > 1 for v in fi.values()) else None
        fi = wait('EVERY voter compacts its log under the AGE trigger (size triggers off)',
                  all_voters_compacted, 300)
        print(f'PASS: all voters log_first_index {fi} (age-driven compaction)', flush=True)

        # A second tiny burst plus idle advances every voter's floor further.
        burst('fill2', 12)
        ...
        manifest_record.update(
            ...
            checks=['a low-traffic group stays far below every entry/byte threshold (both size triggers OFF)',
                    'after the age threshold elapses on the idle group, every voter compacts (log_first_index past 1) — age-driven',
                    'a second tiny burst plus idle advances every voter\'s floor further',
