#!/usr/bin/env python3
"""Check the source-truncation model and explicit defect controls."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    contract = json.loads(
        (root / "proofs/lean/source-truncation/source-contract.json").read_text()
    )
    assert contract["schema"] == 1
    for name, digest in contract["source_sha256"].items():
        assert hashlib.sha256((root / name).read_bytes()).hexdigest() == digest, name
    source = (root / "proofs/lean/source-truncation/Truncation.lean").read_text()
    ...
    def compile_source(label, text, success):
        ...
        directory.mkdir()
        path = directory / "Truncation.lean"
        path.write_text(text)
        command = [args.lean, "-DwarningAsError=true", "-o", str(directory / "Truncation.olean"), str(path)]
    ...
    audit.write_text("import Truncation\n" + "\n".join(
        f"#print axioms Kv9.SourceTruncation.{name}" for name in contract["theorems"]) + "\n")
    ...
    for name in contract["theorems"]:
        line = next(line for line in result.stdout.splitlines()
                    if f"Kv9.SourceTruncation.{name}'" in line)
    ...
    controls = [
        ('decide-without-evidence',
         '(e : s.evidence = true) (f : Nat)',
         '(e : s.evidence = s.evidence) (f : Nat)'),
        ('decide-beyond-the-evidence-cut',
         '(bounded : f \u2264 s.evidenceCut) (fresh : s.floor = 0)',
         '(bounded : f \u2264 f) (fresh : s.floor = 0)'),
        ('compact-beyond-the-floor',
         'Step s {s with retained := s.floor}',
         'Step s {s with retained := s.floor + 1}'),
        ('compact-regresses-below-the-floor',
         'Step s {s with retained := s.floor}',
         'Step s {s with retained := s.floor - 1}'),
        ('compact-without-peer-progress',
         '(progressed : s.floor \u2264 s.matched)',
         '(progressed : s.floor \u2264 s.floor)'),
        ('floor-mutates-after-decision',
         '(bounded : f \u2264 s.evidenceCut) (fresh : s.floor = 0)',
         '(bounded : f \u2264 s.evidenceCut) (fresh : s.floor = s.floor)'),
        ('evidence-rebinds-a-smaller-cut',
         '(c : Nat) (exact : 0 < c) (fresh : s.evidence = false)',
         '(c : Nat) (exact : 0 < c) (fresh : s.evidence = s.evidence)'),
        ('advance-regresses',
         'Step s {s with matched := s.matched + m}',
         'Step s {s with matched := m}'),
    ]
    ...
    report = {
        ...
        "verified_rust_extraction": False, "compaction_without_decision_or_peer_stranding": False,
        "new_chaos_acceptance": False, "new_qps": False,
    }
    (output / "result.json").write_text(json.dumps(report, indent=2) + "\n")
    print(f"SOURCE_TRUNCATION_PROOF_ACCEPTED: {len(contract['theorems'])} theorems; {len(controls)} semantic and 2 policy controls")
