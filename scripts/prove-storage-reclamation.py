#!/usr/bin/env python3
"""Check the storage-reclamation model and explicit defect controls."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    contract = json.loads(
        (root / "proofs/lean/storage-reclamation/source-contract.json").read_text()
    )
    assert contract["schema"] == 1
    for name, digest in contract["source_sha256"].items():
        assert hashlib.sha256((root / name).read_bytes()).hexdigest() == digest, name
    source = (root / "proofs/lean/storage-reclamation/StorageReclamation.lean").read_text()
    ...
    def compile_source(label, text, success):
        ...
        directory.mkdir()
        path = directory / "StorageReclamation.lean"
        path.write_text(text)
        command = [args.lean, "-DwarningAsError=true", "-o", str(directory / "StorageReclamation.olean"), str(path)]
    ...
    audit.write_text("import StorageReclamation\n" + "\n".join(
        f"#print axioms Kv9.StorageReclamation.{name}" for name in contract["theorems"]) + "\n")
    ...
    for name in contract["theorems"]:
        line = next(line for line in result.stdout.splitlines()
                    if f"Kv9.StorageReclamation.{name}'" in line)
    ...
    controls = [
        ('delete-without-the-durable-record',
         '| deletePayload {s} (rec : s.recordDurable = true) :',
         '| deletePayload {s} (rec : s.recordDurable = s.recordDurable) :'),
        ('record-without-the-retired-fence',
         '| commitRecord {s} (r : s.retired = true) (a : s.authorityVerified = true) :',
         '| commitRecord {s} (r : s.retired = s.retired) (a : s.authorityVerified = true) :'),
        ('record-without-committed-authority',
         '| commitRecord {s} (r : s.retired = true) (a : s.authorityVerified = true) :',
         '| commitRecord {s} (r : s.retired = true) (a : s.authorityVerified = s.authorityVerified) :'),
        ('deletion-loses-the-fence',
         '| deletePayload {s} (rec : s.recordDurable = true) :\n      Step s {s with payloadDeleted := true}',
         '| deletePayload {s} (rec : s.recordDurable = true) :\n      Step s {s with payloadDeleted := true, fenceLost := true}'),
        ('deletion-touches-a-leaf',
         '| deletePayload {s} (rec : s.recordDurable = true) :\n      Step s {s with payloadDeleted := true}',
         '| deletePayload {s} (rec : s.recordDurable = true) :\n      Step s {s with payloadDeleted := true, leafTouched := true}'),
    ]
    ...
    report = {
        ...
        "verified_rust_extraction": False, "unfenced_or_unauthorized_deletion": False,
        "new_chaos_acceptance": False, "new_qps": False,
    }
    (output / "result.json").write_text(json.dumps(report, indent=2) + "\n")
    print(f"STORAGE_RECLAMATION_PROOF_ACCEPTED: {len(contract['theorems'])} theorems; {len(controls)} semantic and 2 policy controls")
