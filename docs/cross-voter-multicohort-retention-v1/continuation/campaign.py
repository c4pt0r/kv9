...
OUT=Path('/tmp/kv9-cross-voter-multicohort-continuation-root-20260915-first')
PRIOR_OUT=Path('/tmp/kv9-cross-voter-multicohort-campaign-root-20260915-first')
PRIOR_PREP=Path('/tmp/kv9-cross-voter-multicohort-campaign-preparation-20260915-first')
REPAIR=Path('/tmp/kv9-retention-013-reconciliation-preparation-20260915-first')
REPAIR_PIN='289615ee63d27604b6265aa332d094cf2e96aac4b2c084e855ecb8da0df02e04'
READER=Path('/tmp/kv9-retention-pid-identity-repair-20260915-first')
READER_PIN='3269e44abe84555e70ad1f07e86ea32e057d828515888b23e6663c4e4b724386'
RECONCILED_ROOT=Path('/tmp/kv9-retention-013-reconciliation-root-20260915-first')
FAILURE_ROOT=Path('/tmp/kv9-cross-voter-multicohort-campaign-failure-013-20260915-first')
INDEPENDENT_ROOT=Path('/tmp/kv9-retention-013-independent-acceptance-20260915-first')
INDEPENDENT_SHA='4b957a568e9fb516cdc10312dee27abe5bd31efef22bfe6dc38579dd7fb7afd7'
HISTORICAL_STOP=100000000000
AUTHORITY=None
...
CPUS=set(range(6,16))|set(range(22,32));STOP=85000000000
...
def completed_original(group,i,verification_sha):
...
def resume_prefix(root):
 """Only actual completed continuation ordinals014..095; historical prefix is separately verified."""
 names={int(p.name)for p in root.iterdir()if re.fullmatch(r'[0-9]{3}',p.name)}
 need(not names-set(range(14,96)),'unexpected continuation ordinal directory')
 done=[]
 for i in range(14,96):
  d=root/f'{i:03d}'
  if absent(d):
   need(not any(j>i for j in names),'noncontiguous continuation state');return done,i
  need((d/'complete.json').is_file(),'incomplete/unknown continuation ordinal; no retry')
  r=js(d/'complete.json');need(r['complete']and r['ordinal']==i,'completed continuation binding');done.append(r)
 return done,96
...
def invoke(i,mode,release=None):
 ...
 if release is not None:argv=finish_argv(i,release)
...
def extra_accounting_roots():
 return [(OUT,CAP),(P,16*MIB),(PRIOR_OUT,CAP),(PRIOR_PREP,16*MIB),(FAILURE_ROOT,32*MIB),(RECONCILED_ROOT,32*MIB),(REPAIR,32*MIB),(READER,16*MIB),(INDEPENDENT_ROOT,4*MIB)]

def extra_allocation():
 rows=[];seen=set();device=EXEC.stat().st_dev
 for root,cap in extra_accounting_roots():
  canonical(root);need(root.is_dir()and root.stat().st_dev==device,'missing/wrong-device accounting root')
  need(str(root)not in seen,'duplicate accounting root');seen.add(str(root));n=allocated(root);need(n<=cap,'bounded extra metadata root exceeded');rows.append(dict(path=str(root),allocated_bytes=n,cap_bytes=cap))
 return rows

def accounting(original,group):
 a=original.account(group);rows=extra_allocation();extra=sum(r['allocated_bytes']for r in rows)
 a['continuation_extra_allocations']=rows;a['conservative_net_allocated_change_bytes']-=extra
 a['historical_plan_target_reached']=a['target_reached'];a['historical_plan_target_bytes']=HISTORICAL_STOP
 a['target_reached']=a['actual_available_bytes']>=STOP;a['continuation_capacity_stop_bytes']=STOP
 a['scope']+=' Also charges old failed controller/preparation, failure preservation, actual013 root, corrected reader/finish preparations, and this new preparation/controller; original plan target remains historical.'
 a['benchmark_runtime_ready']=False;return a

def repair_modules():
 global AUTHORITY
 need(pin(REPAIR/'inventory.json')['sha256']==REPAIR_PIN,'corrected finish inventory changed')
 for r in js(REPAIR/'inventory.json')['rows']:check(REPAIR/r['path'],r)
 need(pin(READER/'reader-bindings.json')['sha256']==READER_PIN,'corrected reader manifest changed')
 spec=importlib.util.spec_from_file_location('continuation_repair_authority',REPAIR/'authority.py');AUTHORITY=importlib.util.module_from_spec(spec);spec.loader.exec_module(AUTHORITY)

def finish_argv(i,release):
 need(14<=i<=95,'only continuation finish ordinals')
 return [sys.executable,str(REPAIR/'reconcile.py'),'--code-pins-sha256',CODE,'--mode','finish','--ordinal',str(i),'--release-verified-sha256',release,'--preparation-inventory-sha256',REPAIR_PIN,'--reader-manifest',str(READER/'reader-bindings.json'),'--reader-manifest-sha256',READER_PIN]

def completed(group,i,verification_sha):
 if i<=12:return completed_original(group,i,verification_sha)
 row,s=selected(group,i);root=Path(s['root']);sp=row['selection']['sha256'];binding=dict(selection_sha256=sp,verified_sha256=verification_sha)
 restore=phase_receipt(i,'restore-1',root/'restore-1/result.json',dict(binding,state='RESTORED'))
 rsha=pin(root/'restore-1/result.json')['sha256'];rp=root/'readback-1-repaired-first/result.json'
 rbinding=AUTHORITY.reader_binding(s)
 reader=phase_receipt(i,'readback-1-repaired',rp,dict(selection_sha256=sp,restore_result_sha256=rsha,preparation_inventory_sha256=REPAIR_PIN,**rbinding))
 a=argparse.Namespace(selection_sha256=sp,restore_result_sha256=rsha,preparation_inventory_sha256=REPAIR_PIN,readback_result_sha256=pin(rp)['sha256'])
 AUTHORITY.repaired_reader_ok(reader,s,a)
 reconciliation=AUTHORITY.reconciliation_binding(s,a)
 cold=phase_receipt(i,'retire-2',root/'retire-2/result.json',dict(binding,state='COLD',cycle=2,reconciliation=reconciliation))
 targets={p['target']for p in s['pairs']};exact_rows(cold['rows'],{p['id']for p in s['pairs']});restored=exact_rows(restore['rows'],targets);need(all(x['original_bytes_exact']for x in restored.values()),'restore byte evidence')
 stage=js(root/'staged.json');verified=js(root/'verification-first/result.json');need(pin(root/'verification-first/result.json')['sha256']==verification_sha and pin(root/'staged.json')['sha256']==verified['staged_sha256'],'completed stage/verify changed');pair_proofs(s,stage,verified)
 for r in stage['rows']:need(identity(r['patch']['path'])==r['patch']['identity'],'completed patch identity changed')
 for r in s['metadata']:check(r['path'],r)
 for m in s['members']:
  if m['id']in targets:need(absent(Path(m['path'])),'claimed corrected COLD target resident')
  else:need(identity(m['path'])==m['identity'],'preserved object identity changed')
 need(restore['all_children_reaped']and len(restore['children'])==3*s['target_count'],'complete restoration codec count')
 children=[c for r in stage['rows']for c in r['children']]+verified['children']+restore['children'];need(len(children)==9*s['target_count'],'complete migration child count')
 sidecar_closure(children,root.rglob('*.child.json'),root.rglob('*.child-intent.json'),root.rglob('*.child-start.json'))
 for r in children:need(js(Path(r['output']+'.child.json'))==r,'completed sidecar/embedded receipt differs');outcome(r)
 return dict(ordinal=i,selection_sha256=sp,verified_sha256=verification_sha,restore_sha256=rsha,readback_sha256=pin(rp)['sha256'],readback_path=str(rp),cold_sha256=pin(root/'retire-2/result.json')['sha256'],reader_binding=rbinding,state='COLD',complete=True)

def reconciled_terminal_check(tool,child,expected_argv,result_path,result_sha):
 need(tool['schema']==1 and tool['session_id']==56762 and tool['launch']['session_id']==56762 and tool['launch']['chunk_id']=='44b5a8'and tool['terminal']['exit_code']==0 and tool['terminal']['chunk_id']=='364874', 'actual013 successful tool terminal required')
 need(json.loads(tool['terminal']['output'])==child,'actual013 tool output/child receipt mismatch')
 need(child['complete']is True and child['exit_code']==0 and child['ended_ns']>=child['started_ns'],'actual013 child unsuccessful/incomplete')
 need(child['argv']==expected_argv and child['result_path']==str(result_path)and child['result_sha256']==result_sha,'actual013 command/result binding')

def independent013_check(summary,selection_sha,reader_binding,tool_pin,child_pin,result_sha):
 need(summary['complete']is True and summary['ordinal']==13 and summary['state']=='COLD'and summary['selection_sha256']==selection_sha,'independent013 accepted state/selection')
 need(summary['targets']==209 and summary['objects']==344 and summary['logical_bytes']==10402566933 and summary['actual_fresh_codec_lifetimes']==2569 and summary['historical_producer_lifetimes']==688,'independent013 full scope/counts')
 need(summary['all_original_metadata_unchanged']is True and summary['original_failed_readback_preserved']is True,'independent013 original failure/metadata preservation')
 need(summary['charged_decoded_bytes']==41558085954 and summary['decoded_cap_bytes']==69793218560,'independent013 full failed-plus-corrected decoded charge')
 need(len(summary['preserved_objects'])==135 and len(summary['patches'])==209 and len(summary['lifetimes'])==3266 and all(r['same_lifetime_present']is False for r in summary['lifetimes']),'independent013 preservation/lifetime count')
 need(summary['corrected_reader_binding']==reader_binding and summary['actual_tool_terminal']==tool_pin and summary['actual_child_terminal']==child_pin and summary['phase_hashes']['retire-2/result.json']==result_sha,'independent013 exact reader/terminal/COLD authority')

def historical_prefix(group,a):
 inputs=js(P/'predecessor-inputs.json')
 for r in inputs['files']:check(r['path'],r)
 need(pin(PRIOR_PREP/'inventory.json')['sha256']=='a8b83212496c6ece97376cb14b04c17a3ea899d3c8d7ae3717cd290add1c5c40','historical controller source authority')
 status=js(PRIOR_OUT/'status.json');need(status['state']=='FAILED','original controller failure was overwritten')
 failed_tool=js(FAILURE_ROOT/'tool-terminal.json');failed_child=js(PRIOR_OUT/'launch-first/campaign-child-terminal.json')
 need(failed_tool['session_id']==79586 and failed_tool['chunk_id']=='ce8c44'and failed_tool['exit_code']==1 and json.loads(failed_tool['output'])==failed_child,'actual original failed controller terminal')
 need(failed_child['complete']is False and failed_child['exit_code']==1 and failed_child['result_sha256']==pin(PRIOR_OUT/'status.json')['sha256'],'original failure binding')
 need({int(p.name)for p in PRIOR_OUT.iterdir()if re.fullmatch('[0-9]{3}',p.name)}==set(range(1,14))and absent(PRIOR_OUT/'013/complete.json'),'original completed/failed prefix changed')
 done=[]
 for i in range(1,13):
  r=js(PRIOR_OUT/f'{i:03d}'/'complete.json');need(r['complete']and r['ordinal']==i and completed_original(group,i,r['verified_sha256'])==r['acceptance'],'historical001..012 acceptance changed')
  check(PRIOR_OUT/f'{i:03d}'/'release.json',r['release_pin']);done.append(r)
 tool_path=RECONCILED_ROOT/'tool-terminal.json';child_path=RECONCILED_ROOT/'launch-first/ordinal013_finish-child-terminal.json'
 need(a.reconciled_tool_terminal==str(tool_path)and a.reconciled_child_terminal==str(child_path),'exact013 receipt paths required')
 need(pin(tool_path)['sha256']==a.reconciled_tool_terminal_sha256 and pin(child_path)['sha256']==a.reconciled_child_terminal_sha256,'actual013 receipt pins')
 cmd=js(RECONCILED_ROOT/'commands.json')['ordinal013_finish'];result=Path(group['rows'][13]['root'])/'retire-2/result.json'
 need(pin(result)['sha256']==a.reconciled_result_sha256,'actual013 final COLD pin')
 reconciled_terminal_check(js(tool_path),js(child_path),cmd['argv'],result,a.reconciled_result_sha256)
 verification=js(PRIOR_OUT/'013/release.json')['verified_sha256'];need(verification=='6b17d3766c740a223627b69d2161d86cc7edb7672864370a2a32616d1d686099','original013 independently released verification')
 acceptance=completed(group,13,verification)
 need(acceptance['cold_sha256']==a.reconciled_result_sha256,'actual013 acceptance result')
 summary_path=INDEPENDENT_ROOT/'accepted-summary.json';need(pin(summary_path)['sha256']==INDEPENDENT_SHA,'independent013 accepted summary changed');summary=js(summary_path)
 at=js(INDEPENDENT_ROOT/'tool-terminal.json');need(at['complete']is True and at['session_id']==39909 and at['actual_terminal_receipt']=='f0df7e'and at['exit_code']==0 and at['result_path']==str(summary_path)and at['result_sha256']==INDEPENDENT_SHA,'independent013 actual audit terminal')
 independent013_check(summary,group['rows'][13]['selection']['sha256'],acceptance['reader_binding'],pin(tool_path),pin(child_path),a.reconciled_result_sha256)
 for name,sha in summary['phase_hashes'].items():need(pin(Path(group['rows'][13]['root'])/name)['sha256']==sha,'independent013 phase/failure hash changed')
 done.append(dict(complete=True,ordinal=13,verified_sha256=verification,acceptance=acceptance,origin='separately accepted corrected reconciliation',tool_terminal=dict(path=str(tool_path),sha256=a.reconciled_tool_terminal_sha256),child_terminal=dict(path=str(child_path),sha256=a.reconciled_child_terminal_sha256),independent_acceptance=dict(path=str(summary_path),sha256=INDEPENDENT_SHA)))
 return done

def main():
 ap=argparse.ArgumentParser();ap.add_argument('--mode',choices=['run','status'],required=True);ap.add_argument('--inventory-sha256');ap.add_argument('--cohort-zero-terminal');ap.add_argument('--cohort-zero-terminal-sha256');ap.add_argument('--cohort-zero-child-terminal');ap.add_argument('--cohort-zero-child-terminal-sha256')
 ap.add_argument('--reconciled-tool-terminal');ap.add_argument('--reconciled-tool-terminal-sha256');ap.add_argument('--reconciled-child-terminal');ap.add_argument('--reconciled-child-terminal-sha256');ap.add_argument('--reconciled-result-sha256');a=ap.parse_args()
 if a.mode=='status':print(json.dumps(js(OUT/'status.json')));return 0
 need(os.geteuid()==0 and set(os.sched_getaffinity(0))==CPUS,'root and exact helper CPUs required');need(a.inventory_sha256 and pin(P/'inventory.json')['sha256']==a.inventory_sha256,'continuation inventory pin')
 for r in js(P/'inventory.json')['rows']:check(P/r['path'],r)
 original=original_modules();repair_modules();group=js(OLD/'group-plan.json');need(len(group['rows'])==96 and group['actual_available_stop_target_bytes']==HISTORICAL_STOP,'original historical plan/target changed');zero=zero_audit_binding(group,a)
 mkdir(OUT);fd=os.open(OUT/'campaign.lock',os.O_CREAT|os.O_RDWR|os.O_NOFOLLOW,0o600);fcntl.flock(fd,fcntl.LOCK_EX|fcntl.LOCK_NB)
 priorfd=os.open(PRIOR_OUT/'campaign.lock',os.O_RDWR|os.O_NOFOLLOW);fcntl.flock(priorfd,fcntl.LOCK_EX|fcntl.LOCK_NB);started=time.monotonic()
 def interrupted(signum,frame):raise InterruptedError('continuation signal '+str(signum))
 signal.signal(signal.SIGTERM,interrupted);signal.signal(signal.SIGINT,interrupted)
 try:
  with dispatcher_read_lock():history=historical_prefix(group,a)
  binding=dict(inventory_sha256=a.inventory_sha256,code_pins_sha256=CODE,group_sha256=GROUP,historical_target_bytes=HISTORICAL_STOP,continuation_target_bytes=STOP,cohort_zero=zero,historical_prefix=history)
  if absent(OUT/'binding.json'):save(OUT/'binding.json',binding)
  else:need(js(OUT/'binding.json')==binding,'continuation resume binding changed')
  new_done,next_i=resume_prefix(OUT)
  for r in new_done:need(completed(group,r['ordinal'],r['verified_sha256'])==r['acceptance'],'completed continuation changed/restored');check(OUT/f"{r['ordinal']:03d}"/'release.json',r['release_pin'])
  done=history+new_done
  for i in range(next_i,96):
   resource_guard(started,CAMPAIGN_SECONDS)
   with dispatcher_read_lock():global_prefix(group,len(done));status=accounting(original,group)
   ...
   need(available()>=s['limits']['launch_available_bytes']+256*MIB+CAP,'fresh transaction plus both output allowances')
