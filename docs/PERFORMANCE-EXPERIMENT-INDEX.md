...
| --- | --- |
| WAL payload preallocation | [Default-off candidate, proofs and exact encoder results](WAL-PAYLOAD-PREALLOCATION.md): reuse validated size with unchanged emitter/framing/sync. Eleven capacity theorems/seven controls and 47 CRC theorems/eight controls pass; both default/feature workspace configurations pass 851 tests and Clippy. Original-corpus encoder mean −27.339%; six small cases improve mean/p99. No database QPS, Redis or new actual Chaos result; matching releases and runtime qualification are next. |
