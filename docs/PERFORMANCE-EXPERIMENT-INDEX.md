...
p99 +3.309%. The [first packed-node screen](PACKED-INDEX-EXPERIMENT.md) is also
held: overwrite +26.957% / +12.403%, unique insert -8.679% / -22.518%. Correctness
and independent analysis pass. Next diagnose comparator/minimum-refresh work
before a changed kernel, retaining the detected fixed-stride hit-sampling bias.
...
| --- | --- |
| Packed persistent ordered index, 32-slot nodes | [Standalone implementation, six semantic tests, fixed timing/count screen and sampling diagnosis](PACKED-INDEX-EXPERIMENT.md): overwrite mean +26.957% / +12.403% without/with snapshots; unique insert -8.679% / -22.518%. Point means regress; fixed-stride small-map hits favor shallow rpds nodes (8.027 comparisons vs 11.003 across all keys). All 56 timing/count rows per executable pass state/accounting checks. Reject integration; no full formal proof, production change or database QPS. Diagnose comparator/shared-prefix work before a changed kernel; do not repeat unchanged. |
