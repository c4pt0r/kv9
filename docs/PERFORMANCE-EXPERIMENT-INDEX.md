...
| --- | --- |
| Published WAL directory capability `483b8c3` | Experimental on CRC main. [Conditional proof, 793 tests and syscall fault checks](WRITE-PUBLISHED-DIRECTORY.md), [default release and ordinary recovery](WRITE-PUBLISHED-DIRECTORY-RECOVERY.md), and the [actual 21-window Chaos baseline](WRITE-PUBLISHED-DIRECTORY-CHAOS.md) pass. The latter retains 9,372 operations and 33 exited server lifetimes. The [first rotation supplement](WRITE-PUBLISHED-DIRECTORY-ROTATION.md) independently proves selected rotation on three voters, then fails before injection because the namespace lacks Chaos opt-in; original failure and exact cleanup are preserved. Corrected crash/recovery coverage and matched throughput/p99 remain pending. Initial creation/recovery retain full ancestor sync; rotation reuses the live validated parent directory. |
