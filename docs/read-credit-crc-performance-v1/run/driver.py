#!/usr/bin/env python3
"""Matched v3 point and native batch64 read/write/mixed diagnostic.
...
NEW_REVISION = "de37c71009e8199859b931e0037818f490e8d3f6"
...
SERVER_PINS = {
    ...
    "new": dict(revision=NEW_REVISION,
                binary_sha256="5464adee210f7cebbce7cd57d6e01bfe5a1ace42936edad24f57e63ff9cfbdd7",
                manifest_sha256="164b7999386b042ad2df814fea44fe93724ae15396e9f0cae0356a926fc3d1de"),
}
...
def make_plan(smoke):
    ...
    require(len(rows) == (36 if smoke else 72), "matched v3 c1/c64 cohort inventory differs")
    return rows
...
def main():
    ...
    parser.add_argument("--new-server-source", type=Path, default=Path("/tmp/kv9-read-credit-crc"))
    parser.add_argument("--new-server-build", type=Path, default=Path("/tmp/kv9-read-credit-crc-release-rebuilt-first"))
    ...
    plan = dict(version=2, protocol_id=PROTOCOL_ID, workload_cells=WORKLOAD_CELLS, storage_guards=SPACE_GUARDS,
                ...
                scope="36-cohort v3 c1/c64 correctness smoke only; no timing acceptance" if args.smoke else
                      "72-cohort shared-host v3 point/batch64 read/write/mixed diagnostic; no exclusive host or equivalent durability",
