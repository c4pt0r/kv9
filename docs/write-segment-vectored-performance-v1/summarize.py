...
PREP=Path('/tmp/kv9-write-segment-vectored-ab-preparation-first')
RUN=Path('/tmp/kv9-write-segment-vectored-ab-timing-first/cohorts')
...
def main():
 ...
 try:
  ...
  assert aud['protocol_id']==protocol['protocol_id']=='kv9-write-segment-vectored-ab-c1-c64-v1'and aud['successful_arms']==aud['expected_successful_arms']==16 and len(aud['cases'])==16
  ...
  matrix=bound(RUN/'matrix.json');smoke=bound(Path('/tmp/kv9-write-segment-vectored-ab-smoke-first/matrix.json'))
