...
PREP = Path('/tmp/kv9-receipt-tail-chaos-publication-preparation-20260915-first/selection-first')
OUT = Path('/tmp/kv9-receipt-tail-chaos-evidence-20260915-first/docs/receipt-tail-chaos-v1')
G = 1024**3
...
assert __debug__
B = read(ROOT/'publication-bindings.json')
assert B['source_revision'] == 'a6ac335ef4567f2e1a2b33da1a723d694cd5b7d9'
...
runtime = Path('/tmp/kv9-receipt-tail-chaos-root-20260915-first')
assert B['post_root'] == '/tmp/kv9-receipt-tail-chaos-root-20260915-first'
...
class CheckedReader:
    ...
    def read(self, size=-1):
        ...
        self.count += len(data)
        return data
result = dict(complete=False, started_ns=started, initial_available_bytes=initial, maximum_added_bytes=G, reporting_policy='kv9-receipt-tail-portable-v3', minimum_available_bytes=8*G, launch_metadata_reserve_bytes=8*1024**2, original_benchmark_guards_unchanged=True)
try:
    sink = Parts()
