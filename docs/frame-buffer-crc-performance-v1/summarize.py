...
PREP=Path('/tmp/kv9-frame-crc-performance-preparation-20260914-first')
RUN=Path('/tmp/kv9-frame-crc-performance-timing-20260914-first/cohorts')
...
def main():
 ...
 try:
  ...
  assert aud['protocol_id']==protocol['protocol_id']=='kv9-frame-crc-write-ab-c1-c64-v1'and aud['successful_arms']==aud['expected_successful_arms']==16 and len(aud['cases'])==16
  ...
  matrix=bound(RUN/'matrix.json');smoke=bound(Path('/tmp/kv9-frame-crc-performance-smoke-20260914-first/matrix.json'))
