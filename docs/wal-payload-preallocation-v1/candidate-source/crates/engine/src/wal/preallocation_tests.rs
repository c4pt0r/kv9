...
#[test]
#[ignore = "local small-batch encoder CPU experiment; requires recorded allocator configuration"]
fn small_batch_encoder_microbenchmark() {
    use std::hint::black_box;
    use std::time::Instant;

    for count in [1, 64] {
        for value_len in [0, 256, 4096] {
            let mut batch = WriteBatch::new();
            for index in 0..count {
                let mut key = vec![b'k'; 32];
                key[31] = index as u8;
                batch.put(ColumnFamily::Default, key, vec![0x9d; value_len]);
            }
            let expected = encode_batch(&batch);
            assert_eq!(expected.len(), payload_size(&batch));
            assert_eq!(encode_batch_with_capacity(&batch, expected.len()), expected);
            for reserved in [false, true, true, false] {
                for _ in 0..1024 {
                    black_box(encode_batch(&batch));
                }
                ...
                println!(
                    "WAL_SMALL_MICRO={}",
                    serde_json::json!({
                        "mutations": count, "value_bytes": value_len,
                        "payload_bytes": expected.len(), "reserved": reserved,
                        "batches": timings.len(), "total_ns": total_ns,
                        "mean_ns": total_ns as f64 / timings.len() as f64,
                        "p99_ns": timings[(timings.len() * 99).div_ceil(100) - 1],
                        "scope": "hot fixed batch validation, encoding and buffer drop only; timer overhead included; no I/O or database throughput"
                    })
                );
            }
        }
