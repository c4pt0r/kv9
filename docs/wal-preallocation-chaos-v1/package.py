...
OUT = Path(__file__).resolve().parent / 'packet-second'
        ...
        with tarfile.open(full['archive'], 'r:gz') as source:
            for member in source:
                ...
                if name == 'inventory.json':
                    ...
                    assert hashlib.sha256(data).hexdigest() == full['inventory_sha256']
                    add(name, data, 'original full archive embedded inventory')
                    continue
                reason = None
