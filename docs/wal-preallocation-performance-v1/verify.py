...
def main():
    ...
    try:
        ...
        manifest=json.loads(blob);assert manifest['member_cap_bytes']==8*1024**2 and manifest['total_cap_bytes']==96*1024**2
        assert manifest['compressed_cap_bytes']==32*1024**2 and manifest['decoded_tar_cap_bytes']==96*1024**2
        expected={r['member']:r for r in manifest['members']};assert len(expected)==manifest['original_files']<512
        assert sum(r['bytes']for r in expected.values())==manifest['original_bytes']<=96*1024**2
        ...
        with gzip.GzipFile(fileobj=io.BytesIO(encoded),mode='rb')as gz:
            decoded=gz.read(96*1024**2+1);assert len(decoded)<=96*1024**2 and gz.read(1)==b''
        seen=set();total=0
