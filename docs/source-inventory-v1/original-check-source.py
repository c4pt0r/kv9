#!/usr/bin/env python3
"""Root owns the shared Cargo target for the bounded owner poll source checks."""
...
OUT = Path('/tmp/kv9-owner-poll-source-first')
...
def main():
    ...
    prepared = json.loads(Path('/tmp/kv9-owner-poll-root-first/commands-source.json').read_text())
        ...
        with builder.cache.BuildCache(SOURCE, OUT, True, before) as cache:
            for name, command, artifact_check in commands:
                ...
                if builder.snapshot() != before:
                    raise RuntimeError('Owner poll source changed during checks')
                save(OUT / 'result.json', result)
