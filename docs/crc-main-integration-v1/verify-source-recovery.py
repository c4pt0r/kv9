...
def main():
    ...
    for number, part in enumerate(parts, 1):
        require(part['name'] == f'source-recovery.tar.gz.{number:03d}', 'part sequence')
        ...
        compressed += len(data)
        require(compressed <= COMPRESSED, 'compressed cap')
        digest.update(data)
    require(compressed == index['compressed_bytes'] and digest.hexdigest() == index['compressed_sha256'], 'whole compressed identity')
    require({p.name for p in root.glob('source-recovery.tar.gz.*')} == {p['name'] for p in parts}, 'unexpected archive part')
    ...
    print(json.dumps(dict(complete=True, files=len(seen), decoded_bytes=total, compressed_bytes=compressed,
                         parts=len(parts), scope='Exact original artifact bytes only; no re-execution of source/proof/runtime acceptance.')))
