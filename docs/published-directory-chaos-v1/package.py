...
PREP = Path('/tmp/kv9-published-directory-chaos-publication-20260915-first/selection-second')
OUT = Path('/tmp/kv9-published-directory-chaos-publication-20260915-first/package-first')
G = 1024**3
...
assert B['source_revision'] == '483b8c3629b033734f5d7a2b8653a1352304a4b5'
...
runtime = Path('/tmp/kv9-published-directory-chaos-root-20260914-first')
assert B['post_root'] == '/tmp/kv9-published-directory-chaos-root-20260914-first'
...
def guard():
    ...
    assert free >= max(64*G, initial-G)
...
class CheckedReader:
    ...
    def read(self, size=-1):
        ...
        self.count += len(data)
        return data
result = dict(complete=False, started_ns=started, initial_available_bytes=initial, maximum_added_bytes=G, reporting_policy='kv9-published-directory-portable-v2', minimum_available_bytes=64*G, original_benchmark_guards_unchanged=True)
try:
    sink = Parts()
