...
PREP = Path('/tmp/kv9-write-crc-slicing8-chaos-publication-preparation-first')
OUT = Path('/tmp/kv9-write-crc-chaos-publication-first/docs/write-crc-chaos-v1')
G = 1024**3
...
assert __debug__
assert sha(PREP/'inventory.json') == 'f8c484a61fd04bfb72cb231b4975e715ca5f22784bf3ae56388958df585d1533'
assert sha(PREP/'members.json') == '3f7056c668a8ced29e654e22f695116b4902908d55ee8e52e9ab2bfe070c1b4c'
selected = list(read(PREP/'members.json')['members'].values())
assert len(selected) == 2282 and sum(x['bytes'] for x in selected) == 440155307
...
runtime = Path('/tmp/kv9-write-crc-slicing8-chaos-root-first')
for name in ('terminal.json', 'post-terminal.json'):
    t = read(runtime/name)
    assert t['complete'] and t['exit_code'] == 0 and sha(t['result_path']) == t['result_sha256']
post = read(runtime/'post-result.json')
