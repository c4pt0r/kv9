...
root=Path('/tmp/kv9-receipt-tail-hint-20260914-first')
out=Path('/tmp/kv9-receipt-tail-hint-source-20260914-first')
...
def main():
 ...
 spec=importlib.util.spec_from_file_location('receipt_builder',root/'scripts/build-workload.py')
 ...
 commands=[
  ...
  ('raft-driver-tests',['cargo','test','--locked','-p','kv9-raft','driver::','--','--test-threads=4'],False),
  ...
  with builder.cache.BuildCache(root,out,False,before) as cache:
   ...
   for name,argv,check in commands:
    ...
    try:
     ...
     if name in ('raft-driver-tests','workspace-tests'):
