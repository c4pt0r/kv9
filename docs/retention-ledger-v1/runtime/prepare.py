...
for name,receipt in [('raw','268089'),('ledger','6d7cff')]:
 receipt_value = {'complete':True,'execution_kind':'direct_tool_terminal','actual_terminal_receipt':receipt,'exit_code':0,'result':pin(AUDIT/(name+'-result.json')),'source':pin(AUDIT/('audit-'+name+'.py')),'recording_note':'Recorded after observed original tool completion; no audit was replayed to produce this receipt.'}
 assert json.loads((OUT/('receipts/'+name+'-audit-tool-terminal.json')).read_text())==receipt_value
rows={};excluded=[]
...
for name in ('restoration.json','download.log','kubeconfig-export.stderr','nodes.json','nodes.stderr'):
...
for name in ('prepare.py','prepare-first-failed.py','selection-first-failure.json','selection-correction.diff','pack.py','verify.py','README.md'):
