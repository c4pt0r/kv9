...
The [fresh current-source lifecycle diagnostic](READ-LIFECYCLE-CRC-CREDIT.md) now identifies extra mixed-read queueing under the one-context cap. The next candidate allows two local contexts to overlap while preserving fresh quorum authorization, sealed admission, cancellation bounds and apply-before-read. Its source gates pass, but process/fault and performance acceptance remain separate. Compare c1 and mixed read tails alongside loaded GET throughput; the diagnostic does not replace the uninstrumented results above.
