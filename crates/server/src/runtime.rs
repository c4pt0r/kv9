...
/// every chunk, so chunk N was fenced with chunk 1's authorisation. That is the stale
/// authorisation the per-chunk revalidation exists to remove, it compiles, and it passes
/// both branches' tests — the failure is invisible at every level except the type. Moving
/// the mint out of the loop now fails to compile with "use of moved value in previous
/// iteration", which is the guard `tests::moving_validation_out_of_the_loop_must_not_compile`
/// pins.
///
...
impl ValidatedFence {
    ...
    fn to_region_fence(&self) -> kv9_raft::RegionFence {
...
impl RuntimeBackend {
    ...
    fn commit_batch(
        ...
        let command = Command::fenced_write_from_batch(fence.to_region_fence(), &batch);
...
mod fence_firing_tests {
    ...
    /// **Every chunk of a multi-chunk delete range carries its own authorisation.**
    ///
    /// This is the endpoint evidence the layer-1 ↔ layer-3 merge needs, and the reason it
    /// has to exceed `RAW_DELETE_RANGE_CHUNK`: at or below the chunk size the loop runs
    /// once, one fence is recorded, and a version that validated once before the loop and
    /// reused that authorisation for every chunk is indistinguishable from this one. The
    /// hazard only becomes observable at chunk 2.
    ///
    /// `ValidatedFence` makes the bad merge fail to compile (see
    /// `moving_validation_out_of_the_loop_must_not_compile`); this makes the good one
    /// *observable*, so a future refactor that reintroduces the shape some other way still
    /// reds here.
    #[test]
    fn every_chunk_of_a_multi_chunk_delete_range_carries_its_own_authorisation() {
