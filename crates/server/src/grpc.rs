...
impl proto::kv9_server::Kv9 for Kv9Grpc {
    ...
    async fn attach_migration_learner(
        &self,
        request: Request<proto::AttachMigrationLearnerRequest>,
    ) -> Result<Response<proto::AttachMigrationLearnerResponse>, Status> {
        let auth = auth_context(&request)?;
        let reservation = self.reserve(&request, WorkClass::MetadataWrite)?;
        let request = request.into_inner();
        let root = kv9_common::RootDigest::from_bytes(
        ...
        let result = self
            .backend
            .call(reservation, move |backend| {
                backend.attach_migration_learner(&caller, root, operation)
            })
            .await?;
        Ok(Response::new(proto::AttachMigrationLearnerResponse {
    ...
    async fn record_source_removal(
        &self,
        request: Request<proto::RecordSourceRemovalRequest>,
    ) -> Result<Response<proto::RecordSourceRemovalResponse>, Status> {
        let auth = auth_context(&request)?;
        let reservation = self.reserve(&request, WorkClass::MetadataWrite)?;
        let request = request.into_inner();
        let root = kv9_common::RootDigest::from_bytes(
        ...
        let operation: [u8; 16] = request
            ...
            .try_into()
            .map_err(|_| Status::invalid_argument("operation_id must contain 16 bytes"))?;
        let incarnation: [u8; 16] = request
            .source_incarnation
            .try_into()
            .map_err(|_| Status::invalid_argument("source_incarnation must contain 16 bytes"))?;
        if operation == [0; 16]
            || root.as_bytes() == &[0; 32]
        ...
        {
            return Err(Status::invalid_argument(
                "nonzero root, operation and exact source replica required",
            ));
        }
        let source = kv9_meta::data_groups::InitialReplica {
            node: kv9_common::NodeId(request.source_node),
            incarnation: kv9_common::StoreIncarnation::from_bytes(incarnation),
        };
        let caller = auth.principal.to_string();
        let result = self
            .backend
            .call(reservation, move |backend| {
                backend.record_source_removal(&caller, root, operation, source)
            })
            .await?;
        Ok(Response::new(proto::RecordSourceRemovalResponse {
            task: result.decision.task(),
            changed: result.changed,
        }))
    }

    async fn remove_source_replica(
        &self,
        request: Request<proto::RemoveSourceReplicaRequest>,
    ) -> Result<Response<proto::RemoveSourceReplicaResponse>, Status> {
        let auth = auth_context(&request)?;
        let reservation = self.reserve(&request, WorkClass::MetadataWrite)?;
        let request = request.into_inner();
        let root = kv9_common::RootDigest::from_bytes(
        ...
        let result = self
            .backend
            .call(reservation, move |backend| {
                backend.remove_source_replica(&caller, root, operation)
            })
            .await?;
        Ok(Response::new(proto::RemoveSourceReplicaResponse {
            removed_node: result.removed.0,
            changed: result.changed,
            voters: result.voters,
        }))
    }
    ...
    async fn seal_split_parent(
        &self,
        request: Request<proto::SealSplitParentRequest>,
    ) -> Result<Response<proto::SealSplitParentResponse>, Status> {
        let auth = auth_context(&request)?;
        let reservation = self.reserve(&request, WorkClass::MetadataWrite)?;
        let request = request.into_inner();
        let root = kv9_common::RootDigest::from_bytes(
        ...
        let result = self
            .backend
            .call(reservation, move |backend| {
                backend.seal_split_parent(&caller, root, operation)
            })
            .await?;
        Ok(Response::new(proto::SealSplitParentResponse {
            sealed_version: result.sealed_version,
            changed: result.changed,
            cut_term: result.cut.term,
            cut_index: result.cut.index,
        }))
    ...
    async fn populate_split_child(
        &self,
        request: Request<proto::PopulateSplitChildRequest>,
    ) -> Result<Response<proto::PopulateSplitChildResponse>, Status> {
        let auth = auth_context(&request)?;
        let reservation = self.reserve(&request, WorkClass::MetadataWrite)?;
        let request = request.into_inner();
        let root = kv9_common::RootDigest::from_bytes(
        ...
        let result = self
            .backend
            .call(reservation, move |backend| {
                backend.populate_split_child(&caller, root, operation, request.high)
            })
            .await?;
        Ok(Response::new(proto::PopulateSplitChildResponse {
            rows_copied: result.rows_copied,
            parent_half_digest: result.parent_half_digest.as_bytes().to_vec(),
            child_digest: result.child_digest.as_bytes().to_vec(),
        }))
    }
