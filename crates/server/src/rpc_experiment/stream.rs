...
//! This is an experimental framing adapter, never a mutation/retry coordinator.

use super::{
    Handler, PointRpc, WireMetadata, WireReply, WireRequest, CHANNEL_LIMIT, CONNECTION_LIMIT,
    FRAME_LIMIT,
};
...
use tonic::{
    transport::{server::Connected, Endpoint, Server},
...
pub(super) mod wire {
    tonic::include_proto!("kv9.rpc.experiment");
}
...
impl wire::point_stream_server::PointStream for Service {
    ...
    async fn exchange(
        ...
        let permit = self
            ...
            .clone()
            .try_acquire_owned()
            .map_err(|_| Status::resource_exhausted("experimental stream limit"))?;
                ...
                tokio::select! {
                    ...
                    received = async {
                        ...
                        let frame = match received {
                            ...
                            Err(_) => {
                                reserved.send(Err(Status::unavailable("experimental stream input failed")));
                                break;
                            }
                        };
                        if frame.id <= last_id || frame.operation > 2 ||
                            !(1..=30_000_000).contains(&frame.remaining_micros) {
                            reserved.send(Err(Status::invalid_argument("invalid experimental stream frame")));
                            break;
                        }
                        last_id = frame.id;
                        ...
                        jobs.push(async move {
                            let reply = match timeout_at(deadline, handler.point(
                                tarpc::context::current(), frame.operation as u8,
                                WireRequest { authorization: frame.authorization, payload: frame.payload }
                            )).await {
                                Ok(reply) => reply,
                                Err(_) => WireReply::error(Status::deadline_exceeded("experimental point deadline")),
                            };
                            let response = wire::PointResponse { id: frame.id, reply: Some(reply.into()) };
                            if response.encoded_len() > FRAME_LIMIT {
                                reserved.send(Err(Status::data_loss("experimental response exceeds frame limit")));
                            } else {
...
fn unconfirmed() -> Status {
    Status::unavailable("experimental stream outcome is unconfirmed")
}
...
impl StreamClient {
    ...
    async fn connect(&self) -> Result<Arc<StreamChannel>, Status> {
        ...
        let dispatch = tokio::spawn(async move {
            ...
            while let Ok(Some(frame)) = responses.message().await {
                ...
                let invalid = match &decoded {
                    Ok(reply) => !reply.valid(),
                    Err(status) => {
                        status.code() == tonic::Code::DataLoss
                            || matches!(
                                crate::client::classify_status(status, pending.operation == 0),
    ...
    async fn call<Q: Message>(
        ...
        if remaining == 0 || remaining > 30_000_000 {
            return Err(Status::deadline_exceeded("experimental point deadline"));
        }
        ...
        };
        {
            ...
            let id = frame.id;
            state.pending.insert(id, Pending { operation, reply });
    ...
    pub(crate) async fn raw_get(
        ...
        match self.call(0, request, deadline).await? {
    ...
    pub(crate) async fn raw_put(
        ...
        match self.call(1, request, deadline).await? {
    ...
    pub(crate) async fn raw_delete(
        ...
        match self.call(2, request, deadline).await? {
            Decoded::Write(reply) => Ok(reply),
            _ => Err(unconfirmed()),
        }
    }
